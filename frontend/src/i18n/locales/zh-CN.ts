...
thinkingFormatDeepseek: 'DeepSeek/GLM 格式',
    ...
    internalLlmTimeout: '内部 LLM 超时',
    internalLlmTimeoutHint: '内部 LLM 单次调用从发起到完整结束的总耗时上限（秒）',
...
// 误工分析工具
  ...
  appClock: {
    ...
    lastError: '最近错误',
    actions: '操作',
    clearAndResume: '清除并恢复',
    forceTick: '强制 Tick',
    loadFailed: '加载运行状态失败',
    clearSuccessIdle: '超时状态已清除，调度已恢复',
    clearSuccessRecovering: '超时状态已清除，等待运行中 tick 结束后自动恢复',
    clearFailed: '清除超时状态失败',
    forceTickSuccess: 'Tick 已触发',
    forceTickFailed: '触发 Tick 失败',
    timedOutWarning: '有 {count} 个 App 处于已超时状态，需要管理员手动处理',
  },
}
