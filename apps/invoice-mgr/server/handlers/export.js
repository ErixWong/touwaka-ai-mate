/**
 * 发票管理 - 导出 Excel
 * GET /api/apps/invoice-mgr/export → 导出发票明细（full/custom/negative）
 *
 * URL 映射：/apps/invoice-mgr/export → 本文件
 */
...
export async function get(ctx, deps) {
  ...
  try {
    ...
    if (type === 'full') {
      buffer = await invoiceService.exportFull(params);
      ...
      buffer = await invoiceService.exportCustom({ ...params, fields, includeItems });
      filename = `发票信息个性化导出-${ts}.xlsx`;
    } else if (type === 'negative') {
      buffer = await invoiceService.exportNegative(params);
