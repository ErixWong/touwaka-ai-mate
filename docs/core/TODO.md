...
一个允许 LLM 调用另一个 LLM 的技能，用于：
- **知识过滤**：主 LLM 调用"场外专家"审阅检索结果
- **任务执行**：OCR、图片描述、代码分析等
- **多模型协作**：主模型用便宜的模型，复杂任务调用更强的模型
...
1. **文档清洗专家** 🧹
   - 职责：将各类文档转换为结构化 Markdown
   - 能力：格式转换、VL模型 OCR/描述、媒体上传
   - 技能：kb-clean-file, kb-ocr-image, kb-upload-media

2. **知识整理专家** 📚
   - 职责：将清洗后的文档整理成知识点并入库
   - 能力：归属判断、智能分段、上下文补充、入库管理
   - 技能：kb-analyze-topic, kb-chunk-smart, kb-create-point

**构建流程**：数据清洗 → 知识构造 → 向量化（异步）→ 图谱构建（异步）
