...
【生成阶段】LLM 基于 Psyche 生成回复
    ↓
【可选】LLM 调用 note_take 存入新材料
    ↓
【可选】LLM 调用 note_read 获取 Notes 材料
    ↓
返回用户
```
...
```typescript
// note_take - 存入笔记（兼容别名：notes_take）
{
  "name": "note_take",
...
// note_read - 读取笔记（兼容别名：notes_read）
{
  "name": "note_read",
...
// note_list - 列出笔记（可选，兼容别名：notes_list）
{
  "name": "note_list",
...
// note_forget - 删除笔记（兼容别名：notes_forget）
{
  "name": "note_forget",
  "description": "从当前 Notes 中删除指定笔记",
  "parameters": {
    "key": "string"
  }
}
...
**兼容说明**：
- `note_forget` 用于主动删除笔记（兼容别名 `notes_forget`）
- 更新笔记直接使用 `note_take` 覆盖同名 key（兼容别名 `notes_take`）
...
LLM 看到引用 → 调用 note_read("q1_key_metrics")
  ↓
获取 Q1 数据作为参考
  ↓
LLM 生成回复
...
skillManager.register({
  name: 'notes',
  tools: ['note_take', 'note_read', 'note_list']
});
```
