...
| 原则 | 说明 | 适用场景 |
|------|------|----------|
| **单一状态原则** | 控制同一功能的状态变量只保留一个 | 状态管理、交互控制 |
| **用户意图优先** | 行为由用户操作驱动，而非系统变化驱动 | 滚动、选择、展开/折叠 |
| **高频节流原则** | 高频更新必须使用RAF/防抖/节流 | 流式输出、拖拽、resize |

### 检查点

| 检查项 | 问题信号 | 正确做法 |
|--------|----------|----------|
| **状态变量数量** | 同一功能有2个以上状态变量相互影响 | 合并为单一核心状态 |
| **触发源合理性** | MutationObserver/定时器驱动交互行为 | 由用户事件或业务事件触发 |
| **高频操作处理** | 每次变化都直接执行DOM操作 | RAF节流/防抖/节流 |
| **资源清理** | 无onUnmounted清理异步资源 | 必须清理RAF/定时器/Observer |
| **条件判断完整性** | 无条件执行交互行为 | 检查用户状态后再执行 |

### 反模式警示

| 反模式 | 问题 | 替代方案 |
|--------|------|----------|
| MutationObserver 驱动交互 | DOM变化与用户意图不一致 | 事件驱动 |
| 多状态变量交叉判断 | 状态冲突难以预测 | 单一状态 |
| 高频直接操作DOM | 性能问题、视觉抖动 | RAF节流 |
| 缺少资源清理 | 内存泄漏 | onUnmounted清理 |
