...
class VectorCompressionService {
  ...
  finalizeSegment(segments, points, start, end, globals, currentRange) {
    ...
    const seg = {
      ...
      resolution: parseFloat((maxCur - minCur <= 1e-9 ? 0 : (maxCur - minCur)).toFixed(6)),
  ...
  classifySegment(seg) {
    const absSlope = Math.abs(seg.slope);
    const bw = seg.bandwidth;
    const duration = seg.duration;
    const current = seg.mean_current;

    if (bw < 0.05 && duration < 0.05 && current > 0.5) {
      return 'spike';
    }
    if (absSlope > 1.0) {
      return seg.slope > 0 ? 'rising-fast' : 'falling-fast';
    }
    if (absSlope > 0.2) {
      return seg.slope > 0 ? 'rising' : 'falling';
    }
    if (bw < 0.1) {
      if (current < 0.3) return 'plateau-low';
      if (current < 1.0) return 'plateau-mid';
      return 'plateau-high';
    }
    if (bw > 0.5 && duration < 0.1) return 'burst';
    return 'transition';
  }

  mergeSegmentsWithPolyline(segments, points, absRes, relRes, currentRange) {
    const epsilon = Math.max(absRes * 0.3, relRes * currentRange * 0.3);
    for (const seg of segments) {
      const segPoints = points.slice(seg.start_index, seg.end_index + 1);
      if (segPoints.length <= 2) {
        seg.polyline_points = segPoints.map(([t, c]) => [parseFloat(t.toFixed(6)), parseFloat(c.toFixed(6))]);
        continue;
      }
      if (seg.kind && seg.kind.startsWith('plateau')) {
        const first = segPoints[0];
        const last = segPoints[segPoints.length - 1];
        seg.polyline_points = [
          [parseFloat(first[0].toFixed(6)), parseFloat(first[1].toFixed(6))],
          [parseFloat(last[0].toFixed(6)), parseFloat(last[1].toFixed(6))],
        ];
      } else {
        const simplified = douglasPeucker(segPoints, epsilon);
        seg.polyline_points = simplified.map(([t, c]) => [parseFloat(t.toFixed(6)), parseFloat(c.toFixed(6))]);
      }
    }
    return segments;
  }
