...
export function clearGlobalCache(appId) {
  const keysToDelete = [];
  ...
  logger.info(`[AppRuntimeLoader] Global cache cleared for ${appId}`);
}
...
class AppRuntimeLoader {
  ...
  async validateRuntime(manifest) {
    ...
    if (runtime.server?.routes) {
      ...
      if (!exists) {
        errors.push(`runtime.server.routes path "${runtime.server.routes}" does not exist`);
      }
    }
  ...
  async loadBackupExport(appId) {
    const cacheKey = `backupExport:${appId}`;
  ...
  async loadBackupImport(appId) {
    const cacheKey = `backupImport:${appId}`;
...
export { subscribeGlobalCache, clearGlobalCache };
