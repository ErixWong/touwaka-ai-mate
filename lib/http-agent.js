...
function syncAgentTimeout(agent, timeoutMs) {
  if (!agent || !Number.isFinite(timeoutMs) || timeoutMs <= 0) {
    return agent;
  }

  if (agent.options && agent.options.timeout !== timeoutMs) {
    agent.options.timeout = timeoutMs;
  }
