...
export class PsycheManager {
  ...
  async compress(psyche, maxTokens, userId, expertId) {
    ...
    const stats = {
      ...
      maxTokens,
      warningTokens,
      tempNotesMoved: false,
      notesRefs: {
        initial: psyche.notes_refs.length,
        before: psyche.notes_refs.length,
        after: psyche.notes_refs.length,
        addedFromTempNotes: 0,
        removed: 0,
      },
      topicsContext: { before: psyche.topics_context.length, after: psyche.topics_context.length, removed: 0 },
      keyExchanges: {
        before: psyche.conversation_digest.key_exchanges.length,
        after: psyche.conversation_digest.key_exchanges.length,
        removed: 0,
      },
      overMaxAfterCompression: currentTokens > maxTokens,
    };
    ...
    // 1. 将 working_memory.temp_notes 转为 Notes（Phase 3 阶段二：走 PsycheNotesFacade.take）
    if (psyche.working_memory.temp_notes && this.notesStore && typeof this.notesStore.take === 'function') {
      // wm_ key 同毫秒并发碰撞修复：追加随机后缀（DESIGN-REVIEW §7 风险 2）
      const noteKey = `wm_${Date.now()}_${randomUUID().replace(/-/g, '').slice(0, 8)}`;
      ...
      stats.tempNotesMoved = true;
      stats.notesRefs.addedFromTempNotes += 1;
      stats.notesRefs.after = psyche.notes_refs.length;
    ...
    if (psyche.estimateTokens() > maxTokens) {
      ...
      stats.notesRefs.before = beforeCount;
      psyche.notes_refs = psyche.notes_refs
        .filter(ref => ref.relevance > 0.5)
        .slice(0, this.config.maxNotesRefs);
      stats.notesRefs.after = psyche.notes_refs.length;
      stats.notesRefs.removed = Math.max(0, beforeCount - psyche.notes_refs.length);
    ...
    if (psyche.estimateTokens() > maxTokens) {
      ...
      stats.topicsContext.after = psyche.topics_context.length;
      stats.topicsContext.removed = Math.max(0, beforeCount - psyche.topics_context.length);
    ...
    if (psyche.estimateTokens() > maxTokens) {
      ...
      stats.keyExchanges.after = psyche.conversation_digest.key_exchanges.length;
      stats.keyExchanges.removed = Math.max(0, beforeCount - psyche.conversation_digest.key_exchanges.length);
    ...
    stats.tokenReduction = Math.max(0, currentTokens - finalTokens);
    stats.notesRefs.after = psyche.notes_refs.length;
    stats.topicsContext.after = psyche.topics_context.length;
    stats.topicsContext.removed = Math.max(0, stats.topicsContext.before - psyche.topics_context.length);
    stats.keyExchanges.after = psyche.conversation_digest.key_exchanges.length;
    stats.keyExchanges.removed = Math.max(0, stats.keyExchanges.before - psyche.conversation_digest.key_exchanges.length);
    stats.overMaxAfterCompression = finalTokens > maxTokens;
    this.lastCompressionStats = stats;
    logger.info(`[PsycheManager] 压缩完成: ${currentTokens} → ${finalTokens} tokens`);
    logger.info(`[PsycheManager] compression stats: ${JSON.stringify(stats)}`);
  ...
  getLastCompressionStats() {
    return this.lastCompressionStats ? JSON.parse(JSON.stringify(this.lastCompressionStats)) : null;
  }
}
