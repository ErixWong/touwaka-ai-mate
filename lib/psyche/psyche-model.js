import { estimateTokens as estimateTextTokens } from '../token-utils.js';
...
export class PsycheModel {
  ...
  toText() {
    ...
    // 当前主题（如果没有，显示为"待确定"）
    parts.push(`当前主题：${this.session_meta.current_topic || '待确定'}`);

    // 用户意图
    if (this.session_meta.user_intent) {
      parts.push(`用户意图：${this.session_meta.user_intent}`);
    }
    ...
    parts.push(`工作阶段：${phaseMap[this.methodology.current_phase] || this.methodology.current_phase || '初始化阶段'}`);

    // 方法论
    if (this.methodology.approach) {
      parts.push(`方法论：${this.methodology.approach}`);
    }

    // 下一步行动
    if (this.methodology.next_action) {
      parts.push(`下一步：${this.methodology.next_action}`);
    }

    // 对话轮次
    if (this.session_meta.conversation_round > 0) {
      parts.push(`对话轮次：${this.session_meta.conversation_round}`);
    }

    // 关键对话记录（最近3条）
    if (this.conversation_digest.key_exchanges.length > 0) {
      parts.push('\n关键对话记录：');
      this.conversation_digest.key_exchanges.slice(-3).forEach(exchange => {
        parts.push(`- 第${exchange.round}轮：${exchange.summary}`);
      });
    }
    ...
    if (this.notes_refs.length > 0) {
      parts.push('\n可用笔记（通过 note_read 获取；兼容别名 notes_read）：');
    ...
    // 主题上下文
    if (this.topics_context.length > 0) {
      parts.push('\n相关主题：');
      this.topics_context.slice(0, 3).forEach(t => {
        parts.push(`- ${t.title} (相关度: ${Math.round(t.relevance * 100)}%)`);
      });
    }
   ...
   */
  estimateTokens() {
    return estimateTextTokens(this.toText());
  }
