    ...
    try {
      ...
      for (const doc of pendingDocs) {
        try {
          ...
          const result = await embeddingService.processDocument(doc.id, { allowErrorRetry: true });
          logger.info(
            `[DocEmbeddingWorker] Document ${doc.id}: ${result.status || result.reason || 'unknown'}, ` +
            `${result.success_count || 0}/${result.total_chunks || 0} chunks embedded`
          );
        } catch (error) {
