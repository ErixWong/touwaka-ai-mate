...
class MemorySystem {
  ...
  async compressContext(userId, options = {}) {
    ...
    try {
      ...
      logger.info(`[MemorySystem] 触发压缩: tokenExceeded=${tokenExceeded}, messageExceeded=${messageExceeded}, tokens=${tokens}, messages=${unarchivedMessages.length}`);
...
返回 JSON 格式：
{
  ...
  async getRecentInnerVoices(userId, limit = 3) {
    ...
    return messages
      ...
      .map(m => JSON.parse(m.inner_voice));
  }
