...
class InternalLLMService {
  ...
  _buildThinkingCallOptions(model) {
  ...
  * 提取 JSON 结构化数据
  ...
  async extractJson(systemPrompt, userPrompt, options = {}) {
    ...
    const thinkingConfig = this._buildThinkingCallOptions(model);
    const finalSystemPrompt = appendNoThinkPrompt(systemPrompt, thinkingConfig.append_no_think);
