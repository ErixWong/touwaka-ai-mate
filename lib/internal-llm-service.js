...
class InternalLLMService {
  ...
  /**
   * 获取指定专家的 LLMClient 实例
   * @param {string} expertId - 专家ID
   * @returns {Promise<LLMClient>}
   */
  async getLLMClient(expertId) {
    if (this.llmClientCache.has(expertId)) {
      return this.llmClientCache.get(expertId);
    }
    const llmClient = new LLMClient(this.configLoader, expertId);
    await llmClient.loadConfig();
    this.llmClientCache.set(expertId, llmClient);
    return llmClient;
  }
...
const response = await this.callWithRetry(model, messages, {
  ...
  async call(model, messages, options = {}) {
    ...
    // 获取对应的 LLMClient 实例
    const llmClient = expertId ? await this.getLLMClient(expertId) : await this.getAnyLLMClient();

    // 使用 LLMClient.call() 进行非流式调用
    const result = await llmClient.call(model, finalMessages, {
  ...
  /**
   * 获取任意一个缓存的 LLMClient 实例
   * @returns {Promise<LLMClient>}
   */
  async getAnyLLMClient() {
    const first = this.llmClientCache.values().next().value;
    if (first) return first;
    throw new Error('No LLMClient available. Call getLLMClient(expertId) first.');
  }
