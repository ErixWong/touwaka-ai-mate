...
class InternalLLMService {
  ...
  async judge(systemPrompt, userPrompt, options = {}) {
    ...
    const truncated = this.truncateForContext(model, systemPrompt, userPrompt);

    const messages = [
      { role: 'system', content: truncated.systemPrompt },
      { role: 'user', content: truncated.userPrompt },
    ];
  ...
  async call(model, messages, options = {}) {
    const requestBody = JSON.stringify({
      ...
      ...(options.enableThinking === false && { extra_body: { chat_template_kwargs: { enable_thinking: false } } }),
      ...(options.enableThinking === true && {
        extra_body: {
          ...(options.thinkingBudget ? { thinking_budget: options.thinkingBudget } : {}),
        },
      }),
    });
