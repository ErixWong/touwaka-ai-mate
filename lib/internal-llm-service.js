  ...
  async call(model, messages, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      const req = httpModule.request(requestOptions, (res) => {
          ...
          for (const line of lines) {
            if (line.startsWith('data: ')) {
              ...
              try {
                ...
                logger.debug('[InternalLLMService] Chunk parse error (ignored):', e.message);
              }
            }
          }
        });
        ...
        res.on('close', () => {
          logger.warn('[InternalLLMService] Response stream closed prematurely');
        });

        res.on('end', () => {
          const contentLength = content.length;
          const hasCompleteJson = content.includes('}') && content.includes('{');
          logger.info(`[InternalLLMService] Response complete: length=${contentLength}, hasJsonBraces=${hasCompleteJson}, model=${model.model_name}`);
