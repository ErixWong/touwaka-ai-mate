...
class DocumentIntakeService {
  ...
  async createIntakeDocument({ appId, collectionId, schemaId, attachments, userId }) {
    ...
    const document = await this.db.sequelize.transaction(async (t) => {
      ...
      await this.models.DocVersion.create({
        ...
        // "当前在用"的判定字段统一为 documents.current_revision_id（+ 派生 is_current）。
  ...
  async createIntakeRevision({ documentId, attachments, userId, revisionLabel, changeSummary }) {
    ...
    // 同一事务内完成：版本创建 + is_current 同步 + 文档当前版本切换 + 附件归属，
    // 避免 revision 已创建但后续步骤失败留下孤儿版本。
    const revision = await this.db.sequelize.transaction(async (t) => {
      const created = await revisionService.createRevision(documentId, userId, {
        revision_label: revisionLabel || undefined,
        change_summary: changeSummary || 'Document version upload',
        transaction: t,
      });

      // 统一通过 DocumentRevisionService 同步 is_current 派生字段
      await revisionService._syncIsCurrentFlags(documentId, created.id, t);

      // 两态语义：新版本上传即成为当前在用（effective），旧 effective 版本归回 draft
