...
class DocRecallService {
  ...
  ensureModels() {
    if (!this.models.DocDocument) {
      ...
      this.models.KnowledgeBase = this.db.getModel('knowledge_base');
      this.models.KbParagraph = this.db.getModel('kb_paragraph');
    }
  }
  ...
  async recall(query, options = {}) {
    ...
    try {
      ...
      return await this.recallFromNewTable(vectorJson, {
        scope, doc_types, top_k, threshold, userId, org_id,
      });
  ...
  async recallFromNewTable(vectorJson, options) {
    const { scope, doc_types, top_k, threshold, userId, org_id } = options;

    const docTypeFilter = this.buildDocTypeFilter(scope, doc_types);
    const orgFilter = org_id ? `AND d.org_id = '${org_id}'` : '';
  ...
  buildDocTypeFilter(scope, doc_types) {
    if (doc_types && doc_types.length > 0) {
      return `AND d.doc_type IN (${doc_types.map(t => `'${t}'`).join(',')})`;
    }
  ...
  async getAccessibleKbIds(userId) {
    const kbs = await this.models.KnowledgeBase.findAll({
      attributes: ['id'],
      where: {
