...
async _getDefaultEmbeddingModelId() {
    try {
      const AiModel = this.db.getModel('ai_model');
      if (!AiModel) {
        logger.warn('[DocRecall] ai_model not available');
        return null;
      }
  ...
  async _getEmbeddingClient(modelId) {
    ...
    const defaultModelId = await this._getDefaultEmbeddingModelId();
    if (defaultModelId) {
  ...
  async generateQueryEmbedding(query) {
    const client = await this._getEmbeddingClient();
    if (!client) {
