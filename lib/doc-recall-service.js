 ...
 *
 * 提供统一的文档检索能力，支持：
 * - 新表（doc_documents, doc_chunks）检索
...
class DocRecallService {
  ...
  async recallFromNewTable(vectorJson, options) {
    ...
    const results = await this.db.sequelize.query(`
      ...
      FROM doc_chunks c
      JOIN doc_versions v ON c.version_id = v.id
      JOIN doc_documents d ON v.document_id = d.id
   ...
   */
  async expandContext(versionId, centerSeq, windowSize = 2) {
    this.ensureModels();
    const chunks = await this.models.DocChunk.findAll({
      ...
      order: [['seq', 'ASC']],
      attributes: ['id', 'seq', 'title', 'content', 'chunk_type', 'chapter_title', 'section_title'],
    });
    return chunks.map(c => ({
      seq: c.seq,
      title: c.title,
