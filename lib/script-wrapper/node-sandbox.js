...
if (!['.js', '.cjs'].includes(ext)) {
  console.error(`Script must be .js or .cjs: ${SCRIPT_PATH}`);
  process.exit(1);
}
...
const safeRequire = (moduleName) => {
  if (moduleName.startsWith('./') || moduleName.startsWith('../')) {
    const resolved = userRequire.resolve(moduleName);
    const resolvedLower = resolved.toLowerCase();
    const sandboxRootLower = SANDBOX_ROOT_RESOLVED.toLowerCase();
    if (!resolvedLower.startsWith(sandboxRootLower + path.sep.toLowerCase()) &&
        resolvedLower !== sandboxRootLower) {
