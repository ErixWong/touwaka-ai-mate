...
class DocumentChunkService {
  constructor(db, options = {}) {
    ...
    this.documentReadService = new DocumentReadService(db);
  }
  ...
  async generate(revisionId, options = {}) {
    ...
    if (doc.processing_status !== 'pending_chunk') {
      throw new Error(`Document must be in pending_chunk state (current: ${doc.processing_status})`);
    }
    ...
    const { runId } = await this.advancer.enterStage(revision.document_id, 'pending_chunk', {
  ...
  async _loadRevisionText(revision) {
    return await this.documentReadService.loadRevisionText(revision);
  }
