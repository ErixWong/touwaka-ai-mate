    ...
    try {
      ...
      if (result && result.pending) {
        await AppActionLog.create({
          ...
          result_status: state.name,
          success: false,
          error_message: result.error || 'Task pending',
          duration,
        });
        logger.info(`[AppClock] Record ${record.id} pending in ${state.name}: ${result.error || 'Task pending'} (${duration}ms)`);
        return;
      }
