...
class AppClock {
  constructor(db, config = {}) {
    ...
    this.residentCapabilityExecutor = config.residentCapabilityExecutor || null;
  ...
  async initializeRunTable() {
    try {
      const AppTickRun = this.db.getModel('app_tick_run');
      if (!AppTickRun) {
        logger.warn('[AppClock] app_tick_run model not available, skipping startup cleanup');
        return;
      }
      await AppTickRun.update(
        {
          finished_at: new Date(),
          status: 'interrupted_by_restart',
          final_message: 'Interrupted by server restart before tick settled',
        },
        { where: { finished_at: null } }
      );
      logger.info('[AppClock] Startup cleanup: unclosed tick records settled to interrupted_by_restart');
    } catch (error) {
      logger.warn('[AppClock] Startup cleanup failed (non-fatal):', error.message);
    }
  }
  ...
  async wakeNext() {
    ...
    if (!MiniApp) {
      logger.warn('[AppClock] mini_app model not available');
      return;
    }
  ...
  async invokeTick(entry) {
    ...
    const runId = Utils.newID(20);
    if (AppTickRun) {
      try {
        await AppTickRun.create({
          id: runId,
          app_id: entry.app_id,
        ...
        logger.error(`[AppClock] Failed to insert app_tick_run for ${entry.app_id}, aborting tick: ${err.message}`);
        await AppTickLog.create({
          id: Utils.newID(20),
          registry_id: entry.id,
          app_id: entry.app_id,
          success: false,
          error_message: `app_tick_run insert failed: ${err.message}`,
          duration: Date.now() - startTime,
        }).catch(logErr => {
          logger.warn(`[AppClock] Failed to write tick_log for ${entry.app_id}: ${logErr.message}`);
        });
    ...
    try {
          ...
          await AppTickRun.update(
            {
              finished_at: new Date(),
              status: 'success',
              final_message: `Tick completed in ${duration}ms`,
            },
            { where: { id: runId } }
          );
        } catch (err) {
          logger.warn(`[AppClock] Failed to update app_tick_run for ${entry.app_id}: ${err.message}`);
        }
      }
      ...
      if (AppTickRun) {
        try {
          ...
          logger.warn(`[AppClock] Failed to update app_tick_run for ${entry.app_id}: ${updateErr.message}`);
        }
      }
  ...
  async getRunStatus() {
    ...
    try {
      const rows = await AppTickRun.findAll({
        where: { finished_at: null },
      ...
      logger.warn(`[AppClock] Failed to query run status: ${err.message}`);
    }

    return result;
  }

  async getRunStatusForApp(appId) {
    const AppTickRun = this.db.getModel('app_tick_run');
    ...
    try {
      const row = await AppTickRun.findOne({
        where: { app_id: appId, finished_at: null },
        raw: true,
      });
      ...
      logger.warn(`[AppClock] Failed to query run status for ${appId}: ${err.message}`);
      return null;
    }
  }
  ...
  async getRunHistory(appId, limit = 10) {
    ...
    try {
      ...
      return rows.map(row => ({
        ...
        status: row.status,
        duration_ms: row.started_at && row.finished_at
          ? new Date(row.finished_at).getTime() - new Date(row.started_at).getTime()
          : null,
        final_message: row.final_message,
      }));
    } catch (err) {
      logger.warn(`[AppClock] Failed to query run history: ${err.message}`);
      return [];
    }
  }

  async forceTick(appId) {
    const AppClockRegistry = this.db.getModel('app_clock_registry');
    const MiniApp = this.db.getModel('mini_app');
    
    if (!AppClockRegistry) {
      throw new Error('app_clock_registry model not available');
    }

    if (!MiniApp) {
      throw new Error('mini_app model not available');
    }

    const entry = await AppClockRegistry.findOne({
      where: { app_id: appId },
      include: [{
        model: MiniApp,
        as: 'app',
    ...
    if (!entry) {
      throw new Error(`App ${appId} not found in clock registry or not active`);
    }

    if (this.runningApps.has(appId)) {
      throw new Error(`App ${appId} is currently running`);
    }
  ...
  buildContext(app, entry) {
    ...
    const documentCleanService = new DocumentCleanService(this.db, {
      getDocPipelineConfig,
    });
    ...
    return {
      ...
      registry: entry?.toJSON ? entry.toJSON() : entry,
      documentOcrService,
      ...
      services: {
        ...
        documentClean: documentCleanService,
  ...
  async validateTickScripts() {
    ...
    if (!MiniApp) {
      logger.warn('[AppClock] mini_app model not available, skipping tick script validation');
      return;
    }
    ...
    for (const entry of entries) {
      const miniAppActive = entry.app?.is_active === true;
      
      try {
        const runtimeInfo = await this.runtimeLoader.getAppRuntimeInfo(entry.app_id);
        
        if (runtimeInfo.hasTick) {
          if (!miniAppActive) {
            logger.info(`[AppClock] App "${entry.app_id}" tick exists but mini_app is inactive, keeping clock disabled`);
            if (entry.is_active !== false) {
              await entry.update({ is_active: false });
            }
          } else if (entry.is_active === false) {
            logger.info(`[AppClock] App "${entry.app_id}" tick exists and mini_app is active, auto-reactivating clock (previously disabled by validation)`);
            await entry.update({ is_active: true });
          } else {
            logger.info(`[AppClock] App "${entry.app_id}" tick validated via runtime (hasTick=true)`);
          }
          continue;
        }
        ...
        if (fs.existsSync(indexPath)) {
          if (!miniAppActive) {
            logger.info(`[AppClock] App "${entry.app_id}" legacy tick exists but mini_app is inactive, keeping clock disabled`);
            if (entry.is_active !== false) {
              await entry.update({ is_active: false });
            }
          } else if (entry.is_active === false) {
            logger.info(`[AppClock] App "${entry.app_id}" legacy tick exists and mini_app is active, auto-reactivating clock`);
            await entry.update({ is_active: true });
          } else {
            logger.info(`[AppClock] App "${entry.app_id}" tick validated via legacy path: ${indexPath}`);
          }
        } else {
          logger.warn(`[AppClock] App "${entry.app_id}" has no tick (runtime or legacy), marking inactive`);
          await entry.update({ is_active: false });
        }
      } catch (err) {
        logger.warn(`[AppClock] Failed to validate tick for "${entry.app_id}": ${err.message}`);
        ...
        if (fs.existsSync(indexPath)) {
          if (!miniAppActive) {
            logger.info(`[AppClock] App "${entry.app_id}" fallback tick exists but mini_app is inactive, keeping clock disabled`);
            if (entry.is_active !== false) {
              await entry.update({ is_active: false });
            }
          } else if (entry.is_active === false) {
            logger.info(`[AppClock] App "${entry.app_id}" fallback tick exists and mini_app is active, auto-reactivating clock`);
            await entry.update({ is_active: true });
          }
        } else {
          logger.warn(`[AppClock] App "${entry.app_id}" has no tick (runtime or legacy), marking inactive`);
  ...
  async loadTickScript(entry, app) {
    try {
      const tickModule = await this.runtimeLoader.loadTick(entry.app_id);
      if (!tickModule) {
        throw new Error(`No tick module found for ${entry.app_id}`);
      }
      return tickModule;
    } catch (e) {
      logger.warn(`[AppClock] RuntimeLoader failed for ${entry.app_id}, falling back to legacy loader: ${e.message}`);
      ...
      try {
        ...
        throw new Error(`Cannot load tick script: ${normalizedPath} - ${fallbackErr.message}`);
      }
    }
  }
  ...
  async callSkill(name, method, params) {
    ...
    if (this.residentCapabilityExecutor?.getByName(name, method)) {
      return await this.residentCapabilityExecutor.invoke({
        skillId: name,
        toolName: method,
        params,
