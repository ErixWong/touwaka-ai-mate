...
class AppClock {
  ...
  async wakeNext() {
    ...
    if (this.timedOutApps.has(entry.app_id)) {
      logger.warn(`[AppClock] Skip app ${entry.app_id}: in timed_out state, awaiting manual recovery`);
      return;
    }
  ...
  async invokeTick(entry) {
    ...
    this._updateRunStatus(entry.app_id, 'running', {
    ...
    try {
      ...
      tickPromise = scriptModule.tick(context);
      ...
      const result = await Promise.race([
        ...
        new Promise((_, reject) => {
          setTimeout(() => {
            isTimedOut = true;
            reject(new Error(`Tick timeout after ${this.tickTimeoutMs}ms`));
          }, this.tickTimeoutMs);
        }),
      ]);
      ...
      this.timedOutApps.delete(entry.app_id);
      this._updateRunStatus(entry.app_id, 'idle', {
        started_at: null,
      ...
      if (isTimedOut) {
        this.timedOutApps.add(entry.app_id);
        this._updateRunStatus(entry.app_id, 'timed_out', {
        ...
        if (tickPromise) {
          tickPromise.then(
            () => {
              logger.info(`[AppClock] App ${entry.app_id} tick resolved after timeout, result discarded`);
              this.runningApps.delete(entry.app_id);
              this._onTickSettleAfterTimeout(entry.app_id);
            },
            (lateErr) => {
              logger.error(`[AppClock] App ${entry.app_id} tick failed after timeout: ${lateErr.message}`);
              this.runningApps.delete(entry.app_id);
              this._onTickSettleAfterTimeout(entry.app_id);
            }
          );
        ...
        logger.error(`[AppClock] App ${entry.app_id} tick timed out after ${duration}ms, entering manual recovery state`);
        return;
      }
  ...
  _updateRunStatus(appId, runStatus, fields = {}) {
    const existing = this.runStatus.get(appId) || {
      app_id: appId,
      run_status: 'idle',
      started_at: null,
      last_error: null,
  ...
  getRunStatus() {
    const result = [];
    for (const [appId, status] of this.runStatus) {
      const duration = status.started_at
        ? Date.now() - new Date(status.started_at).getTime()
        : null;
      result.push({
        ...status,
  ...
  getRunStatusForApp(appId) {
    const status = this.runStatus.get(appId);
    if (!status) return null;
    const duration = status.started_at
      ? Date.now() - new Date(status.started_at).getTime()
      : null;
    return { ...status, duration_ms: duration };
  }

  clearTimedOut(appId) {
    this.timedOutApps.delete(appId);
    this.appFailures.delete(appId);
    const alreadySettled = !this.runningApps.has(appId);
    const targetStatus = alreadySettled ? 'idle' : 'recovering';
    this._updateRunStatus(appId, targetStatus, {
      last_timeout_at: null,
      manual_clear_required: false,
      ...(alreadySettled ? { started_at: null } : {}),
    });
    logger.info(`[AppClock] App ${appId} timed_out state cleared by admin, target=${targetStatus}${alreadySettled ? ' (old tick already settled)' : ' (waiting for running tick to settle)'}`);
    return targetStatus;
  }

  _onTickSettleAfterTimeout(appId) {
    const current = this.runStatus.get(appId);
    if (current?.run_status === 'recovering') {
      this._updateRunStatus(appId, 'idle', {
        started_at: null,
        last_timeout_at: null,
      });
      logger.info(`[AppClock] App ${appId} old tick settled, transitioned recovering → idle`);
    }
  }
