...
class AppClock {
  ...
  async processRecord(state, record) {
    ...
    try {
      ...
      const filesWithAttachment = files.map(f => {
        const json = f.toJSON();
        return json;
      });
      ...
      result = await processFn(context);
  ...
  async callMcp(serviceName, method, params) {
    ...
    // TODO: integrate with MCP service manager
    throw new Error(`MCP service "${serviceName}" not configured`);
  }
  ...
  async callSkill(skillName, method, params) {
    ...
    throw new Error(`Skill "${skillName}" not configured`);
  }
}
