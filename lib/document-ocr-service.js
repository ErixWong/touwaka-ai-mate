...
class DocumentOcrService {
  ...
  async finalizeCompletedTask(ctx, ocrResult, options = {}) {
    ...
    const rawResultAttachment = await this.createTextAttachment(ctx.revision.id, ctx.revision.created_by || null, 'ocr-raw-result.json', JSON.stringify(defaultDeliverable, null, 2), 'application/json');
    const deliverablesManifestAttachment = await this.createTextAttachment(ctx.revision.id, ctx.revision.created_by || null, 'ocr-deliverables.json', JSON.stringify(deliverables, null, 2), 'application/json');
    const imageManifestAttachment = await this.createTextAttachment(ctx.revision.id, ctx.revision.created_by || null, 'ocr-images.json', JSON.stringify(imageDeliverables, null, 2), 'application/json');
