...
class DocumentOcrService {
  ...
  async probeGatewayTask(taskId) {
    ...
    try {
      ...
      const matched = tasks.find(t => t?.task_id === taskId);
    ...
    if (statusResult.status === 'completed') {
      try {
        ...
        imageCount = artifacts.filter(a => a?.artifact_type === 'image_file').length;
      } catch (error) {
    ...
    const existing = await DocOcrResult.findOne({
  ...
  async _importTaskArtifacts(documentId, taskId) {
    ...
    const timeoutMs = finalizeConfig?.mcp?.timeout_ms || 300000;
  ...
  async syncTaskStatus(documentId, options = {}) {
    ...
    const finalizeGuard = this._checkFinalizeGuard(ocrResult);
    if (finalizeGuard.skip) {
      logger.info(`[DocumentOcrService] sync: document=${documentId} ${finalizeGuard.reason}, skipping re-finalize`);
      if (ctx.document?.processing_status === 'ocr_processing') {
        await this.advancer.finishStage(documentId, 'ocr_processing', {
          runId,
          nextStage: 'pending_clean',
          message: 'OCR already finalized, advancing stage',
          metadata: {
            ocr_result_id: ocrResult.id,
            task_id: ocrResult.task_id,
          },
      ...
      return { ocrResult, statusResult, completed: true };
    }

    await this.assertDocumentNotDeleted(documentId);
    await this._markFinalizeInProgress(ocrResult);
    let finalized;
    try {
      finalized = await this.finalizeCompletedTask(ctx, ocrResult, options);
    } finally {
      await ocrResult.update({
        metadata: this.buildMergedMetadata(ocrResult.metadata, {
  ...
  _checkFinalizeGuard(ocrResult) {
    if (ocrResult.main_markdown_attachment_id) {
      return { skip: true, reason: `already finalized (markdown=${ocrResult.main_markdown_attachment_id})` };
    }
    const metadata = this.toPlainObject(ocrResult.metadata);
    if (metadata.finalize_in_progress === true) {
      const requestedAtMs = Date.parse(metadata.finalize_requested_at || '');
      const stale = Number.isNaN(requestedAtMs) || (Date.now() - requestedAtMs) > 10 * 60 * 1000;
      if (!stale) {
        return { skip: true, reason: 'finalize already in progress' };
      }
      logger.warn(`[DocumentOcrService] finalize_in_progress flag is stale, allowing re-finalize for ocr_result=${ocrResult.id}`);
    }
    return { skip: false };
  }

  async _markFinalizeInProgress(ocrResult) {
    await ocrResult.update({
      metadata: this.buildMergedMetadata(ocrResult.metadata, {
        finalize_in_progress: true,
        finalize_requested_at: new Date().toISOString(),
      }),
    });
  }
