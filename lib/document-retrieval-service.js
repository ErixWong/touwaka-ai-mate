/**
 * Document Retrieval Service - 文档检索统一编排入口
 *
 * 这是面向业务调用的文档检索统一入口，内部编排完整的 document-first 检索链路：
 *
 *   DocumentQueryDecisionService  →  查询意图决策
 *        ↓
 *   DocumentSearchService        →  文档级候选检索
 *        ↓
 *   DocRecallService             →  文档内 chunk 证据召回
 *        ↓
...
class DocumentRetrievalService {
    ...
    try {
      ...
      if (sufficiency === 'none' || sufficiency === 'weak') {
        if (allow_fallback && decision.anchor_strength !== 'strong') {
          ...
          if (fallbackPacket) {
            this._logMetrics(traceId, RETRIEVAL_STRATEGY.DOCUMENT_FIRST, startTime, true);
        ...
        this._logMetrics(traceId, RETRIEVAL_STRATEGY.DEGRADE, startTime);
        return { packet, strategy: RETRIEVAL_STRATEGY.DEGRADE, metrics: this.metrics };
      }

      // 成功路径
      this.metrics.document_first_count++;
      this._logMetrics(traceId, RETRIEVAL_STRATEGY.DOCUMENT_FIRST, startTime);
  ...
  async _handleChunkFirstFallback(query, options, decision, traceId, startTime) {
    ...
    this._logMetrics(traceId, RETRIEVAL_STRATEGY.CHUNK_FIRST_FALLBACK, startTime);
  ...
  async _tryFallbackSupplement(query, options, decision, traceId, existingPacket) {
    try {
      ...
      const mergedPacket = this.packer.pack(
        [], // 不传 candidates，让 packer 从 evidence 中提取文档信息
        newItems,
        decision,
        traceId
      );
  ...
  _logMetrics(traceId, strategy, startTime, fallbackSupplemented = false) {
    const latency = Date.now() - startTime;
    logger.info('[DocRetrieval] Completed:', {
      trace_id: traceId,
      strategy,
      latency_ms: latency,
      fallback_supplemented: fallbackSupplemented,
      metrics: this.metrics,
    });
  }
