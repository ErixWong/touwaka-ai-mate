...
/**
 * 将 LLM 提取的章节标题重新锚定到文本真实行号。
 *
 * LLM 不擅长数行号（尤其遇到整行几千字符的 HTML 表格时），from_line/to_line
 * 经常整体漂移。本函数只信任 LLM 找出的标题文本与顺序，行号通过文本匹配重新定位：
 * - 优先精确匹配，其次前缀/包含匹配；
 * - 多个候选行时取与 LLM 声称行号最近者（LLM 的行号虽不准但通常不离谱）；
 * - 匹配失败时保留 LLM 行号并标记 anchored=false；
 * - 最后强制行号单调递增，并按"下一标题前一行"重算 to_line。
 */
export function reanchorOutlines(outlines, text) {
  ...
  const anchored = [];
  for (const o of outlines) {
    const title = normalizeHeadingText(o?.title);
    const claimed = Number.isFinite(o?.from_line) ? o.from_line : 0;
    let bestLine = -1;
    ...
    if (title) {
      for (let i = 0; i < normalizedLines.length; i++) {
        ...
        if (!ln) continue;
        let score = 0;
        if (ln === title) score = 100;
        else if (ln.startsWith(title)) score = 80;
        else if (title.startsWith(ln) && ln.length >= 4) score = 60;
        else if (title.length >= 6 && ln.includes(title)) score = 40;
        if (score === 0) continue;
