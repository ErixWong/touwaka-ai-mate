...
class DocPipelineAdvancer {
  ...
  /**
   * 校验 document 当前阶段与期望阶段一致
   * 若不一致且未设置 forceStageTransition，抛出错误
   */
  ...
  /**
   * 当未提供 runId 时自动关闭该阶段的 running 记录
   * 返回关闭的 runId（如果有），供调用方用于 metadata 补充
   */
  async _autoCloseRunningRun(documentId, stage, resultStatus, runId, message) {
    if (runId) {
      await this._finishRunRecord(runId, resultStatus, message);
      return runId;
    }
    return await this._closeRunningStage(documentId, stage, resultStatus, message);
  }
   ...
   * 阶段成功结束
   *
   * - 校验当前阶段与预期一致（可 forceStageTransition 跳过）
   * - 推进 documents 到 nextStage
   * - 更新 current_stage_started_at / processing_updated_at
   * - 将对应的 run 记录更新为 ok（若未提供 runId 则自动查找并关闭）
   *
   * @param {string} documentId
   * @param {string} stage - 当前阶段（用于一致性校验和日志语义）
   * @param {object} [result]
   ...
   * @param {boolean} [result.forceStageTransition] - 跳过阶段一致性校验
  ...
  async finishStage(documentId, stage, result = {}) {
    ...
    await this._autoCloseRunningRun(documentId, stage, 'ok', result.runId, message);
   ...
   * - 将对应的 run 记录更新为 nok（若未提供 runId 则自动查找并关闭）
   *
   * @param {string} documentId
   ...
   * @param {boolean} [error.forceStageTransition] - 跳过阶段一致性校验
  ...
  async failStage(documentId, stage, error = {}) {
    ...
    await this._autoCloseRunningRun(documentId, stage, 'nok', error.runId, message);
   ...
   * - 设置 documents 为 error 状态
   * - 错误码标记为 timeout
   * - 将对应的 run 记录更新为 nok，metadata 标记 timeout: true（若未提供 runId 则自动查找并关闭）
   *
   * @param {string} documentId
   * @param {string} stage
   * @param {object} [context]
   ...
   * @param {boolean} [context.forceStageTransition] - 跳过阶段一致性校验
  ...
  async timeoutStage(documentId, stage, context = {}) {
    ...
    await this._autoCloseRunningRun(documentId, stage, 'nok', context.runId, message);
   ...
   * - 设置 documents 为 error 状态
   * - 错误码标记为 cancelled
   * - 将对应的 run 记录更新为 nok，metadata 标记 cancelled: true（若未提供 runId 则自动查找并关闭）
   *
   * @param {string} documentId
   * @param {string} stage
   * @param {object} [context]
