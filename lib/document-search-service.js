...
import { sortRevisionList } from './doc-version-utils.js';
...
class DocumentSearchService {
  ...
  async search(query, options = {}) {
    ...
    try {
      ...
      // 8. 附加版本列表，支撑"该文档有几个版本/有哪些版本号"类问题
      await this._attachVersions(candidates);
  ...
  /**
   * 为候选文档批量附加版本列表（versions + version_count）
   * 排序与版本管理面板一致：年份优先 → revision_no → created_at
   */
  async _attachVersions(candidates) {
    if (!candidates || candidates.length === 0) return;

    const documentIds = [...new Set(candidates.map(c => c.document_id).filter(Boolean))];
    if (documentIds.length === 0) return;

    const placeholders = documentIds.map(() => '?').join(',');
    try {
      const rows = await this.db.sequelize.query(`
        SELECT id, document_id, revision_no, revision_label, revision_status, is_current, created_at
      ...
      for (const c of candidates) {
        const versions = sortRevisionList(byDoc.get(c.document_id) || []);
        c.version_count = versions.length;
        c.versions = versions;
      }
    } catch (error) {
      logger.warn('[DocSearch] Attach versions failed:', error.message);
    }
  }
