...
class DocumentSearchService {
  ...
  async getDocumentInfo(documentIds) {
    ...
    // 2. 按文档ID批量获取关联的 attachment 文件名（通过 OCR 结果链路）
    // 路径：documents.id → doc_ocr_results.document_id → 取所有 attachment 关联字段 → attachments.file_name
    let attachmentMap = new Map();
    try {
      const attachRows = await this.db.sequelize.query(`
        SELECT
          ocr.document_id,
          a.file_name,
