...
const DOC_PIPELINE_DEFAULTS = {
  ...
  pending_embedding: {
    ...
    retry_times: 3,
    // NOTE: pending_embedding 由 document-embedding-worker 后台任务扫描处理，
    // embedding_timeout_ms 为该 worker 中每次 embedding 请求的超时（毫秒）
...
function normalizeStageConfig(stageConfig, stageKey) {
  ...
  if (stageKey === 'pending_outline') {
    ...
    if (normalized.timeout_ms == null) {
      normalized.timeout_ms = timeoutValue;
    }
    return normalized;
  }
