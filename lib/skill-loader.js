...
class SkillLoader {
  ...
  convertToolToOpenAIFormat(toolRow, skill) {
    ...
    // 使用 toolName__skillId后4位 格式作为工具名称（LLM友好，保持唯一性）
    const toolId = toolRow.id;  // 保留数据库 ID 用于内部引用
    const skillIdShort = skill.id.slice(-4);  // 取后4位，保证唯一性
    const toolFunctionName = `${toolRow.name}__${skillIdShort}`;  // 如 "zip__8h90"
  ...
  buildSkillEnvironment(skillId, config, sourcePath = null, scriptPath = 'index.js', userContext = {}, packageWhitelist = null, timeoutConfig = null) {
    ...
    const baseEnv = {
      ...
      API_BASE: process.env.API_BASE || 'http://localhost:3000',  // 11. API 基础地址
      WORKING_DIRECTORY: workingDirectory || '',         // 12. 工作目录（相对 DATA_BASE_PATH）
      PROJECT_ROOT: process.cwd(),                       // 13. 项目根目录（供管理员访问）
      ...configEnv,               // 14. 展开的配置环境变量
  ...
  parseToolsFromMarkdown(markdown, skill) {
    ...
    for (const match of toolMatches) {
      ...
      // 从 Markdown 解析的工具使用 toolName__skillId后4位 格式（与 convertToolToOpenAIFormat 一致）
      const skillIdShort = skill.id.slice(-4);  // 取后4位，保证唯一性
      const toolFunctionName = `${toolName}__${skillIdShort}`;
