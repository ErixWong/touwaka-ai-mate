...
class SkillLoader {
  ...
  async executeSkillTool(skillId, toolName, params, context = {}, scriptPath = 'index.js') {
    return new Promise(async (resolve, reject) => {
      ...
      // 构建该技能的最小化环境变量（安全隔离）
      const env = this.buildSkillEnvironment(skillId, config, sourcePath, scriptPath, context);
  ...
  buildSkillEnvironment(skillId, config, sourcePath = null, scriptPath = 'index.js', userContext = {}) {
    ...
    const RESERVED_ENV_VARS = ['SKILL_ID', 'DATA_BASE_PATH', 'SKILL_CONFIG', 'NODE_OPTIONS', 'SCRIPT_PATH', 'USER_ACCESS_TOKEN', 'USER_ID'];
