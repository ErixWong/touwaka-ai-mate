...
class SkillLoader {
  ...
  async executeSkillTool(skillId, toolName, params, context = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      logger.info(`[SkillLoader] 技能环境 DATA_BASE_PATH: ${env.DATA_BASE_PATH}`);
  ...
  buildSkillEnvironment(skillId, config, sourcePath = null) {
    ...
    const RESERVED_ENV_VARS = ['SKILL_ID', 'DATA_BASE_PATH', 'SKILL_CONFIG', 'NODE_OPTIONS'];
    ...
    // 2. 如果 source_path 以 "skills/" 开头，基于 dataBasePath
    // 3. 否则 source_path 只是技能目录名，拼接 skillsBasePath
    let skillPath;
    if (sourcePath) {
      if (path.isAbsolute(sourcePath)) {
        // 绝对路径，直接使用
        skillPath = sourcePath;
      } else if (sourcePath.startsWith('skills/')) {
        // 相对于 dataBasePath 的路径（如 "skills/file-operations"）
        skillPath = path.join(dataBasePath, sourcePath);
      } else {
        // source_path 只是技能目录名（如 "file-operations"），拼接 skillsBasePath
        skillPath = path.join(this.skillsBasePath, sourcePath);
      }
    } else {
      // 没有 source_path，使用 skillId 作为目录名
    ...
    return {
      ...
      DATA_BASE_PATH: dataBasePath,  // 4. 数据基础路径（技能目录为 DATA_BASE_PATH/skills）
