...
class SkillLoader {
  ...
  async executeSkillTool(skillId, toolName, params, context = {}, scriptPath = 'index.js') {
    return new Promise(async (resolve, reject) => {
      ...
      // 构建该技能的最小化环境变量（安全隔离）
      const env = this.buildSkillEnvironment(skillId, config, sourcePath, scriptPath);
   ...
   */
  buildSkillEnvironment(skillId, config, sourcePath = null, scriptPath = 'index.js') {
    // 最小化系统环境变量白名单（仅保留必要的）
    const allowedSystemVars = ['PATH', 'NODE_ENV', 'HOME', 'TMPDIR', 'LANG', 'TZ', 'INTERNAL_API_SECRET', 'KB_API_BASE'];
    ...
    const RESERVED_ENV_VARS = ['SKILL_ID', 'DATA_BASE_PATH', 'SKILL_CONFIG', 'NODE_OPTIONS', 'SCRIPT_PATH'];
