 ...
 *
 * 架构：
 * - 文件系统模式（推荐用于 Docker 共享卷）: /shared/skills/{skillId}/index.js 或 index.py
...
class SkillLoader {
  ...
  async loadSkillsForExpert(expertId) {
    ...
    logger.debug(`[SkillLoader] 从数据库获取到 ${skillRows.length} 个技能记录 for expert ${expertId}`);
    skillRows.forEach((row, i) => {
      logger.debug(`[SkillLoader] 技能记录 ${i + 1}: id=${row.id}, name=${row.name}, source_type=${row.source_type}, source_path=${row.source_path}`);
    });

    const skills = [];
    for (const row of skillRows) {
      if (isRetiredSkill(row)) {
        logger.info(`[SkillLoader] 跳过已退役技能: ${row.name || row.id}`);
        continue;
      }
      try {
    ...
    logger.debug(`[SkillLoader] 专家 ${expertId} 最终加载了 ${skills.length} 个技能`);
    return skills;
  }
  ...
  async loadSkillTools(skill) {
    try {
      ...
      logger.debug(`[SkillLoader] 从 skill_tools 表加载了 ${toolRows.length} 个工具 for skill ${skill.id}（已过滤驻留工具）`);
  ...
  async executeSkillTool(skillId, toolName, params, context = {}, scriptPath = 'index.js') {
    ...
    if (skillId === SkillLoader.USER_CODE_EXECUTOR_SKILL_ID) {
      logger.debug(`[SkillLoader] 检测到 user-code-executor 技能，直接执行用户代码`);
    ...
    try {
      ...
      logger.debug(`[SkillLoader] 执行技能工具: ${skillId}.${toolName}`, {
        sourcePath,
        scriptPath,
    ...
    logger.debug(`[SkillLoader] 技能环境 DATA_BASE_PATH: ${env.DATA_BASE_PATH}, SCRIPT_PATH: ${env.SCRIPT_PATH}`);
  ...
  async executeInSubprocess(skillId, toolName, params, context, env, startTime, timeoutConfig = null) {
    ...
    logger.debug(`[SkillLoader] 技能调用超时设置: ${skillCallTimeout}秒 (${executionTimeout}ms)`);
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        ...
        if (stderr) {
          ...
          logger.debug(`[SkillLoader] ${skillId} stderr:\n${stderr}`);
        }
        ...
        try {
          ...
          if (result.success) {
            logger.debug(`[SkillLoader] 技能执行完成: ${skillId}.${toolName} (${duration}ms)`);
            resolve(result.data);
          } else {
  ...
  async executeUserCode(code, context = {}, scriptPath = null) {
    ...
    logger.debug(`[SkillLoader] 执行用户代码`, {
      hasCode: !!code,
      scriptPath,
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        ...
        if (stderr) {
          logger.debug(`[SkillLoader] user-code stderr:\n${stderr}`);
        }
        ...
        try {
          ...
          if (result.success) {
            logger.debug(`[SkillLoader] 用户代码执行完成 (${duration}ms)`);
            resolve(result.data);
          } else {
  ...
  async getSkillSourcePath(skillId) {
    try {
      ...
      // 优先按 id 查询，如果没找到则按 name 查询（支持传入技能名称的情况）
      ...
      if (!skill?.source_path) {
        ...
        if (skill?.source_path) {
          logger.debug(`[SkillLoader] 通过 name 查询到 source_path for ${skillId}`);
        }
      }

      // 如果按 name 也没找到，尝试按 mark 查询
      if (!skill?.source_path) {
        skill = await Skill.findOne({
          where: { mark: skillId },
          attributes: ['source_path'],
        ...
        if (skill?.source_path) {
          logger.debug(`[SkillLoader] 通过 mark 查询到 source_path for ${skillId}`);
        }
      }
  ...
  buildSkillEnvironment(skillId, config, sourcePath = null, scriptPath = 'index.js', userContext = {}, packageWhitelist = null, timeoutConfig = null) {
    ...
    // 注意：现在统一传递绝对路径，不再转换为相对路径
    let workingDirectory = userContext.workingDirectory || null;
    
    // 验证：如果是相对路径，抛出错误（强制使用绝对路径）
    if (workingDirectory && !path.isAbsolute(workingDirectory)) {
      logger.error(`[SkillLoader] 工作目录必须是绝对路径，收到: ${workingDirectory}`);
      throw new Error(`工作目录必须是绝对路径，请传递绝对路径而非相对路径`);
    }
    
    if (workingDirectory) {
      logger.debug(`[SkillLoader] 设置工作目录(绝对路径): ${workingDirectory}`);
    }
    ...
    const baseEnv = {
      ...
      API_BASE: process.env.API_BASE || 'http://localhost:3017',  // 12. API 基础地址
      WORKING_DIRECTORY: workingDirectory || '',         // 13. 工作目录（绝对路径）
    ...
    if (timeoutConfig) {
      ...
      logger.debug(`[SkillLoader] 超时配置: VM=${timeoutConfig.vm_execution}s, Python=${timeoutConfig.python_execution}s`);
    }
  ...
  invalidateCache(skillId = null) {
    if (skillId) {
      ...
      logger.debug(`[SkillLoader] 技能缓存已清除: ${skillId}`);
    } else {
      this.skillCache.clear();
      logger.debug('[SkillLoader] 所有技能缓存已清除');
    }
  }
