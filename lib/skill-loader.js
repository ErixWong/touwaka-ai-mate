...
// 默认超时配置（毫秒）- 从 SystemSettingService 统一获取
...
class SkillLoader {
  ...
  async executeSkillTool(skillId, toolName, params, context = {}, scriptPath = 'index.js') {
    ...
    try {
      ...
      // 获取超时配置
      timeoutConfig = await this.getTimeoutConfig();
   ...
   * @returns {Promise<object>} 执行结果
   */
  executeInSubprocess(skillId, toolName, params, context, env, startTime, timeoutConfig = DEFAULT_TIMEOUTS) {
    // 使用 python_execution 作为子进程超时（因为它通常最长）
    const subprocessTimeout = Math.max(
      timeoutConfig.python_execution,
      timeoutConfig.vm_execution
    );
  ...
  /**
   * 从数据库获取超时配置
   * @returns {Promise<{vm_execution: number, python_execution: number, skill_http: number, resident_skill: number}>}
   */
  async getTimeoutConfig() {
    try {
      const SystemSetting = this.db.getModel('system_setting');
      if (!SystemSetting) {
        logger.warn(`[SkillLoader] SystemSetting model not found, using default timeouts`);
        return DEFAULT_TIMEOUTS;
      }

      const timeoutKeys = ['vm_execution', 'python_execution', 'skill_http', 'resident_skill'];
      const settings = await SystemSetting.findAll({
        where: {
          setting_key: timeoutKeys.map(k => `timeout.${k}`),
        },
        raw: true,
      });
      ...
      for (const setting of settings) {
        const key = setting.setting_key.replace('timeout.', '');
      ...
      logger.error(`[SkillLoader] Failed to get timeout config:`, error.message);
      return DEFAULT_TIMEOUTS;
    }
  }
   ...
   * @param {object} timeoutConfig - 超时配置（包含 vm_execution, python_execution, skill_http, resident_skill）
  ...
  buildSkillEnvironment(skillId, config, sourcePath = null, scriptPath = 'index.js', userContext = {}, packageWhitelist = null, timeoutConfig = null) {
    ...
    const RESERVED_ENV_VARS = [
      ...
      'TIMEOUT_VM_EXECUTION', 'TIMEOUT_PYTHON_EXECUTION', 'TIMEOUT_SKILL_HTTP', 'TIMEOUT_RESIDENT_SKILL',  // 超时配置
    ];
    ...
    // 添加超时配置环境变量（如果提供）
    if (timeoutConfig) {
      baseEnv.TIMEOUT_VM_EXECUTION = String(timeoutConfig.vm_execution || DEFAULT_TIMEOUTS.vm_execution);
      baseEnv.TIMEOUT_PYTHON_EXECUTION = String(timeoutConfig.python_execution || DEFAULT_TIMEOUTS.python_execution);
      baseEnv.TIMEOUT_SKILL_HTTP = String(timeoutConfig.skill_http || DEFAULT_TIMEOUTS.skill_http);
      baseEnv.TIMEOUT_RESIDENT_SKILL = String(timeoutConfig.resident_skill || DEFAULT_TIMEOUTS.resident_skill);
    }
