...
class SkillLoader {
  ...
  async executeSkillTool(skillId, toolName, params, context = {}, scriptPath = 'index.js') {
    ...
    try {
      ...
      // 构建该技能的最小化环境变量（安全隔离）
      env = this.buildSkillEnvironment(skillId, config, sourcePath, scriptPath, context, packageWhitelist);
    } catch (error) {
   ...
   */
  buildSkillEnvironment(skillId, config, sourcePath = null, scriptPath = 'index.js', userContext = {}, packageWhitelist = null) {
    // 最小化系统环境变量白名单（仅保留必要的）
