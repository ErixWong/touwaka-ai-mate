 ...
 * }}
 */
export function createTouwakaTranscriptStore({
  db,
  ...
  const runStateQuery = async (sql, replacements) => {
    try {
      ...
      if (isMissingRunStateTableError(error)) {
        throw new Error(
          `[llm-kit-adapters] ${runStateTableName} 表不存在，请先运行 scripts/upgrade-database.js`,
          { cause: error },
        );
      }
      throw error;
  ...
  const runStateJsonFor = (runId, nextState) =>
    runStateQuery(`
      INSERT INTO ${runStateTableName} (run_id, state, updated_at)
  ...
  // state 列兼容两种形态：saveRunState 写入的 JSON 对象，或纯状态字符串。
  const readRunStateColumn = async (runId) => {
    const [rows] = await runStateQuery(`
      SELECT state
      FROM ${runStateTableName}
      WHERE run_id = ?
      LIMIT 1
    `, [runId]);
    return rows[0]?.state;
  ...
  const parseRunStateColumn = (raw) => {
    if (raw === null || raw === undefined) return undefined;
    try {
      const parsed = JSON.parse(raw);
      if (isRecord(parsed)) return parsed;
      // 旧行可能是 JSON 字符串字面量（如 '"running"'），取解析后的裸值，避免把
      // 引号带进 state。
      if (typeof parsed === "string") return { state: parsed };
      return { state: raw };
    } catch {
      return { state: raw };
    }
  ...
  const markRunState = async (runId, state) => {
    // mark 只改 state 字段，保留 saveRunState 已写入的 stateVersion/deterministic 等
    const existing = parseRunStateColumn(await readRunStateColumn(runId));
    await runStateJsonFor(runId, { ...(existing ?? {}), state });
  };

  const saveRunState = async (runId, state) => {
    await runStateJsonFor(runId, isRecord(state) ? state : { state });
  };

  const loadRunState = async (runId) => {
    const existing = parseRunStateColumn(await readRunStateColumn(runId));
    return existing === undefined ? undefined : { ...existing, runId };
  };

  const saveCheckpoint = async (runId, checkpoint) => {
    await runStateQuery(`
      INSERT INTO ${runStateTableName} (run_id, checkpoint, updated_at)
      VALUES (?, ?, CURRENT_TIMESTAMP)
      ON DUPLICATE KEY UPDATE
