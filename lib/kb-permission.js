...
async function isDepartmentManager(db, userId) {
  try {
    ...
    return position?.is_manager === 1 || position?.is_manager === true;
  } catch (error) {
 ...
 * @returns {Promise<boolean>}
 */
async function canAccessKbArticle(db, articleId, userId) {
  try {
    const KbArticle = db.getModel('kb_article');
    const article = await KbArticle.findByPk(articleId, {
      include: [{
        model: db.getModel('knowledge_basis'),
        as: 'kb',
    ...
    // 使用知识库权限检查
    return canAccessKb(db, article.kb?.id, userId);
  } catch (error) {
    logger.error('[KB-Permission] canAccessKbArticle error:', error);
    return false;
  }
}
