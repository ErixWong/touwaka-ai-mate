 ...
 */
  ...
  async _executeLLMRounds(expertService, { modelConfig, thinkingConfig, tools, currentMessages, llmPayload, user_id, expert_id, taskContext, topic_id, task_id, session, request_id, onDelta }) {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      // 检测 document_retrieval skill tool 结果，决定是否短路 LLM 或注入证据约束
      ...
      if (docRetrievalResult?.success) {
        ...
        // 可观测性日志：记录 tool 结果消费决策
        logger.info('[ChatService._executeLLMRounds] doc retrieval result consumed:', {
          retrieval_source: 'tool_only',
          tool_name: docRetrievalResult.tool_name || 'document_retrieval',
          ...
          document_count: docRetrievalResult.documents?.length || 0,
