  ...
  async streamChat(params, onDelta, onComplete, onError) {
    ...
    try {
      ...
      for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        ...
        // 执行工具
        const toolResults = await expertService.handleToolCalls(collectedToolCalls, user_id, access_token);
...
class ExpertChatService {
   ...
   * 处理工具调用
   */
  async handleToolCalls(toolCalls, user_id, access_token = null) {
    const context = {
