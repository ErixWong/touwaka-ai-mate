...
class ChatService {
  ...
  async streamChat(params, onDelta, onComplete, onError) {
    const { topic_id: providedTopicId, user_id, expert_id, content, model_id, task_id, task_path } = params;
  ...
  async getTaskContext(task_id, user_id, task_path = '') {
    try {
      ...
      const targetDir = task_path
        ? path.join(task.workspace_path, task_path)
        : path.join(task.workspace_path, 'input');
...
class ExpertChatService {
  ...
  async buildContext(user_id, currentMessage, topic_id, taskContext = null) {
    ...
    const context = await this.contextManager.buildContext(
      this.memorySystem,
      user_id,
      { currentMessage, skills, taskContext }
    );
