    ...
    try {
      ...
      if (tools.length > 0) {
        ...
        if (llmResponse.toolCalls && llmResponse.toolCalls.length > 0) {
          ...
          // P0-2: 文档检索工具结果 → 显式回答编排
          // 当 LLM 调用了 document_retrieval 后，根据 suggested_response_mode 决定回答策略：
          // - candidate_list: 直接格式化候选文档列表，不调用 LLM（避免 LLM 替用户做选择）
          // - clarify / conservative_answer: 注入强约束 system prompt，仍走 LLM
          // - direct_answer / answer_with_citation: 正常注入证据后调用 LLM
          const docRetrievalResult = toolResults?.find(r => r.toolName === 'document_retrieval');

          if (docRetrievalResult?.success && docRetrievalResult.suggested_response_mode === 'candidate_list') {
            // 候选列表模式：不调用 LLM，直接格式化结构化回复
            response = expertService._buildCandidateListResponse(docRetrievalResult);
            logger.info('[ChatService.chat] candidate_list 显式编排（短路 LLM）:', {
              doc_count: docRetrievalResult.documents?.length || 0,
            });
            // 跳过 LLM 调用，直接进入保存/后处理阶段
          } else {
            if (docRetrievalResult?.success) {
              const evidenceInjection = expertService.buildEvidenceInjection(docRetrievalResult, { maxTokens: 4000 });
              if (evidenceInjection) {
                followUpMessages.unshift({
                  role: 'system',
                  content: evidenceInjection,
                });
...
class ExpertChatService {
  ...
  async _maybeLegacyAutoRetrieval(currentMessage, user_id) {
    ...
    logger.info('[ExpertChatService] 文档检索 gate check (auto path, legacy):', {
