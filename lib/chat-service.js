...
**不确定性提示：** [明确指出哪些部分缺乏充分证据支撑，建议用户进一步核实]`;
  ...
  /**
   * P0-1: 从 toolResults 中按 skill_namespace 查找 document retrieval 结果
   *
   * 替代旧写法 `toolResults?.find(r => r.toolName === 'document_retrieval')`，
   * 不再依赖具体 tool 名称，为未来多 tool（find_document / verify_fact）做准备。
   *
   * 兼容策略：优先匹配 skill_namespace，回退匹配 toolName（兼容旧版 tool-manager 返回）。
   *
