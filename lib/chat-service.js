...
class ChatService {
   ...
   * @param {string} params.task_path - 当前浏览的目录路径（可选）
  ...
  async streamChat(params, onDelta, onComplete, onError) {
    ...
    try {
      ...
      if (task_id) {
        taskContext = await this.getTaskContext(task_id, user_id, task_path);
        if (taskContext) {
          logger.info('[ChatService] 任务上下文已加载:', taskContext.title, '路径:', task_path || '根目录');
        }
      }
   ...
   */
  async chat(params) {
    const { topic_id, user_id, expert_id, content, model_id, access_token } = params;
    ...
    try {
      ...
      if (tools.length > 0) {
        ...
        if (llmResponse.toolCalls && llmResponse.toolCalls.length > 0) {
          toolCalls = await expertService.handleToolCalls(llmResponse.toolCalls, user_id, access_token);
   ...
   * @param {string} task_path - 当前浏览的目录路径（可选）
   * @returns {Promise<object|null>} 任务上下文对象
   */
  async getTaskContext(task_id, user_id, task_path = '') {
    try {
      if (!this.Task) {
      ...
      const taskContext = {
        ...
        userId: user_id,
        currentPath: task_path || '',  // 当前浏览的目录路径
        status: task.status,
      };

      // 确定要读取的目录（后端使用完整路径）
      const targetDir = task_path
        ? path.join(WORKSPACE_ROOT, task.workspace_path, task_path)
          ...
          files.map(async (filename) => {
            try {
              ...
              return {
                ...
                path: task_path ? `${task_path}/${filename}` : filename,
              };
            } catch {
              return null;
            }
...
class ExpertChatService {
  ...
  async buildContext(user_id, currentMessage, topic_id, taskContext = null) {
    ...
    const context = await this.contextManager.buildContext(
      this.memorySystem,
      user_id,
      { currentMessage, skills, taskContext, ragContext }
    );
   ...
   */
  async handleToolCalls(toolCalls, user_id, access_token = null, taskContext = null, onToolComplete = null) {
    const context = {
   ...
   * 获取思考模式配置
   * 根据模型名称自动检测是否支持思考模式：
   * - DeepSeek 推理模型（deepseek-reasoner）→ 启用 thinking
   * - OpenAI o1/o3 系列 → 启用 reasoning
  ...
  getThinkingConfig(modelConfig = null) {
    ...
    if (modelName.includes('deepseek-reasoner') || modelName.includes('deepseek-r1')) {
      logger.info(`[ExpertChatService] 检测到 DeepSeek 推理模型: ${model.model_name}, 启用 thinking`);
      return {
    ...
    if (modelName.startsWith('o1-') && !modelName.includes('mini') && !modelName.includes('preview')) {
      logger.info(`[ExpertChatService] 检测到 OpenAI o1 模型: ${model.model_name}, 启用 reasoning`);
      return {
        thinking: null,
    ...
    if (modelName.startsWith('o3-') || modelName.startsWith('o4-')) {
      logger.info(`[ExpertChatService] 检测到 OpenAI 推理模型: ${model.model_name}, 启用 reasoning`);
      return {
        thinking: null,
        reasoning: { effort: 'medium' },
      };
