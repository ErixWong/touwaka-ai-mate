  ...
  async streamChat(params, onDelta, onComplete, onError) {
    ...
    try {
      ...
      for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        ...
        // 执行工具（传递任务上下文和 topic_id）
        const toolResults = await expertService.handleToolCalls(collectedToolCalls, user_id, access_token, taskContext, topic_id);
    ...
    try {
      ...
      if (tools.length > 0) {
        ...
        if (llmResponse.toolCalls && llmResponse.toolCalls.length > 0) {
          toolCalls = await expertService.handleToolCalls(llmResponse.toolCalls, user_id, access_token, null, topic_id);
...
class ExpertChatService {
   ...
   */
  async handleToolCalls(toolCalls, user_id, access_token = null, taskContext = null, topic_id = null) {
    const context = {
