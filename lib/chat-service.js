    ...
    try {
      ...
      if (tools.length > 0) {
        ...
        if (llmResponse.toolCalls && llmResponse.toolCalls.length > 0) {
          ...
          // P0-1: 文档检索工具结果 → 注入证据消费指引到 System Prompt
          // 当 LLM 调用了 document_retrieval 后，将结构化证据转换为回答指引，
          // 帮助 LLM 根据 suggested_response_mode 切换回答行为
          ...
          if (docRetrievalResult?.success) {
            ...
            if (evidenceInjection) {
              // 作为 system 消息注入，引导 LLM 根据证据强度回答
...
class ExpertChatService {
  ...
  async buildContext(user_id, currentMessage, topic_id, taskContext = null) {
    ...
    // 文档检索（自动预检索路径，旧链路）
    let ragContext = null;
    let retrievalSource = 'none'; // 'auto_rag' | 'tool' | 'none' — 追踪检索来源
    ...
    logger.info('[ExpertChatService] 文档检索 gate check (auto path):', {
      expert_id: this.expertId,
      has_rag_service: !!this.ragService,
      knowledge_config_type: knowledgeConfig === null ? 'null' : typeof knowledgeConfig,
      ...
      collection_id: knowledgeConfig?.collection_id ?? null,
      doc_types: Array.isArray(knowledgeConfig?.doc_types) ? knowledgeConfig.doc_types : null,
      top_k: knowledgeConfig?.top_k ?? null,
      threshold: knowledgeConfig?.threshold ?? null,
    });
    if (knowledgeConfig?.enabled && !retireAutoPath && this.ragService) {
      try {
        logger.info('[ExpertChatService] Entering 文档检索 (auto path):', {
          expert_id: this.expertId,
        ...
        if (ragResult.success && ragResult.results.length > 0) {
          ...
          retrievalSource = 'auto_rag';
          logger.info(`[ExpertChatService] 文档检索到 ${ragResult.results.length} 条相关内容 (auto path)`);
        }
      } catch (error) {
        logger.warn('[ExpertChatService] 文档检索失败 (auto path):', error.message);
      }
    }
  ...
  async buildMinimalContext(user_id, currentMessage, topic_id, taskContext = null) {
    ...
    // WP-4: 文档检索（自动预检索路径，旧链路）
    let ragContext = null;
    let retrievalSource = 'none'; // 'auto_rag' | 'tool' | 'none'
    const knowledgeConfig = this.expertConfig.expert?.knowledge_config;
    const retireAutoPath = knowledgeConfig?.retire_auto_path === true;
    logger.info('[ExpertChatService] 文档检索 gate check (auto path, minimal):', {
      expert_id: this.expertId,
      has_rag_service: !!this.ragService,
      knowledge_config_type: knowledgeConfig === null ? 'null' : typeof knowledgeConfig,
      knowledge_enabled: knowledgeConfig?.enabled ?? null,
    ...
    if (knowledgeConfig?.enabled && !retireAutoPath && this.ragService) {
      try {
        logger.info('[ExpertChatService] Entering 文档检索 (auto path, minimal):', {
          expert_id: this.expertId,
          collection_id: knowledgeConfig.collection_id,
          doc_types: knowledgeConfig.doc_types,
          top_k: knowledgeConfig.top_k || 5,
        ...
        if (ragResult.success && ragResult.results.length > 0) {
          ...
          retrievalSource = 'auto_rag';
          logger.info(`[ExpertChatService] 文档检索到 ${ragResult.results.length} 条相关内容 (auto path, minimal)`);
        }
      } catch (error) {
        logger.warn('[ExpertChatService] 文档检索失败 (auto path, minimal):', error.message);
      }
    }
  ...
  buildEvidenceInjection(toolResult, options = {}) {
    ...
    if (this.ragService) {
      ...
      return this.ragService.buildEvidenceContextMessage(packet, { maxTokens });
    }
    ...
    if (toolResult.suggested_response_mode === 'conservative_answer') {
      context += '⚠️ 证据不足，请保守回答并说明依据有限。\n';
    }
    context += '\n';
