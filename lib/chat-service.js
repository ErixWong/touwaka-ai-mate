 ...
 */
  ...
  async _executeLLMRounds(expertService, { modelConfig, thinkingConfig, tools, currentMessages, llmPayload, user_id, expert_id, taskContext, topic_id, task_id, session, request_id, onDelta }) {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      // 检测 document_retrieval tool 结果，决定是否短路 LLM 或注入证据约束
      const docRetrievalResult = toolResults?.find(r => r.toolName === 'document_retrieval');
      if (docRetrievalResult?.success) {
        const modeDecision = expertService._getResponseModeDecision(docRetrievalResult);
    ...
    try {
      ...
      if (tools.length > 0) {
        ...
        if (llmResponse.toolCalls && llmResponse.toolCalls.length > 0) {
          ...
          // _getResponseModeDecision 是流式/非流式共用的单一决策入口：
          // - candidate_list → 直接格式化输出，短路 LLM
          // - clarify / conservative_answer → LLM + 证据约束 + 输出骨架
          // - direct_answer / answer_with_citation → LLM + 证据注入（无额外约束）
          const docRetrievalResult = toolResults?.find(r => r.toolName === 'document_retrieval');
          const modeDecision = expertService._getResponseModeDecision(docRetrievalResult);
          ...
          if (modeDecision.isShortCircuit) {
            ...
            logger.info('[ChatService.chat] response mode 显式编排（短路 LLM）:', {
              mode: modeDecision.mode,
              doc_count: docRetrievalResult?.documents?.length || 0,
            });
...
class ExpertChatService {
  constructor(db, expertId, options = {}) {
    ...
    this.ragService = null;  // 文档检索服务（兼容壳层，底层为文档平台检索）
  ...
  /**
   * P0-1: 自动预检索门控（旧链路收敛入口）
   *
   * 统一 buildContext() 与 buildMinimalContext() 中的重复逻辑。
   * 当 retire_auto_path=true 时直接返回空结果，不再触发自动预检索。
   * 此方法仅服务于旧自动路径兼容，不应在新逻辑中调用。
   *
   * @param {string} currentMessage - 当前用户消息
   * @param {string} user_id - 用户ID
   * @returns {Promise<{ragContext: string|null, retrievalSource: 'auto_rag'|'none'}>}
   */
  async _maybeLegacyAutoRetrieval(currentMessage, user_id) {
    const knowledgeConfig = this.expertConfig.expert?.knowledge_config;

    // ── knowledge_config 字段分类 ──────────────────────────────────────────
    // 【控制面】新旧路径共用：
    //   enabled           — 文档检索总开关
    ...
    //   doc_types         — 文档类型白名单
    //
    // 【旧自动路径专属】（retire_auto_path=true 后不再消费）：
    //   top_k             — 自动检索返回条数
    //   threshold         — 相似度阈值
    ...
    const retireAutoPath = knowledgeConfig?.retire_auto_path === true;

    const gateInfo = {
      expert_id: this.expertId,
      has_rag_service: !!this.ragService,
      knowledge_enabled: knowledgeConfig?.enabled ?? null,
      retire_auto_path: retireAutoPath,
      retrieval_path: retireAutoPath ? 'tool_only' : (knowledgeConfig?.enabled ? 'auto_rag' : 'disabled'),
    };

    logger.info('[ExpertChatService] 文档检索 gate check (auto path, legacy):', gateInfo);

    if (!knowledgeConfig?.enabled || retireAutoPath || !this.ragService) {
      logger.info('[ExpertChatService] retrieval round result (legacy auto):', {
        retrieval_source: 'none',
        reason: retireAutoPath ? 'retire_auto_path' : (!knowledgeConfig?.enabled ? 'knowledge_disabled' : 'no_rag_service'),
      });
      return { ragContext: null, retrievalSource: 'none' };
    }

    try {
      logger.info('[ExpertChatService] Entering 文档检索 (auto path, legacy):', {
        expert_id: this.expertId,
        collection_id: knowledgeConfig.collection_id,
        doc_types: knowledgeConfig.doc_types,
        top_k: knowledgeConfig.top_k || 5,
      });

      const ragResult = await this.ragService.retrieve(currentMessage, {
        expertId: this.expertId,
        collectionId: knowledgeConfig.collection_id,
        docTypes: knowledgeConfig.doc_types,
      ...
      if (ragResult.success && ragResult.results.length > 0) {
        const ragContext = this.ragService.buildContextMessage(ragResult.results, {
          maxTokens: knowledgeConfig.max_tokens || 2000,
          style: knowledgeConfig.style || 'default',
        });
        logger.info(`[ExpertChatService] 文档检索到 ${ragResult.results.length} 条相关内容 (auto path, legacy)`);
        logger.info('[ExpertChatService] retrieval round result (legacy auto):', {
          retrieval_source: 'auto_rag',
          doc_count: ragResult.results.length,
          evidence_injected: !!ragContext,
        });
        return { ragContext, retrievalSource: 'auto_rag' };
      }
    } catch (error) {
      logger.warn('[ExpertChatService] 文档检索失败 (auto path, legacy):', error.message);
    }
  ...
  async buildContext(user_id, currentMessage, topic_id, taskContext = null) {
    ...
    // 文档检索（自动预检索路径，旧链路 — 已收敛到 _maybeLegacyAutoRetrieval）
    const { ragContext, retrievalSource } = await this._maybeLegacyAutoRetrieval(currentMessage, user_id);

    // 使用 ContextManager 构建完整上下文
    const context = await this.contextManager.buildContext(
      this.memorySystem,
      user_id,
      { currentMessage, skills, taskContext, ragContext, retrievalSource, assistants }
    );
  ...
  async buildMinimalContext(user_id, currentMessage, topic_id, taskContext = null) {
    ...
    if (skills.length > 0) {
      ...
      baseSystemPrompt += `\n\n【可用技能】\n${skillsDesc}`;
      baseSystemPrompt += `\n\n【文档检索优先规则】\n当用户明确提到“文档库”“知识库”“资料库”“内部文档”“帮我找文档”“帮我看看在哪个文档里”“某企业是否有相关规定”“文档里怎么说”“请根据文档回答”等场景时，应优先调用 document_retrieval，而不是直接调用 web_search。\n如果用户没有给出具体文档名，也不代表不能先查文档；只要问题明显是在让你帮助定位文档、核实文档内容、或基于内部文档回答，就应先尝试 document_retrieval。\n只有在 document_retrieval 返回证据不足、无法定位、或问题明显超出文档平台范围时，才再考虑使用 web_search 作为补充。`;
    }

    // WP-4: 注入文档检索上下文到 System Prompt（旧自动预检索路径）
    if (ragContext) {
      baseSystemPrompt += `\n\n## 相关文档平台内容（自动预检索）\n${ragContext}`;
    }
   ...
   *
   * 这是"证据约束回答"的基础设施方法。当 LLM 调用了 document_retrieval tool 后，
   * tool 返回的结构化 evidence 通过此方法转换为 LLM 可理解的上下文文本，
   * 包含证据充分性评估和建议的回答模式。
   *
   * @param {Object} toolResult - document_retrieval tool 的返回结果
  ...
  buildEvidenceInjection(toolResult, options = {}) {
    ...
    if (this.ragService) {
      ...
      context += this.ragService.buildEvidenceContextMessage(packet, { maxTokens });
      return context;
    }
    ...
    context += '## 文档检索结果\n';
    context += `证据充分性: ${toolResult.evidence_sufficiency || 'unknown'}\n`;
    context += '\n';

    for (const doc of toolResult.documents) {
      context += `- **${doc.document_title || '未命名'}** (${doc.doc_type || 'unknown'})`;
      if (doc.top_evidence?.length > 0) {
        context += `\n  ${doc.top_evidence[0].content?.substring(0, 300) || ''}`;
      }
      context += '\n';
    }
...
**不确定性提示：** [明确指出哪些部分缺乏充分证据支撑，建议用户进一步核实]`;
   ...
   * 直接基于 document_retrieval tool 返回的结构化文档列表格式化输出，
   * 不经过 LLM。这消除了 LLM 替用户选择单一文档的风险。
   *
   * @param {Object} docRetrievalResult - document_retrieval tool 的返回结果
   * @returns {string} Markdown 格式的候选文档列表
   */
  _buildCandidateListResponse(docRetrievalResult) {
    const documents = docRetrievalResult?.documents || [];
   ...
   *
   * 单一决策入口，根据 document_retrieval tool 返回的 suggested_response_mode
   ...
   *
   * @param {Object|null} docRetrievalResult - document_retrieval tool 的返回结果
