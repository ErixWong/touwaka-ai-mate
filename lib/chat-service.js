...
import { getWorkspaceRoot, getTaskWorkspaceAbsolutePath, getDefaultWorkspaceAbsolutePath, toLogicalWorkspacePath } from './paths.js';
...
class ChatService {
  ...
  async _prepareTaskContext({ task_id, user_id, working_path, session }) {
    if (task_id) {
      ...
      // working_path 已经是相对路径（如 skills/xxx），不需要转换
      return {
        // 兼容旧字段名
        fullWorkspacePath: working_path,
        // 新字段：绝对路径（供执行层使用）
        absolute_workspace_path: path.join(getWorkspaceRoot(), '..', working_path),
        // 新字段：逻辑路径（供展示层使用）
        logical_workspace_path: working_path,
        currentPath: '',
      ...
      return {
        // 兼容旧字段名
        fullWorkspacePath: toLogicalWorkspacePath(absolutePath),
        // 新字段：���对路径（供执行层使用）
  ...
  async getTaskContext(task_id, user_id, working_path = '', session = null) {
    try {
      ...
      const taskContext = {
        ...
        // 兼容旧字段名（逻辑路径）
        fullWorkspacePath: taskWorkspacePath,
        // 新字段：绝对路径（供执行层使用）
        absolute_workspace_path: absolutePath,
        // 新字段：逻辑路径（供展示层使用）
