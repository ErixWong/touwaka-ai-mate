...
class ChatService {
  ...
  async streamChat(params, onDelta, onComplete, onError) {
    ...
    try {
      ...
      if (!skip_user_message_persist) {
        userMessageId = await this.saveUserMessage(topic_id, user_id, content, expert_id, task_id);
  ...
  async chat(params) {
    ...
    try {
      ...
      // 3. 保存用户消息
      await this.saveUserMessage(topic_id, user_id, content, expert_id, task_id);
