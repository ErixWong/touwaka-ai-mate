...
class ChatService {
  ...
  async streamChat(params, onDelta, onComplete, onError) {
    const { topic_id: providedTopicId, user_id, expert_id, content, model_id, task_id, task_path, access_token, is_admin, roles } = params;
   ...
   */
  async chat(params) {
    const { topic_id, user_id, expert_id, content, model_id, access_token, task_id, task_path, is_admin, roles } = params;
...
class ExpertChatService {
   ...
   * @param {Array} roles - 用户角色列表（由 auth 中间件设置）
   */
  async handleToolCalls(toolCalls, user_id, access_token = null, taskContext = null, topic_id = null, onToolComplete = null, is_admin = false, roles = []) {
    const context = {
