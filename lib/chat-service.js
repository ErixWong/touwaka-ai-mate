...
class ChatService {
  ...
  async streamChat(params, onDelta, onComplete, onError) {
    ...
    try {
      ...
      const MAX_TOOL_ROUNDS = await systemSettingService.getMaxToolRounds();
      ...
      for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        ...
        // 执行工具（传递任务上下文）
        const toolResults = await expertService.handleToolCalls(collectedToolCalls, user_id, access_token, taskContext);
...
class ExpertChatService {
   ...
   */
  async handleToolCalls(toolCalls, user_id, access_token = null, taskContext = null) {
    const context = {
