...
class DocumentOutlineService {
  ...
  async extract(revisionId, options = {}) {
    ...
    const timeoutMs = stageConfig.timeout_ms || DEFAULT_TIMEOUT_MS;
    ...
    try {
      ...
      if (text.length <= windowSize) {
        ...
        outlines = await this._extractSingleChunk(callLlm, text, maxLevel, temperature, modelId, timeoutMs);
      } else {
        logger.info(`[DocumentOutlineService] Revision ${revisionId}: multi-chunk extraction (window=${windowSize}, step=${stepSize})`);
        const result = await this._extractMultiChunk(callLlm, text, windowSize, stepSize, maxLevel, temperature, modelId, deduplicateTitles, timeoutMs);
  ...
  async _extractSingleChunk(callLlm, text, maxLevel, temperature, modelId, timeoutMs) {
  ...
  async _extractMultiChunk(callLlm, text, windowSize, stepSize, maxLevel, temperature, modelId, deduplicateTitles, timeoutMs) {
