...
class DocCompareExecutor {
  ...
  async execute(runId) {
    ...
    try {
      ...
      const baseUnits = await this.models.DocContentUnit.findAll({
        where: { version_id: run.base_version_id },
        order: [['position', 'ASC']],
      });
      const targetUnits = await this.models.DocContentUnit.findAll({
        where: { version_id: run.target_version_id },
        order: [['position', 'ASC']],
      });
  ...
  compareUnits(baseUnits, targetUnits) {
    const items = [];
    const baseMap = new Map(baseUnits.map(u => [u.position, u]));
    const targetMap = new Map(targetUnits.map(u => [u.position, u]));
    ...
    for (const pos of [...allPositions].sort((a, b) => a - b)) {
      const base = baseMap.get(pos);
      const target = targetMap.get(pos);
