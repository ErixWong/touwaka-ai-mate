...
export function createEmbeddingTask(options = {}) {
  ...
  async function vectorizeParagraph(paragraph, sectionMap, articleMap, kbMap, db) {
    ...
    try {
      ...
      // 避免使用 X'...' 十六进制语法，Sequelize 会正确处理 Buffer 为 BLOB
      await db.sequelize.query(
        `UPDATE kb_paragraphs SET embedding = ?, token_count = ? WHERE id = ?`,
        {
          replacements: [finalBuffer, tokenCount, paragraph.id],
          type: db.sequelize.QueryTypes.UPDATE,
        }
      );
    } catch (dbError) {
      logger.error(`[EmbeddingTask] Database error for paragraph ${paragraph.id}:`, dbError.message);
      logger.error(`[EmbeddingTask] Buffer length:`, finalBuffer.length, 'bytes, expected dim:', expectedDim);
      throw dbError;
    }
