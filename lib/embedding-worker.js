...
export function createEmbeddingTask(options = {}) {
  ...
  async function generateEmbeddingVector(text, modelId, db) {
    ...
    try {
      ...
      const service = getSystemSettingService(db);
      const settings = await service.getAllSettings();
      timeoutMs = (settings?.timeout?.embedding || 120) * 1000;
    } catch {
      timeoutMs = 120000;
    }

    return await client.embed(text, timeoutMs);
  }
  ...
  async function vectorizeParagraph(paragraph, sectionMap, articleMap, kbMap, db) {
    ...
    if (!vectorToStore) {
      logger.error(`[EmbeddingTask] Invalid embedding for paragraph ${paragraph.id}`);
      return false;
    }
    
    if (adjusted) {
      logger.warn(`[EmbeddingTask] Vector dimension mismatch for paragraph ${paragraph.id}: DB requires ${DB_VECTOR_DIM}, got ${originalDim}`);
      logger.info(`[EmbeddingTask] ${message}`);
    }
    ...
    try {
      ...
      await db.execute(
        ...
        [vectorToJson(vectorToStore), tokenCount, paragraph.id]
      );
    } catch (dbError) {
