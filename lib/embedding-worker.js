...
export function createEmbeddingTask(options = {}) {
  ...
  async function vectorizeParagraph(paragraph, sectionMap, articleMap, kbMap, db) {
    ...
    // 检查向量维度
    // 注意：数据库字段定义为 VECTOR(1536)，必须填充到 1536 维
    // 支持的模型维度：
    // - bge-m3: 1024 维（填充到 1536）
    // - text-embedding-3-small: 1536 维
    // - text-embedding-ada-002: 1536 维
    const DB_VECTOR_DIM = 1536;  // 数据库字段固定维度
    const vectorDim = embedding.length;
    let vectorToStore = embedding;
    
    if (vectorDim !== DB_VECTOR_DIM) {
      logger.warn(`[EmbeddingTask] Vector dimension mismatch for paragraph ${paragraph.id}: DB requires ${DB_VECTOR_DIM}, got ${vectorDim}`);
      // 如果维度不匹配，尝试截断或填充到数据库要求的维度
      if (vectorDim > DB_VECTOR_DIM) {
        vectorToStore = embedding.slice(0, DB_VECTOR_DIM);
        logger.warn(`[EmbeddingTask] Truncated vector from ${vectorDim} to ${DB_VECTOR_DIM} (may lose information)`);
      } else {
        vectorToStore = [...embedding, ...new Array(DB_VECTOR_DIM - vectorDim).fill(0)];
        logger.info(`[EmbeddingTask] Padded vector from ${vectorDim} to ${DB_VECTOR_DIM} with zeros`);
      }
    }
    ...
    try {
      // 将向量转换为 JSON 数组字符串格式
      // VEC_FromText 接受 JSON 数格式的字符串，如 '[0.1, 0.2, 0.3]'
      const vectorJson = JSON.stringify(vectorToStore);
      
      // 使用 VEC_FromText 函数转换 JSON 数组为 VECTOR 类型
      // 注意：需要转义 SQL 中的单引号，但 JSON.stringify 不会产生单引号
      const sql = `UPDATE kb_paragraphs SET embedding = VEC_FromText('${vectorJson}'), token_count = ${tokenCount} WHERE id = '${paragraph.id}'`;
      await db.execute(sql, [], { raw: true });
    } catch (dbError) {
      logger.error(`[EmbeddingTask] Database error for paragraph ${paragraph.id}:`, dbError.message);
      logger.error(`[EmbeddingTask] Vector dimension:`, vectorToStore?.length, 'DB requires:', DB_VECTOR_DIM);
      throw dbError;
    }
