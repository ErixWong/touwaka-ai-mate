...
/**
 * 解析工作目录绝对路径（新版本：只接受绝对路径）
 * @param {string} workingDirectory - 工作目录（必须是绝对路径）
 * @param {string} userId - 用户ID
 * @returns {string} 绝对路径
 * @throws {Error} 如果工作目录不是绝对路径
 */
function resolveWorkingDirectoryPath(workingDirectory, userId = process.env.USER_ID || '') {
  ...
  if (workingDirectory && workingDirectory.trim() !== '') {
    ...
    if (!path.isAbsolute(workingDirectory)) {
      throw new Error(`工作目录必须是绝对路径，收到: ${workingDirectory}`);
    }
    return path.resolve(workingDirectory);
  }

  // 如果没有提供工作目录但有 userId，使用默认 temp 目录
  if (userId) {
...
function getScriptAuthoringRequirements(scriptType) {
  if (scriptType === ScriptType.PYTHON) {
    return [
      'Python 技能要求：入口文件必须是 .py，且必须定义 execute(tool, params, context)。',
      'Python 技能应使用标准 import 语法，并确保依赖包已安装且在 allowed_python_packages 白名单中。',
    ];
  }

  return [
    'Node.js 技能要求：入口文件必须是 .js，使用 CommonJS 写法，不支持 .mjs / import / export。',
    'Node.js 技能应使用 require() 与 module.exports / exports，并导出 execute(toolName, params, context)。',
    'Node.js 技能不支持相对 require("./helper")，如需拆分逻辑请改为单文件或主项目依赖。',
  ];
}

function getErrorSpecificGuidance(scriptType, errorMessage = '') {
  const guidance = [];

  if (scriptType === ScriptType.NODEJS) {
    if (/Cannot use import statement|Unexpected token 'export'|Unexpected identifier/.test(errorMessage)) {
      guidance.push('检测到 ESM 语法错误：请把 import/export 改为 require() / module.exports。');
    }

    if (/module is not defined|exports is not defined/.test(errorMessage)) {
      guidance.push('请使用 module.exports = { execute } 或 exports.execute = ... 导出 execute。');
    }

    if (/Relative imports are not allowed/.test(errorMessage)) {
      guidance.push('技能沙箱不允许相对 require，本地 helper 需要内联到同一个脚本，或改成主项目已安装依赖。');
    }

    if (/Module '.*' is not allowed|Failed to load external module/.test(errorMessage)) {
      guidance.push('请检查该 npm 依赖是否已安装到主项目；如果依赖或模块未在白名单中，请提示管理员将其加入 allowed_node_modules。');
    }
  }

  if (scriptType === ScriptType.PYTHON) {
    if (/must have an execute function/.test(errorMessage)) {
      guidance.push('请在 Python 入口文件中定义 execute(tool, params, context) 函数。');
    }

    if (/Package '.*' is not allowed|No module named /.test(errorMessage)) {
      guidance.push('请检查 Python 依赖是否已安装；如果包未在白名单中，请提示管理员将其加入 allowed_python_packages。');
    }

    if (/SyntaxError|IndentationError/.test(errorMessage)) {
      guidance.push('请检查 Python 脚本语法、缩进和字符串引号是否完整。');
    }
  }

  if (/Path not allowed in sandbox|Absolute path not allowed/.test(errorMessage)) {
    guidance.push('文件访问应使用当前工作目录下的相对路径，不要写绝对路径，也不要尝试通过 ../ 逃逸。');
  }

  if (/Working directory does not exist/.test(errorMessage)) {
    guidance.push('当前工作目录不存在：请确认任务工作区已创建，或改用系统分配的 work/<user>/temp 目录。');
  }

  if (/timeout/i.test(errorMessage)) {
    guidance.push('执行超时：请减少一次调用内的工作量，拆分步骤，或避免在顶层加载过重资源。');
  }

  return guidance;
}

function formatSkillExecutionError(error, scriptType) {
  const rawMessage = error?.message || String(error);

  if (rawMessage.includes('脚本修复建议:')) {
    return rawMessage;
  }
  ...
  const guidance = [
    ...getScriptAuthoringRequirements(scriptType),
    ...getErrorSpecificGuidance(scriptType, rawMessage),
  ];

  const uniqueGuidance = [...new Set(guidance.filter(Boolean))];
  if (uniqueGuidance.length > 0) {
    lines.push('', '脚本修复建议:');
    for (const item of uniqueGuidance) {
      lines.push(`- ${item}`);
    }
  }
...
function executeSkill(code, skillId) {
  ...
  // 现在统一要求 WORKING_DIRECTORY 必须是绝对路径
  ...
  if (workingDirectory && workingDirectory.trim() !== '') {
    if (!path.isAbsolute(workingDirectory)) {
      throw new Error(`工作目录必须是绝对路径，收到: ${workingDirectory}。请使用 absolute_workspace_path 字段传递绝对路径。`);
    }
    effectiveCwd = path.resolve(workingDirectory);
    cwdRule = '规则1: WORKING_DIRECTORY为绝对路径 -> 直接使用';
  } else if (process.env.USER_ID) {
    effectiveCwd = path.resolve(path.join(workspaceRoot, process.env.USER_ID, 'temp'));
    cwdRule = `规则2: WORKING_DIRECTORY为空但有USER_ID -> 默认temp目录: ${effectiveCwd}`;
  } else {
    effectiveCwd = dataBasePath;
    cwdRule = `规则3: 无WORKING_DIRECTORY和USER_ID -> 使用DATA_BASE_PATH: ${effectiveCwd}`;
  }
...
async function executeUserCodeDirectly(code, source = 'inline') {
  ...
  if (workingDirectory && workingDirectory.trim() !== '') {
    ...
    effectiveCwd = path.resolve(workingDirectory);
    cwdRule = `规则1: WORKING_DIRECTORY为绝对路径 -> ${effectiveCwd}`;
  } else if (process.env.USER_ID) {
    effectiveCwd = path.resolve(path.join(workspaceRoot, process.env.USER_ID, 'temp'));
    cwdRule = `规则2: WORKING_DIRECTORY为空但有USER_ID -> 默认temp目录: ${effectiveCwd}`;
  } else {
    effectiveCwd = dataBasePath;
    cwdRule = `规则3: 无WORKING_DIRECTORY和USER_ID -> 使��DATA_BASE_PATH: ${effectiveCwd}`;
  }
...
`;
    ...
    const timeout = setTimeout(() => {
      pythonProcess.kill();
      reject(new Error(formatSkillExecutionError(new Error(`Python skill execution timeout (${PYTHON_TIMEOUT}ms)`), ScriptType.PYTHON)));
    }, PYTHON_TIMEOUT);
    ...
    pythonProcess.on('close', (code) => {
      ...
      if (code !== 0) {
          reject(new Error(formatSkillExecutionError(new Error(`Python skill exited with code ${code}: ${stderr}`), ScriptType.PYTHON)));
          return;
        }
      ...
      try {
        ...
        resolve(result);
          ...
          reject(new Error(formatSkillExecutionError(new Error(`Failed to parse Python output: ${parseError.message}\nOutput: ${stdout}`), ScriptType.PYTHON)));
        }
      });
    
    pythonProcess.on('error', (error) => {
      clearTimeout(timeout);
      reject(new Error(formatSkillExecutionError(new Error(`Failed to start Python process: ${error.message}`), ScriptType.PYTHON)));
    });
...
async function main() {
  ...
  process.stdin.on('end', async () => {
    ...
    try {
      ...
      const scriptType = typeof currentScriptType !== 'undefined'
        ? currentScriptType
        : (process.env.SCRIPT_PATH && process.env.SCRIPT_PATH.endsWith('.py') ? ScriptType.PYTHON : ScriptType.NODEJS);
      const formattedError = formatSkillExecutionError(error, scriptType);
      process.stdout.write(JSON.stringify({
        success: false,
