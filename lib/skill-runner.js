...
function executeSkill(code, skillId) {
  ...
  // 统一解析最终工作目录，并将其作为技能侧唯一真值注入
  const workingDirectory = process.env.WORKING_DIRECTORY;
  const effectiveCwd = resolveWorkingDirectoryPath(workingDirectory, process.env.USER_ID || '');
  let cwdRule;
  if (workingDirectory && workingDirectory.trim() !== '') {
  ...
  safeEnv.WORKING_DIRECTORY = effectiveCwd;
  ...
  const context = {
    ...
    setInterval,
    clearInterval,
    // Node 18+ 全局 fetch（R2-1：沙箱 HTTP 能力，供技能工具调 API）
    fetch: (...args) => {
      // 透传全局 fetch，确保 this 指向正确
...
`;
    ...
    if (workDirEnv && workDirEnv.trim() !== '') {
      ...
      workingDir = resolveWorkingDirectoryPath('', '');
    }
