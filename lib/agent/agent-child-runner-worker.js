...
const DEFAULT_TERMINAL_RETENTION_MS = 2 * 60 * 60 * 1000;
...
export class AgentChildRunnerWorker {
  constructor({
    ...
    this.terminal_retention_ms = terminal_retention_ms;
  ...
  cancel(child_run_id) {
    ...
    record.abort_controller?.abort?.(new Error('Request aborted by user'));
    if (record.status === 'queued') {
      record.status = 'cancelled';
      record.cancelled_at = nowIso();
      record.error = 'Request aborted by user';
      record.terminal_at_ms = Date.now();
    }
  ...
  async runRecord(record, delegation, options) {
    ...
    const abortController = typeof AbortController === 'function'
      ? new AbortController()
      : null;
    record.abort_controller = abortController;
  ...
  cleanupRuns() {
    if (!Number.isFinite(this.terminal_retention_ms) || this.terminal_retention_ms < 0) {
      return;
    }

    const now = Date.now();
    for (const [childRunId, record] of this.runs) {
      if (!TERMINAL_STATUSES.includes(record.status) || record.terminal_at_ms === null) {
        continue;
      }
      if (now - record.terminal_at_ms >= this.terminal_retention_ms) {
        this.runs.delete(childRunId);
      }
    }

    if (!Number.isFinite(this.max_runs) || this.max_runs <= 0 || this.runs.size <= this.max_runs) {
      return;
    }

    const removable = [...this.runs.values()]
      .filter(record => TERMINAL_STATUSES.includes(record.status))
      .sort((a, b) => (a.terminal_at_ms || 0) - (b.terminal_at_ms || 0));
    for (const record of removable) {
      if (this.runs.size <= this.max_runs) {
        return;
      }
      this.runs.delete(record.child_run_id);
    }
  }
}
