    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          input: {
            type: 'object',
            description: 'Structured task input for the child agent. For images/OCR, pass attachment references in input.attachments; do not pass raw base64.',
            properties: {
              attachments: {
                type: 'array',
                description: 'Optional delegated attachments. Use workspace_path/workspace references; the child runtime resolves them safely before the first LLM call.',
                items: {
                  type: 'object',
                  properties: {
                    type: {
                      type: 'string',
                      enum: ['image'],
                      description: 'Attachment type. Only image is supported by the current child Agent runtime.',
                    },
                    source: {
                      type: 'string',
                      enum: ['workspace_path', 'workspace'],
                      description: 'Attachment source. Paths are resolved against the delegated workspace and must stay inside it.',
                    },
                    path: {
                      type: 'string',
                      description: 'Workspace relative path or full path to the image file.',
                    },
                    purpose: {
                      type: 'string',
                      description: 'Optional purpose, such as ocr or image_analysis.',
                    },
                  },
                  required: ['type', 'path'],
                },
