...
export class AgentLoop {
  ...
  async runErix(expertService, {
    ...
    const taskDirName = taskDir ? String(taskDir).split(/[\\/]/).pop() : '';
    const briefDir = taskContext?.logical_workspace_path || taskDirName || taskDir;
    const isTaskMode = TASK_MODES.has(mode);
    const hasTaskDir = Boolean(taskDir || briefDir);
    const isChatMode = mode === 'chat' || !hasTaskDir;
    let taskBrief = null;
    if (isTaskMode && hasTaskDir) {
