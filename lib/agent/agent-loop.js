...
const TASK_MODES = new Set(['task', 'repo_task']);
const SKILL_MODE = 'skill';
const TASK_BRIEF_CODE_POINT_LIMIT = 1500;
const TASK_GOAL_CODE_POINT_LIMIT = 600;
const LATEST_USER_CODE_POINT_LIMIT = 300;
const MAX_WORKSPACE_DOC_BYTES = 262144;
...
function getUtf8SafePrefixLength(buffer, maxBytes) {
  ...
  const sequenceLength = leadingByte <= 0x7f
    ? 1
    : (leadingByte & 0xe0) === 0xc0
      ...
      : (leadingByte & 0xf0) === 0xe0
        ...
        : (leadingByte & 0xf8) === 0xf0
          ? 4
          : 0;
  return sequenceLength > 0 && start + sequenceLength <= end ? end : start;
}
...
function truncateUtf8Bytes(value, maxBytes) {
  ...
  const safeEnd = getUtf8SafePrefixLength(buffer, maxBytes);
  return buffer.subarray(0, safeEnd).toString('utf8');
}
...
function findNextMarkdownHeading(text, start) {
  const headingPattern = /^#{1,6}\s*\S.*$/gmu;
  headingPattern.lastIndex = start;
  const nextHeading = headingPattern.exec(text);
  return nextHeading ? nextHeading.index : text.length;
}

function hasMeaningfulGoalText(value) {
  const content = String(value ?? '')
    .split(/\r?\n/u)
    .map(line => line
      .replace(/^\s*#{1,6}\s*/u, '')
      .replace(/^\s*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))+/u, '')
      .replace(/^\s*(?:(?:项目|任务)?(?:目标|需求|验收|目的)|Requirements?|Objectives?|Goals?|User Story)\s*[:：]?\s*/iu, '')
      .trim())
    .filter(Boolean)
    .join('')
    .replace(/\s+/gu, '');
...
function extractGoalSummary(readmeText) {
  const text = String(readmeText ?? '');
  if (text.trim() === '') return '';

  const headingPatterns = [
    /^#{1,6}\s*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))?(?:项目|任务)?(?:目标|需求|目的)(?:\([^)]*\))?(?:\s*[:：]\s*[^\n]*)?$/gimu,
    /^#{1,6}\s*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))?(?:Requirements?|Objectives?|Goals?|User Story)\b[^\n]*$/gimu,
  ];
  ...
  headingCandidates.sort((left, right) => left.start - right.start);

  const summarizeSection = (start, end) => truncateCodePoints(
    text.slice(start, end).trim(),
    TASK_GOAL_CODE_POINT_LIMIT,
  );

  for (const candidate of headingCandidates) {
    const sectionEnd = findNextMarkdownHeading(text, candidate.end);
    const summary = summarizeSection(candidate.start, sectionEnd);
    if (hasMeaningfulGoalText(summary)) return summary;
  }

  const inlineGoal = /^[ \t]*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))?(?:(?:项目|任务)?(?:目标|需求|目的)|Requirements?|Objectives?|Goals?|User Story)\s*[:：]\s*[^\n]{5,}/gimu;
  for (const match of text.matchAll(inlineGoal)) {
    const sectionStart = match.index;
    const sectionEnd = findNextMarkdownHeading(
      text,
      sectionStart + match[0].length,
    );
    const summary = summarizeSection(sectionStart, sectionEnd);
    if (hasMeaningfulGoalText(summary)) return summary;
  }

  const explicitGoalLine = /^[ \t]*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))?(?!(?:non-goals?|non-objectives?|非目标|不在[^：]{0,20}范围))(?:项目|任务)?(?:目标|需求|目的|Goal|Objectives?|Requirements?)\s*(?:[:：]|是|为|包括|在于)/iu;
  for (const line of text.split(/\r?\n/u)) {
    if (explicitGoalLine.test(line) && hasMeaningfulGoalText(line)) {
      return truncateCodePoints(line.trim(), TASK_GOAL_CODE_POINT_LIMIT);
    }
  }
  return '';
}

async function readWorkspaceText(taskDir, fileName) {
  const filePath = join(taskDir, fileName);
  try {
    const fs = await import('node:fs/promises');
    const { size } = await fs.stat(filePath);
    const bytesToRead = Math.min(size, MAX_WORKSPACE_DOC_BYTES);
    ...
    try {
      ...
      return {
        status: 'present',
        text: decodeUtf8Prefix(buffer.subarray(0, bytesRead), bytesRead),
      };
    } finally {
    ...
    logger.warn('[AgentLoop] 工作空间文件读取失败，使用任务描述回退:', {
      path: filePath,
...
function composeTaskBrief(prefix, suffix = '') {
  const taskBriefSuffix = truncateCodePoints(suffix, LATEST_USER_CODE_POINT_LIMIT);
  const prefixBudget = Math.max(
    0,
    TASK_BRIEF_CODE_POINT_LIMIT - Array.from(taskBriefSuffix).length,
  );
  return `${truncateCodePoints(prefix, prefixBudget)}${taskBriefSuffix}`;
}
...
export class AgentLoop {
  ...
  async runErix(expertService, {
    ...
    const mode = taskContext?.workspace_mode;
    const taskDir = taskContext?.absolute_workspace_path || null;
    const isTaskMode = TASK_MODES.has(mode);
    const isSkillMode = mode === SKILL_MODE;
    const hasTaskDir = (isTaskMode || isSkillMode) && Boolean(taskDir);
    ...
    if (hasTaskDir) {
      ...
      const latestUser = typeof latestUserText === 'string'
        ? truncateCodePoints(latestUserText, LATEST_USER_CODE_POINT_LIMIT)
        : (Array.isArray(latestUserText)
          ? truncateCodePoints(
            latestUserText
              .map(block => block?.type === 'text' && typeof block.text === 'string' ? block.text : '')
              .join(' '),
            LATEST_USER_CODE_POINT_LIMIT,
          )
          : '');
      const taskDescription = truncateCodePoints(
        taskContext?.description || '',
        TASK_GOAL_CODE_POINT_LIMIT,
      );

      if (isTaskMode) {
        let readmeStatus = 'present';
        let readmeText = null;
        if (hasOwn(taskContext, 'readme')) {
          readmeText = truncateUtf8Bytes(taskContext.readme, MAX_WORKSPACE_DOC_BYTES);
        } else {
          const readmeResult = await readWorkspaceText(taskDir, 'README.md');
          readmeStatus = readmeResult.status;
          readmeText = readmeResult.text;
        }

        const extractedGoal = readmeStatus === 'present'
          ? extractGoalSummary(readmeText)
          : '';
        isGoalDefinitionPhase = readmeStatus === 'missing'
          || (readmeStatus === 'present' && extractedGoal === '');
        const goalSummary = extractedGoal || taskDescription;
        const taskBriefPrefix = isGoalDefinitionPhase
          ? `任务目录:${taskDir}\n（${readmeStatus === 'missing' ? 'README 尚无目标' : 'README 尚无有效目标'}）\n任务目标:${goalSummary}\n本 run 任务:向用户确认任务目标并写入 README.md（目标/需求/验收），未确认前不要开始实现。`
          : `任务目录:${taskDir}\n任务目标:${goalSummary}\n最新指令:`;
        taskBrief = composeTaskBrief(
          taskBriefPrefix,
          isGoalDefinitionPhase ? '' : latestUser,
        );
      } else {
        const skillResult = await readWorkspaceText(taskDir, 'SKILL.md');
        let skillSummary = '';
        if (skillResult.status === 'present' && skillResult.text?.trim()) {
          skillSummary = truncateCodePoints(skillResult.text, TASK_GOAL_CODE_POINT_LIMIT);
        } else if (hasOwn(taskContext, 'readme') && taskContext.readme?.trim()) {
          skillSummary = truncateCodePoints(taskContext.readme, TASK_GOAL_CODE_POINT_LIMIT);
        }
        skillSummary = skillSummary || taskDescription;
        taskBrief = composeTaskBrief(
          `任务目录:${taskDir}\n技能目标:${skillSummary}\n最新指令:`,
          latestUser,
        );
