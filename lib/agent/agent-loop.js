...
function extractSkillBrief(canonicalMessages, taskDir) {
  const skillName = taskDir ? taskDir.split(/[\\/]/).pop() : '';
  const systemText = canonicalMessages
    ...
    .join('\n');
  const skillsSection = systemText.match(/【可用技能】\s*([\s\S]*)/);
  if (!skillsSection) return '';
  const lines = skillsSection[1].split('\n');
  const skillLine = lines.find(line => line.includes(skillName) && line.includes(':'));
  if (!skillLine) return '';
  return truncateCodePoints(skillLine.replace(/^-\s*/, '').trim(), 300);
}
    ...
    if (hasTaskDir) {
      ...
      const latestUser = typeof latestUserText === 'string'
        ...
        : (Array.isArray(latestUserText)
          ...
          : '');
      const taskBriefPrefix = isSkillMode
        ? `任务目录:${taskDir}\n技能任务:（技能描述:${extractSkillBrief(canonicalMessages, taskDir) || '详见技能定义'}）\n最新指令:`
        : `任务目录:${taskDir}\n任务描述:${truncateCodePoints(taskContext?.description || '', 600) || '（无描述）'}\n最新指令:`;
      taskBrief = composeTaskBrief(taskBriefPrefix, latestUser);
    }
