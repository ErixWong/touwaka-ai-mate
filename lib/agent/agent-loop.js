...
function hasMeaningfulGoalText(value) {
  const content = String(value ?? '')
    ...
    .map(line => line
      .replace(/^\s*#{1,6}\s*/u, '')
      .replace(/^\s*(?:(?:项目)?(?:目标|需求|验收|Goal)|目的)\s*[:：]?\s*/iu, '')
      .trim())
    .filter(Boolean)
    .join('')
    .replace(/\s+/gu, '');
...
function extractGoalSummary(readmeText) {
  ...
  const goalHeading = /^#{1,6}\s*(?:项目)?(?:目标|需求|验收|Goal)[^#\n]*$/imu.exec(text);
  if (goalHeading) {
    const sectionStart = goalHeading.index;
    const sectionEnd = findNextMarkdownHeading(
      text,
      sectionStart + goalHeading[0].length,
    );
    const summary = truncateCodePoints(
      text.slice(sectionStart, sectionEnd).trim(),
      TASK_GOAL_CODE_POINT_LIMIT,
    );
    return hasMeaningfulGoalText(summary) ? summary : '';
  }

  const inlineGoal = /(?:^|\n)(?:目标|需求|验收)[:：]\s*([^\n]{5,})/mu.exec(text);
  if (inlineGoal) {
    const sectionStart = inlineGoal.index + (inlineGoal[0].startsWith('\n') ? 1 : 0);
    const sectionEnd = findNextMarkdownHeading(
      text,
      sectionStart + inlineGoal[0].length,
    );
    const summary = truncateCodePoints(
      text.slice(sectionStart, sectionEnd).trim(),
      TASK_GOAL_CODE_POINT_LIMIT,
  ...
  const explicitGoalLine = /(?:项目)?(?:目标|需求|验收|目的)\s*(?:[:：]|是|为|包括|在于)|(?:请|需要|完成|实现|确保|交付|产出)(?=.{4,})/u;
  const goalLines = text
    .split(/\r?\n/u)
    .filter(line => explicitGoalLine.test(line));
  const summary = truncateCodePoints(goalLines.join('\n').trim(), TASK_GOAL_CODE_POINT_LIMIT);
  return hasMeaningfulGoalText(summary) ? summary : '';
}
