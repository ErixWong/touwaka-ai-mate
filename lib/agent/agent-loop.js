...
function hasMeaningfulGoalText(value) {
  const content = String(value ?? '')
    ...
    .map(line => line
      .replace(/^\s*#{1,6}\s*/u, '')
      .replace(/^\s*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))+/u, '')
      .replace(/^\s*(?:(?:项目|任务)?(?:目标|需求|验收|目的)|Requirements?|Objectives?|Goals?|Acceptance(?: Criteria)?|User Story)\s*[:：]?\s*/iu, '')
      .trim())
    .filter(Boolean)
    .join('')
    .replace(/\s+/gu, '');
...
function extractGoalSummary(readmeText) {
  ...
  const headingPatterns = [
    /^#{1,6}\s*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))?(?:项目|任务)?(?:目标|需求|验收|目的)(?:\([^)]*\))?(?:\s*[:：]\s*[^\n]*)?$/gimu,
    /^#{1,6}\s*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))?(?:Requirements?|Objectives?|Goals?|Acceptance Criteria|User Story)\b[^\n]*$/gimu,
  ];
  ...
  const inlineGoal = /^[ \t]*(?:(?:\d+[.、]\s*)|(?:[-*+]\s*))?(?:(?:项目|任务)?(?:目标|需求|验收|目的)|Requirements?|Objectives?|Goals?|Acceptance(?: Criteria)?|User Story)\s*[:：]\s*[^\n]{5,}/gimu;
  ...
  const explicitGoalLine = /(?:项目|任务)?(?:目标|需求|验收|目的|Goal|Objective|Requirement|Acceptance)[\s\S]{0,10}?(?:[:：]|是|为|包括|在于|——|-)/iu;
