...
import { isNotesEnabled } from '../psyche/notes-config.js';
...
function buildChildToolContext({ expert_service, invocation_context, session, taskContext = null }) {
  const user_id = invocation_context.principal_user_id;
  const expert_id = invocation_context.callee_agent_id;
  const expertConfig = expert_service?.expertConfig || {};
  ...
  return {
    ...
    session,
    taskContext,
    context_strategy: expertConfig.expert?.context_strategy || 'full',
    enable_notes: isNotesEnabled(expertConfig),
  };
}
...
export async function getExpertChildScopedTools({
  ...
  const toolContext = buildChildToolContext({ expert_service, invocation_context, session, taskContext });
