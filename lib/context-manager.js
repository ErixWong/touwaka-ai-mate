...
class ContextManager {
  ...
  async buildContext(memorySystem, userId, options = {}) {
    ...
    // 7. 构建系统提示（包含 Skills、Task Context、Topic 总结和引导提示）
...
`;
  ...
  enhanceWithTaskContext(systemPrompt, taskContext) {
    ...
    const currentPathDisplay = taskContext.currentPath
      ? `${taskContext.workspacePath}/${taskContext.currentPath}`
      : `${taskContext.workspacePath}/input`;
...
**工作空间根目录**: ${taskContext.workspacePath}
**当前浏览目录**: ${currentPathDisplay}

### 当前目录下的文件
${filesDescription}

**重要提示**：
- 你可以使用文件操作工具读取当前目录中的文件内容
- 处理结果可以保存到 output 目录（路径: ${taskContext.workspacePath}/output）
- 临时文件可以放在 temp 目录（路径: ${taskContext.workspacePath}/temp）
- 请根据任务需求合理使用工具，如果需要访问文件，请先告知用户你要做什么
`;
