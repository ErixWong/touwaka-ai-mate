 ...
 * - workspace prompt 渲染已统一委托给 BaseContextOrganizer
 */

import logger from './logger.js';
import LLMClient from './llm-client.js';
import { ContextOrganizerFactory } from './context-organizer/index.js';
import { BaseContextOrganizer } from './context-organizer/base-organizer.js';
...
class ContextManager {
  ...
  buildToolMessageSummary(options) {
    ...
    if (strategy === 'simple') {
      // Simple 策略：极简摘要
      return `${toolName} → ${contentLength} 字符 | recall({ mode: 'messages', action: 'detail', message_id: "${toolCallId}" })`;
    }

    // Full 策略：详细摘要
    return `工具: ${toolName}
结果: ${contentLength} 字符
→ 调用 recall({ mode: 'messages', action: 'detail', message_id: "${toolCallId}" }) 获取完整结果`;
  }
   ...
   * @param {string} ragContext - ⚠️ Legacy: 旧自动预检索路径的字符串注入输入。
   *   新主路径已迁移到 evidence packet → _getResponseModeDecision。
   *   此参数仅在旧自动预检索兼容路径中使用，计划随旧路径退场移除。
...
/**
   * 生成任务上下文段落（根据模式分发）
   * 现在委托给 BaseContextOrganizer 的统一渲染方法
  ...
  generateTaskContextSection(taskContext) {
    ...
    logger.info(`[ContextManager] generateTaskContextSection: 模式=${workspaceMode || 'task'}, 路径=${fullPath}`);

    const baseOrganizer = new BaseContextOrganizer(this.expertConfig);
    return baseOrganizer.generateTaskContextSection(taskContext);
  }

  /**
   * 生成任务工作空间段落（任务模式）
   * @deprecated 现在委托给 BaseContextOrganizer 处理
   * @param {object} taskContext - 任务上下文
   * @returns {string} 任务工作空间段落内容
   */
  generateTaskWorkspaceSection(taskContext) {
    const baseOrganizer = new BaseContextOrganizer(this.expertConfig);
    return baseOrganizer.generateTaskWorkspaceSection(taskContext);
  }
  ...
  generateSkillContextSection(taskContext) {
    const baseOrganizer = new BaseContextOrganizer(this.expertConfig);
    return baseOrganizer.generateSkillContextSection(taskContext);
  }
...
generateChatContextSection(taskContext) {
    const baseOrganizer = new BaseContextOrganizer(this.expertConfig);
    return baseOrganizer.generateChatContextSection(taskContext);
  }

  /**
   * 生成路径权限范围说明段落
   * @deprecated 此方法仅用于历史兼容，不再被主路径使用。路径权限逻辑以 base-organizer + view model 为准。
   * @param {string} userId - 用户ID
...
${topicSummaries}`;
  ...
  /**
   * 生成文档检索上下文段落（Legacy: 旧自动预检索兼容）
   *
   * ⚠️ Legacy：此方法仅服务于旧自动预检索路径的字符串注入。
   * 新主路径已迁移到 evidence packet → _getResponseModeDecision，不经过此方法。
   * 新逻辑不应继续依赖此方法——请使用 buildEvidenceInjection / _getResponseModeDecision。
   * 计划随旧自动预检索路径退场后移除此方法。
   *
   * @param {string} ragContext - 文档检索上下文文本（旧自动路径输出的字符串）
   * @returns {string|null} 上下文段落内容
   */
  generateRAGContextSection(ragContext) {
    if (!ragContext) return null;

    return `## 相关文档平台内容
以下是从文档平台中检索到的相关内容，请参考这些信息回答用户问题。
如果检索内容不足以支撑确定性的回答，请明确告知用户当前依据有限：
