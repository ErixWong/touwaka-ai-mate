...
export class MinimalContextOrganizer extends BaseContextOrganizer {
  ...
  async _initialize(llmClient) {
    ...
    // P0 修复：使用全局共享 NotesStore（与 ToolManager 共用同一实例）
    const { getSharedNotesStore } = await import('../psyche-store/index.js');
    const notesStore = await getSharedNotesStore();
...
- 如需查看笔记内容，使用 note_read 工具
- 如需保存新材料，使用 note_take 工具
- 如需查看所有笔记，使用 note_list 工具
- note_* 是首选名称，兼容别名 notes_* 也可使用`);
    } else {
      parts.push(`
...
- note_take 内容需先摘要到 4000 字符以内；note_read 命中后会滑动续期
