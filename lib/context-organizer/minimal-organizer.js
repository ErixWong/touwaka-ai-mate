 ...
 * 
 * 注意：workspace prompt 现已统一使用 base-organizer 中的 view model builder 和 renderer
 */
...
import { RECALL_USAGE_GUIDANCE } from './recall-guidance.js';
...
export class MinimalContextOrganizer extends BaseContextOrganizer {
  constructor(expertConfig = {}, options = {}) {
    ...
    // 从 expertConfig.expert.psyche_config 读取配置，options.psycheConfig 可覆盖
    const psycheConfig = options.psycheConfig || expertConfig?.expert?.psyche_config || {};
    const reflectiveModel = expertConfig?.reflectiveModel || {};
    const expressiveModel = expertConfig?.expressiveModel || {};
    
    this.config = {
      lookbackRounds: psycheConfig.reflection_lookback || options.lookbackRounds || 4,
      maxTokensRatio: psycheConfig.max_tokens_ratio || options.maxTokensRatio || 0.3,
      enableNotes: psycheConfig.enable_notes !== false && options.enableNotes !== false,
      reflectionModel: reflectiveModel.model_name || 'gpt-4o-mini',
      reflectionContextSize: reflectiveModel.max_tokens || 128000,
      reflectionInputRatio: 0.85,
      expressiveContextSize: expressiveModel.max_tokens || 128000,
    };

    logger.info(`[MinimalContextOrganizer] 配置加载完成: lookbackRounds=${this.config.lookbackRounds}, maxTokensRatio=${this.config.maxTokensRatio}, enableNotes=${this.config.enableNotes}`);
  ...
  async _initialize(llmClient) {
    ...
    // 创建存储工厂（PsycheStore 保持独立，NotesStore 使用全局共享实例）
    ...
    // Phase 3 阶段三：宿主笔记走 PsycheNotesFacade（MariaDB CAS NoteRecord，issue #1132 决策 3 修订）。
    // facade 不可用时（如 db 连接失败）降级为 null，compress 跳过笔记迁移，
    // 不阻断上下文组织。
    let notesFacade = null;
    try {
      const { getSharedPsycheNotesFacade } = await import('../notes/index.js');
      notesFacade = await getSharedPsycheNotesFacade();
    } catch (error) {
      logger.error('[MinimalContextOrganizer] PsycheNotesFacade 初始化失败，压缩时将跳过笔记迁移:', error.message);
    }

    // 创建 Psyche 管理器
    this._psycheManager = new PsycheManager(psycheStore, notesFacade, {
  ...
  async organize(memorySystem, userId, options = {}) {
    ...
    if (recentMessages.length > 0) {
      try {
        ...
        // P0 观测：记录反思来源
        logger.info(`[MinimalContextOrganizer] 反思完成 (source=psyche), 专家: ${expertId}, 对话轮数: ${dialogCount}`);
        
        logger.debug(`[MinimalContextOrganizer] 反思结果:`, JSON.stringify(reflection, null, 2).substring(0, 500));
    ...
    const rawSystemPrompt = this._buildSystemPrompt(baseSystemPrompt, psycheText, taskContext);
    const budget = this.getBudgetConfig({ maxTokens, contextThreshold });
    const systemPrompt = this.truncateTextToTokenBudget(rawSystemPrompt, budget.systemBudget);
    const systemPromptBudgetStats = {
      beforeTokens: estimateTokens(rawSystemPrompt),
      afterTokens: estimateTokens(systemPrompt),
      systemBudget: budget.systemBudget,
      truncated: systemPrompt !== rawSystemPrompt,
    };
    if (systemPromptBudgetStats.truncated) {
      logger.info('[MinimalContextOrganizer] system prompt budget applied:', systemPromptBudgetStats);
    }

    // 7. 构建 messages 数组
    // 包含：最近4轮对话（user+assistant+tool）+ 当前用户消息
    const rawMessages = this._buildMessagesArray(recentMessages, currentMessage);
    const budgeted = this.applyMessageBudget(
      [{ role: 'system', content: systemPrompt }, ...rawMessages],
      { maxTokens, contextThreshold }
    );
    const messages = budgeted.messages.filter((msg, index) => !(index === 0 && msg.role === 'system'));
    ...
    return new ContextResult({
      ...
      metadata: {
        ...
        budget: {
          systemPrompt: systemPromptBudgetStats,
          messages: budgeted.stats,
        },
      }
    });
  }
  ...
  _buildSystemPrompt(basePrompt, psycheText, taskContext = null) {
    ...
    // P1-1 修复：根据 enableNotes 配置条件添加 notes 使用说明
    if (this.config.enableNotes !== false) {
      parts.push(`

【使用说明】
- 以上【心神】是你的工作记忆，包含当前主题、关键决策和可用笔记
- 如需保存新材料，使用 note_take 工具（内容需先摘要到 4000 字符以内）
- 如需查看笔记内容，使用 note_read 工具（按精确 key 读取）
- 如需查看所有笔记，使用 note_list 工具
- 如需删除笔记，使用 note_forget 工具（物理删除，不可恢复）`);
    } else {
      parts.push(`

【使用说明】
- 以上【心神】是你的工作记忆，包含当前主题和关键决策`);
    }
...
- note_take 内容上限 4000 字符；note_read 读取不会滑动续期（命中也不延长过期时间）
- 长期记忆依靠 Topic 归档与 recall，不要把 Notes 当作永久存储`);
  ...
  /**
   * 构建任务上下文段落
   * 现在委托给 base-organizer 中的统一 view model builder 和 renderer
  ...
  _buildTaskContextSection(taskContext) {
    ...
    return this.generateTaskContextSection(taskContext);
  }
