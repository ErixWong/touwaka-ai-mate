...
- 以上【心神】是你的工作记忆，包含当前主题和关键决策`);
  ...
  _buildTaskContextSection(taskContext) {
    ...
    } else if (taskContext.id && taskContext.title) {
  ...
  /**
    const fullPath = taskContext.logical_workspace_path || '';
...
_buildSkillContextSection(taskContext) {
    const fullPath = taskContext.logical_workspace_path || 'skills/unknown';
...
- ⚠️ 技能目录是只读的，不应该写入文件`;
  ...
  _buildChatContextSection(taskContext) {
    const userId = taskContext.userId || 'unknown';
    const fullPath = taskContext.logical_workspace_path || `${userId}/temp`;
...
### 路径使用规则
- 使用相对当前工作目录的路径，例如 \`input/file.xlsx\`
- 可以读取临时文件夹中的现有文件
