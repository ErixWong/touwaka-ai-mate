...
export function buildWorkspacePromptViewModel(taskContext) {
  ...
  const userId = taskContext.userId || taskContext.user_id || 'unknown';
  const logicalPath = taskContext.logical_workspace_path || '';
  const currentPath = taskContext.currentPath || taskContext.current_path || '';
  ...
  const vm = {
    ...
    permission_scope: getPermissionScope(userId, taskContext.isAdmin, taskContext.isSkillCreator),
    permission_note: getPermissionNote(userId, taskContext.isAdmin, taskContext.isSkillCreator),
    can_write: mode === 'task',
    can_read: true,
    ...
    files_description: formatFilesDescription(taskContext.inputFiles),
    files_count: taskContext.inputFiles ? taskContext.inputFiles.length : 0,
    has_files: taskContext.inputFiles && taskContext.inputFiles.length > 0,
    ...
    entity_id: taskContext.id || taskContext.task_id || null,
    ...
    user_id: userId,
    user_role_label: getUserRoleLabel(taskContext.isAdmin, taskContext.isSkillCreator),
  };
