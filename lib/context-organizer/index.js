...
import { MinimalContextOrganizer } from './minimal-organizer.js';

// 可用的策略列表
const AVAILABLE_STRATEGIES = ['full', 'simple', 'minimal'];

/**
 * 创建上下文组织器（静态方法）
 * @param {string} strategy - 策略名称 ('full' | 'simple' | 'minimal')
...
export function createContextOrganizer(strategy, expertConfig, options = {}) {
  switch (strategy) {
    ...
    case 'minimal':
      return new MinimalContextOrganizer(expertConfig, options);
    default:
      // 默认使用 full 策略
      return new FullContextOrganizer(expertConfig, options);
  }
...
export function getAvailableStrategies() {
  return [
    ...
    { name: 'minimal', description: '精简上下文组织策略：使用 Psyche 替代原始 Messages，支持 Notes 存取' },
  ];
}
...
export function createOrganizerFactory(expertConfig) {
  ...
  // 注册精简上下文组织器
  organizerFactory.register('minimal', new MinimalContextOrganizer(expertConfig));
