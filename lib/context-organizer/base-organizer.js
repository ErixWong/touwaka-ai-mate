...
**注意**：如果信息不足，先询问用户再调用工具。`;
  ...
  generateTaskContextSection(taskContext) {
    ...
    const workspaceMode = taskContext.workspace_mode;

    if (workspaceMode === 'skill') {
      return this.generateSkillContextSectionFromViewModel(viewModel, taskContext);
    } else if (workspaceMode === 'chat') {
      return this.generateChatContextSectionFromViewModel(viewModel, taskContext);
    } else {
      return this.generateTaskWorkspaceSectionFromViewModel(viewModel, taskContext);
    }
  }
   ...
   */
  generateTaskWorkspaceSectionFromViewModel(viewModel, taskContext) {
    const { workspace_path_display, current_path_display, workspace_path_formatted,
            ...
            show_readme, readme_content, show_todo, todo_content } = viewModel;

    const entityId = taskContext.id || taskContext.task_id || '';
    const entityTitle = taskContext.title || '';
    const entityDescription = taskContext.description || '';
...
- **任务ID**: ${entityId}
- **任务标题**: ${entityTitle}
${entityDescription ? `- **任务描述**: ${entityDescription}` : ''}
...
${files_description}`;
   ...
   */
  generateSkillContextSectionFromViewModel(viewModel, taskContext) {
...
- **权限说明**: ${permission_note}`;
   ...
   */
  generateChatContextSectionFromViewModel(viewModel, taskContext) {
...
请友好地引导用户创建任务来处理需要文件操作的需求。`;
  ...
  generateTaskWorkspaceSection(taskContext) {
    const viewModel = buildWorkspacePromptViewModel(taskContext);
    return viewModel ? this.generateTaskWorkspaceSectionFromViewModel(viewModel, taskContext) : null;
  }
  ...
  generateSkillContextSection(taskContext) {
    const viewModel = buildWorkspacePromptViewModel(taskContext);
    return viewModel ? this.generateSkillContextSectionFromViewModel(viewModel, taskContext) : null;
  }
  ...
  generateChatContextSection(taskContext) {
    const viewModel = buildWorkspacePromptViewModel(taskContext);
    return viewModel ? this.generateChatContextSectionFromViewModel(viewModel, taskContext) : null;
  }
...
- **权限说明**: ${permissionNote}`;
  ...
  generateTopicSummariesSection(topicSummaries, topicCount = 10, messageCount = 15) {
    ...
    return `## 之前的对话话题总结
以下是最新的 ${topicCount} 个话题摘要，以及最近 ${messageCount} 条对话消息。这只是部分历史，如果需要更多历史话题详情，可以使用 \`recall\` 工具：
...
${guidance}`;
   ...
   * @param {string|null} ragContext - RAG 上下文
   * @param {object} options - 可选配置
   * @param {number} options.topicCount - Topic 数量（用于提示说明）
   * @param {number} options.messageCount - 消息数量（用于提示说明）
   */
  buildBaseSystemPrompt(innerVoices = [], topicSummaries = null, userInfoGuidance = null, skills = [], taskContext = null, ragContext = null, options = {}) {
    ...
    if (topicSummaries) {
      systemPrompt = this.appendSection(systemPrompt, this.generateTopicSummariesSection(topicSummaries, topicCount, messageCount));
    }
