...
${topicSummaries}`;
  ...
  /**
   * 生成 RAG 上下文段落（Legacy: 旧自动预检索兼容）
   *
   * ⚠️ Legacy：仅服务于旧自动预检索路径的字符串注入。
   * 新主路径使用 evidence packet → _getResponseModeDecision，不经过此方法。
   * 计划随旧路径退场后移除。
   */
...
${guidance}`;
   ...
   * @param {string|null} ragContext - ⚠️ Legacy: 旧自动预检索字符串注入（计划退场）
   * @param {object} options - 可选配置
