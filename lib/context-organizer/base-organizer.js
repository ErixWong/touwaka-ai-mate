...
import { buildWorkspacePromptViewModel } from './workspace-view-model.js';
import { estimateMessagesTokens, estimateTokens, safeTruncate } from '../token-utils.js';
import { buildRecallUsageGuidance } from './recall-guidance.js';
...
export class BaseContextOrganizer extends IContextOrganizer {
  ...
  constructor(expertConfig, options = {}) {
    ...
    this.budgetConfig = {
      maxTokens: options.maxTokens || expressiveModel.max_tokens || 128000,
      contextThreshold: options.contextThreshold || expert.context_threshold || 0.7,
      systemPromptRatio: options.systemPromptRatio || 0.35,
    };
  }
...
/**
   * 生成任务上下文段落（根据模式分发）
   * 使用统一的 view model builder 和 renderer
   */
  generateTaskContextSection(taskContext) {
    if (!taskContext) return null;

    const viewModel = buildWorkspacePromptViewModel(taskContext);
    if (!viewModel) return null;

    const mode = viewModel.mode;

    if (mode === 'skill') {
      return this.generateSkillContextSectionFromViewModel(viewModel);
    } else if (mode === 'chat') {
      return this.generateChatContextSectionFromViewModel(viewModel);
    } else {
      return this.generateTaskWorkspaceSectionFromViewModel(viewModel);
    }
  }

  /**
   * 生成任务工作空间段落（任务模式）- 使用 View Model
   */
  generateTaskWorkspaceSectionFromViewModel(viewModel) {
    const { workspace_path_display, current_path_display, workspace_path_formatted,
            files_description, permission_scope, permission_note,
            show_readme, readme_content, show_todo, todo_content,
            entity_id, entity_title, entity_description } = viewModel;

    let permissionSection = `### 路径使用规则
...
### ⚠️ 路径权限范围
- **可访问目录**: ${permission_scope}
- **当前任务目录**: ${workspace_path_formatted}
- **路径格式**: 相对当前工作目录，不要写绝对路径
- **权限说明**: ${permission_note}`;
...
- **任务ID**: ${entity_id || ''}
- **任务标题**: ${entity_title || ''}
${entity_description ? `- **任务描述**: ${entity_description}` : ''}
${show_readme ? readme_content : ''}${show_todo ? todo_content : ''}
### 目录说明
当前目录是一个任务目录，可以根据用户的需要组织合适的目录结构。

- **工作目录**: ${workspace_path_display}
- **当前浏览**: ${current_path_display}
...
${files_description}`;
  ...
  /**
   * 生成技能目录段落（技能模式）- 使用 View Model
   */
  generateSkillContextSectionFromViewModel(viewModel) {
    const { workspace_path_display, workspace_path_formatted, skill_name,
            permission_scope, permission_note } = viewModel;
...
### 路径使用规则
- 当前工作目录: ${workspace_path_formatted}
...
- **权限说明**: ${permission_note}`;
  ...
  /**
   * 生成对话模式段落 - 使用 View Model
   */
  generateChatContextSectionFromViewModel(viewModel) {
    const { workspace_path_display, workspace_path_formatted,
            permission_scope, permission_note, path_warning } = viewModel;

    return `## 当前工作目录

你正在**对话模式**中，当前工作目录是用户的临时文件夹：${workspace_path_formatted}

### 路径使用规则
- 可以读取临时文件夹中的现有文件
- ${path_warning ? path_warning : '⚠️ **禁止创建文件**：对话模式不支持文件创建操作'}
...
请友好地引导用户创建任务来处理需要文件操作的需求。`;
  ...
  /**
   * 兼容旧方法：生成任务工作空间段落（任务模式）
   * @deprecated 请使用 generateTaskWorkspaceSectionFromViewModel
   */
  generateTaskWorkspaceSection(taskContext) {
    const viewModel = buildWorkspacePromptViewModel(taskContext);
    return viewModel ? this.generateTaskWorkspaceSectionFromViewModel(viewModel) : null;
  }

  /**
   * 兼容旧方法：生成技能目录段落（技能模式）
   * @deprecated 请使用 generateSkillContextSectionFromViewModel
   */
  generateSkillContextSection(taskContext) {
    const viewModel = buildWorkspacePromptViewModel(taskContext);
    return viewModel ? this.generateSkillContextSectionFromViewModel(viewModel) : null;
  }

  /**
   * 兼容旧方法：生成对话模式段落
   * @deprecated 请使用 generateChatContextSectionFromViewModel
   */
  generateChatContextSection(taskContext) {
    const viewModel = buildWorkspacePromptViewModel(taskContext);
    return viewModel ? this.generateChatContextSectionFromViewModel(viewModel) : null;
  }
...
- **权限说明**: ${permissionNote}`;
   ...
   * @param {number} topicCount - 实际注入的 Topic 数量（可选）
   * @param {number} messageCount - 实际注入的消息数量（可选）
   */
  generateTopicSummariesSection(topicSummaries, topicCount = 10, messageCount = 15) {
    if (!topicSummaries) return null;

    return `## 之前的对话话题总结
${buildRecallUsageGuidance({ topicCount, messageCount })}
...
${guidance}`;
  ...
  getBudgetConfig(options = {}) {
    const maxTokens = options.maxTokens || this.budgetConfig.maxTokens || 128000;
    const contextThreshold = options.contextThreshold || this.budgetConfig.contextThreshold || 0.7;
    const systemPromptRatio = options.systemPromptRatio || this.budgetConfig.systemPromptRatio || 0.35;
    const totalBudget = Math.floor(maxTokens * contextThreshold);
    const systemBudget = Math.min(totalBudget, Math.max(512, Math.floor(totalBudget * systemPromptRatio)));
  ...
  composeBudgetedSystemPrompt(sections, options = {}) {
    const budget = this.getBudgetConfig(options);
    const activeSections = (sections || [])
      .filter(section => section?.content)
      .map(section => ({ ...section }));

    const estimate = () => estimateTokens(activeSections.map(section => section.content).join('\n\n'));
    const beforeTokens = estimate();
    const removed = [];

    for (const priority of [3, 2, 1]) {
      while (estimate() > budget.systemBudget) {
        const removeIndex = activeSections
          .map((section, index) => ({ section, index }))
          .reverse()
          .find(item => item.section.priority === priority)?.index;

        if (removeIndex === undefined) break;
        const [section] = activeSections.splice(removeIndex, 1);
        removed.push(section.name || `priority_${priority}`);
      }
    }

    if (estimate() > budget.systemBudget) {
      const truncateCandidates = ['task_context', 'soul', 'timestamp', 'base_prompt'];
      for (const name of truncateCandidates) {
        if (estimate() <= budget.systemBudget) break;
        const section = activeSections.find(item => item.name === name);
        if (!section?.content) continue;

        const otherTokens = estimateTokens(
          activeSections
            .filter(item => item !== section)
            .map(item => item.content)
            .join('\n\n')
        );
        const sectionBudget = Math.max(64, budget.systemBudget - otherTokens);
        const truncated = this.truncateTextToTokenBudget(section.content, sectionBudget);
        if (truncated !== section.content) {
          section.content = truncated;
          removed.push(`${name}:truncated`);
        }
      }
    }

    let prompt = activeSections.map(section => section.content).join('\n\n');
    if (estimateTokens(prompt) > budget.systemBudget) {
      prompt = this.truncateTextToTokenBudget(prompt, budget.systemBudget);
      removed.push('core:truncated');
    }
    const afterTokens = estimateTokens(prompt);

    if (removed.length > 0 || afterTokens > budget.systemBudget) {
      logger.info('[BaseContextOrganizer] system prompt budget applied:', {
        beforeTokens,
        afterTokens,
        systemBudget: budget.systemBudget,
        removed,
    ...
    return {
      ...
      stats: {
        ...
        systemBudget: budget.systemBudget,
        removed,
        overBudget: afterTokens > budget.systemBudget,
      },
    };
  }

  applyMessageBudget(messages, options = {}) {
    const budget = this.getBudgetConfig(options);
    const result = [...(messages || [])];
    const beforeTokens = estimateMessagesTokens(result);
    let removed = 0;

    const currentUserMessage = (() => {
      for (let i = result.length - 1; i >= 0; i--) {
        if (result[i]?.role === 'user') return result[i];
      }
      return null;
    })();

    while (result.length > 1 && estimateMessagesTokens(result) > budget.totalBudget) {
      const removeIndex = result.findIndex((msg, index) => (
        msg.role !== 'system' && msg !== currentUserMessage
      ));
    ...
    if (removed > 0 || afterTokens > budget.totalBudget) {
      logger.info('[BaseContextOrganizer] message budget applied:', {
        beforeTokens,
        afterTokens,
    ...
    return {
      ...
      stats: {
        ...
        totalBudget: budget.totalBudget,
        removed,
        overBudget: afterTokens > budget.totalBudget,
      },
    };
  }

  truncateTextToTokenBudget(text, tokenBudget, suffix = '\n\n[... 内容超出上下文预算已截断 ...]') {
    const content = text || '';
    if (estimateTokens(content) <= tokenBudget) return content;

    const charsPerToken = content.length / Math.max(1, estimateTokens(content));
    const suffixTokens = estimateTokens(suffix);
    const safeLength = Math.max(0, Math.floor((tokenBudget - suffixTokens) * charsPerToken * 0.9));
    return safeTruncate(content, safeLength, suffix);
  }
   ...
   * - 职责分离，便于测试和维护
   * 
   * @param {Array} innerVoices - Inner Voices 数组
   * @param {string|null} topicSummaries - Topic 总结文本
   * @param {string|null} userInfoGuidance - 用户信息引导
   * @param {Array} skills - 技能列表
   * @param {object|null} taskContext - 任务上下文
   * @param {object} options - 可选配置
   * @param {number} options.topicCount - Topic 数量（用于提示说明）
   * @param {number} options.messageCount - 消息数量（用于提示说明）
   */
  buildBaseSystemPrompt(innerVoices = [], topicSummaries = null, userInfoGuidance = null, skills = [], taskContext = null, options = {}) {
    const { topicCount = 10, messageCount = 15 } = options;
    const expert = this.expertConfig.expert || this.expertConfig;

    const sections = [
      { name: 'base_prompt', priority: 0, content: expert.prompt_template || expert.system_prompt || expert.introduction || '' },
      { name: 'timestamp', priority: 0, content: this.generateTimestampSection() },
      { name: 'soul', priority: 0, content: this.generateSoulSection(this.soul, expert) },
      { name: 'skills', priority: 3, content: skills?.length > 0 ? this.generateSkillsSection(skills) : null },
      { name: 'task_context', priority: 0, content: taskContext ? this.generateTaskContextSection(taskContext) : null },
      { name: 'topic_summaries', priority: 1, content: topicSummaries ? this.generateTopicSummariesSection(topicSummaries, topicCount, messageCount) : null },
      { name: 'inner_voices', priority: 2, content: innerVoices.length > 0 ? this.generateInnerVoicesSection(innerVoices) : null },
      { name: 'user_info_guidance', priority: 3, content: userInfoGuidance ? this.generateUserInfoGuidanceSection(userInfoGuidance) : null },
    ];

    const result = this.composeBudgetedSystemPrompt(sections, options);
    this.lastSystemPromptBudgetStats = result.stats;
