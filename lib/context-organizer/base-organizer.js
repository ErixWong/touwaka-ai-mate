...
- **权限说明**: ${permissionNote}`;
   ...
   */
  generateTopicSummariesSection(topicSummaries, topicCount = 10) {
    if (!topicSummaries) return null;

    return `## 之前的对话话题总结
以下是最新的 ${topicCount} 个话题，帮助你了解对话历史。如果需要更多历史话题详情，可以使用 \`recall\` 工具通过 Topic ID 获取：
...
${guidance}`;
  ...
  buildBaseSystemPrompt(innerVoices = [], topicSummaries = null, userInfoGuidance = null, skills = [], taskContext = null, ragContext = null, options = {}) {
    ...
    if (topicSummaries) {
      systemPrompt = this.appendSection(systemPrompt, this.generateTopicSummariesSection(topicSummaries, topicCount));
    }
