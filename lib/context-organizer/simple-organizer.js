...
export class SimpleContextOrganizer extends BaseContextOrganizer {
  ...
  async organize(memorySystem, userId, options = {}) {
    ...
    const systemPrompt = this.buildBaseSystemPrompt(
      ...
      skills,
      taskContext,
      { topicCount: this.topicCount, messageCount: this.messageCount, maxTokens, contextThreshold }
    );

    // 8. 构建 LLM 消息数组（传入工具内容截断配置）
    // 注意：recentMessages 已经是按时间倒序获取的，需要反转为正序
    const rawMessages = this.buildMessages(systemPrompt, [...recentMessages].reverse(), currentMessage, {
      maxToolContentLength: this.maxToolContentLength,
    });
    const budgetedMessages = this.applyMessageBudget(rawMessages, { maxTokens, contextThreshold });
    const messages = budgetedMessages.messages;

    logger.info(`[SimpleContextOrganizer] 上下文组织完成: messages=${messages.length}, tokens≈${budgetedMessages.stats.afterTokens}`);
    ...
    return new ContextResult({
      ...
      metadata: {
        ...
        budget: {
          systemPrompt: this.lastSystemPromptBudgetStats,
          messages: budgetedMessages.stats,
        },
      },
    });
  }
