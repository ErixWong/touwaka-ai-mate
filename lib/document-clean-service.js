...
import { getStageDefault, createCallLlmFn, normalizeStageConfig } from './doc-pipeline-defaults.js';
...
// Round 02 实验用 Prompt：取消表格保护后，LLM 直接看到表格并可修复
...
class DocumentCleanService {
  ...
  async cleanWithRetry(callLlm, stageConfig, options) {
    // 不自动重试：失败即向上抛出，由上层统一标记 error / timeout，仅保留手动重试入口
  ...
  parseLlmJsonResponse(response) {
    ...
    // 处理 createCallLlmFn (parseJsonLikeContent) 返回的 _parse_failed 包装对象，
    // 此时 _raw 中保存了 LLM 原始返回文本，尝试再解析一次（如提取 JSON 块）
  ...
  normalizeMarkdownTable(table) {
    ...
    // Happy path: all rows have consistent column count — fast normalize
    if (columnCount && parsed.every(row => row.length === columnCount)) {
      const normalized = [];
      normalized.push(`| ${parsed[0].join(' | ')} |`);

      // Check whether line 1 is a real separator; if not, treat it as a data row
    ...
    // Repair path: column counts are inconsistent — apply heuristic fix
  ...
  /**
   * Heuristically repair a Markdown table with inconsistent column counts.
   * Strategy: use header row column count as baseline; pad short rows /
   * merge overflow cells into the last column.
   * Falls back to the original table string if repair is impossible.
   */
  repairAndBuildMarkdownTable(lines, parsed) {
    // parsed[0] = header row, parsed[1] ≈ separator row, parsed[2+] = data rows
    let baseColumnCount = parsed[0]?.length || 0;

    // Fallback: use the most common column count across all rows
    ...
    // Check whether parsed[1] is a real separator row
    const hasSeparator = parsed.length > 1 && this.isMarkdownSeparatorRow(lines[1]);
  ...
  /**
   * Detect whether a line is a Markdown table separator row, e.g. |---|:---:|---|
   */
  ...
  /**
   * Fit a parsed row (array of cell strings) to targetColumnCount.
   * - Exact match → return as-is (shallow copy).
   * - Too few   → pad with empty strings.
   * - Too many  → merge overflow cells into the last column (handles OCR spill).
   */
  fitRowToColumnCount(cells, targetColumnCount) {
    ...
    // cells.length > targetColumnCount: merge extra cells into last column
  ...
  /**
   * Build a normalized Markdown table string from header + body rows.
   */
