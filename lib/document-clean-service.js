...
import { getSourceAttachment } from './doc-source-attachment.js';
...
class DocumentCleanService {
  ...
  async clean(documentId, options = {}) {
    ...
    try {
      ...
      const cleanResult = stageConfig.enabled === false
        ? { text: precleanedText, publish_date: null }
      ...
      const publishDate = this._normalizePublishDate(cleanResult.publish_date);

      const finalText = this.finalizeText(
        cleanResult.text || precleanedText || originalText,
      ...
      metadata.cleaning_summary = {
        ...
        publish_date_extracted: publishDate || null,
      ...
      // 回写发布日期（非致命；revision 已有日期则不覆盖）
      if (publishDate) {
        try {
          await this._tryWriteBackPublishDate(ctx.revision, publishDate);
        } catch (dateErr) {
          logger.warn(`[DocumentCleanService] Publish date writeback failed (non-fatal): ${dateErr.message}`);
        }
      }
      ...
      // 使用原始上传文件名作为文档标题（比 OCR 正文 H1 更准确）
      try {
        const sourceAttachment = await getSourceAttachment(this.db, ctx.revision.id, { attributes: ['file_name'] });
        await this._tryWriteBackTitle(ctx.document, sourceAttachment?.file_name || null);
      } catch (titleErr) {
  ...
  async cleanWithLlm(stageConfig, options) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const result = await this.cleanSingleChunk(modelConfig, stageConfig, chunkInput, contextSummary, {
        ...options,
        // 仅第一块（文档开头，含封面/前言）提取发布日期
        isFirstChunk: i === 0,
      });
      ...
      if (i === 0) {
        publishDate = this._normalizePublishDate(result.publish_date);
      }
  ...
  async cleanSingleChunk(modelConfig, stageConfig, chunkInput, contextSummary, options) {
    ...
    return {
      processed_part: this.stripContextNoteFromOutput(parsed.processed_part || ''),
      carried_over: this.stripContextNoteFromOutput(parsed.carried_over || ''),
  ...
  buildPrompt(stageConfig, options = {}) {
    ...
    // 仅第一块（文档开头，含封面/前言）要求提取发布日期，其余块返回 null
    const publishDateRule = options.isFirstChunk
      ? '- publish_date: 从文档开头（封面、前言、版次记录等）提取正式发布日期，格式 YYYY-MM-DD；找不到则返回 null。\n'
      : '- publish_date: 非首块一律返回 null。\n';
    return `${userPrompt}\n\n你必须返回严格 JSON：\n{\n  "processed_part": "string",\n  "carried_over": "string",\n  "publish_date": "string|null",\n  "context_summary": {\n    "key_terms": {},\n    "points": []\n  }\n}\n\n规则：\n- processed_part: 本轮已清洗完成、可直接进入后续处理的正文。\n- carried_over: 若末尾内容疑似残缺、未处理完，放这里，下一轮继续。\n${publishDateRule}- context_summary: 提炼术语与上下文要点，供后续分块保持一致。\n${tableBlockRule}- 任意形如 [[FORMULA_BLOCK_n]] 或 [[FORMULA_INLINE_n]] 的占位符都必须原样保留，不能改字、不能丢失、不能移动顺序。\n- 不要输出任何 JSON 之外的内容。\n- 用户消息中的 [前文上下文摘要] 块只是参考信息，**严禁将其内容复述进 processed_part / carried_over**；输出中若出现该摘要块会被直接剥离。`;
  }
  ...
  /**
   * R16-1: 确定性剥离 LLM 复述的上下文摘要块。
   *
   * 背景：buildContextNote 注入的 "[前文上下文摘要]{JSON}请参考以上摘要…" 是
   * prompt 附件（供保持术语一致），但 LLM 有时会把它**原样复述进输出**，
   * 导致摘要块混入清洗后的正文（污染锚点匹配底本 original_text）。
   *
   * 该剥离是主防线（确定性，不依赖 LLM 自觉），在 processed_part / carried_over
   * 落库前强制执行。结尾锚定 "请参考以上摘要…"（到行尾），避免 JSON 内
   * 嵌套花括号干扰非贪婪匹配。
   *
   * @param {string} text - LLM 输出的清洗文本
   * @returns {string} 剥离后的文本
   */
  stripContextNoteFromOutput(text) {
    if (!text) return text;
    // 兼容两种复述形态：
    //  1) LLM 复述完整块（含 "请参考以上摘要…" 结尾）
    //  2) LLM 只复述 "[前文上下文摘要]" + JSON（实测存量污染全为此形态，
    //     复述时丢掉了结尾说明句）。JSON 为缩进格式，外层对象结束的 "}" 顶格，
    //     故以 "\n}" 后跟空行/结尾作为块边界。
    const pattern = /\[前文上下文摘要\][\s\S]*?(?:\n\}(?:\s*\n|$)|请参考以上摘要[^\n]*)/g;
  ...
  /**
   * 将 LLM 提取的发布日期规范化为 YYYY-MM-DD（DATEONLY 可直接入库）。
   * 支持：2021年11月20日 / 2021-11-20 / 2021/11/20 / 2021.11.20 / 2021年 / 2021
   * 无法解析时返回 null。
   * @param {*} value
   * @returns {string|null}
   */
  _normalizePublishDate(value) {
    if (value == null) return null;
    const s = String(value).trim();
    if (!s || /^(null|n\/a|无|unknown|none|-)$/i.test(s)) return null;

    // 完整日期格式（含年月日）：匹配到即强制走完整分支，非法日期直接返回 null，不回退年份
  ...
  /**
   * 将提取到的发布日期回写到 document_revisions.publish_date（仅当为空时）。
   * @param {object} revision - Sequelize document_revision 实例
   * @param {string} publishDate - YYYY-MM-DD
   */
  async _tryWriteBackPublishDate(revision, publishDate) {
    if (!publishDate) return;
    if (revision.publish_date) {
      logger.info(`[DocumentCleanService] Publish date writeback skipped: revision ${revision.id} already has ${revision.publish_date}`);
      return;
    }
    await revision.update({ publish_date: publishDate });
    logger.info(`[DocumentCleanService] Publish date writeback: revision ${revision.id} -> ${publishDate}`);
  }

  /**
   * 将文档标题从占位值（Intake xxx / Document xxx）回写为原始上传文件名（去扩展名）。
   * @param {object} document - Sequelize document model instance
   * @param {string|null} sourceFileName - 原始上传文件名（如 "GB-T 4208-2017 外壳防护等级(IP代码).pdf"）
   */
  async _tryWriteBackTitle(document, sourceFileName) {
    if (!sourceFileName) {
      logger.info('[DocumentCleanService] Title writeback skipped: no source filename available');
      return;
    }

    // 去掉扩展名作为标题
    const titleFromFileName = sourceFileName.replace(/\.[^.]+$/, '').trim();
    if (!titleFromFileName) {
      logger.info('[DocumentCleanService] Title writeback skipped: empty after stripping extension');
      return;
    }

    const currentTitle = document.title || '';

    // 判断是否为占位值（Intake xxx / Document xxx）
    ...
    // 占位值 → 覆盖为文件名（去扩展名）
    await document.update({ title: titleFromFileName });
    logger.info(`[DocumentCleanService] Title writeback: "${currentTitle}" -> "${titleFromFileName}" for document ${document.id}`);
  }
}
