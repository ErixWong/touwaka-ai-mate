...
class DocumentCleanService {
  ...
  async submit(documentId, options = {}) {
    ...
    if (document.processing_status === 'pending_clean' && runningRun) {
      logger.info(`[DocumentCleanService] Reusing existing clean run ${runningRun.id} for document ${documentId}`);
      this._executeClean(documentId, runningRun.id, options).catch(err => {
        logger.error(`[DocumentCleanService] Background clean failed for document ${documentId}: ${err.message}`);
      });
      return {
        accepted: true,
        status: 'accepted_existing_run',
