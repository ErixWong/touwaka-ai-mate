...
// 全局 NotesStore 单例（P0 修复：跨组件共享同一实例）
let sharedNotesStore = null;
let sharedNotesStoreFactory = null;

/**
 * 获取共享 NotesStore 实例
 * @returns {Promise<INotesStore>}
 */
export async function getSharedNotesStore() {
  if (sharedNotesStore) return sharedNotesStore;

  if (!sharedNotesStoreFactory) {
    sharedNotesStoreFactory = new StoreFactory({
      notes: {
        store: process.env.NOTES_STORE || 'memory',
        ttl: DEFAULT_NOTES_TTL_SECONDS,
        maxCount: 100
      }
    });
  }

  sharedNotesStore = await sharedNotesStoreFactory.getNotesStore();
  logger.info(`[StoreFactory] 共享 NotesStore 单例已创建: ${process.env.NOTES_STORE || 'memory'}`);
  
  return sharedNotesStore;
}
