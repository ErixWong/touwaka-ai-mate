...
`;
   ...
   * 解析工作路径
   * 注意：本轮协议重构后，只接受绝对路径
   * @param {string} cwd - 工作目录（必须是绝对路径）
   * @param {string} userId - 用户ID（当 cwd 为空时用于生成默认路径）
   * @returns {string} 解析后的绝对路径
   * @throws {Error} 如果输入不是绝对路径
   */
  resolveWorkPath(cwd, userId) {
    ...
    if (!path.isAbsolute(cwd)) {
      throw new Error(`工作目录必须是绝对路径，收到: ${cwd}。请使用 absolute_workspace_path 字段传递绝对路径。`);
    }
