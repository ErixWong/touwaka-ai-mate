...
updates.updated_at = new Date();   // ← 强制行变更
await this.Task.update(updates, { ... }); * @param {number} options.contextCharLimit PM上下文总字符预算（默认 12000）
...
export function createAutonomousTaskExecutor(options = {}) {
  ...
  async function executeTask(task, db) {
    try {
      ...
      logger.info(`[AutonomousExecutor] 任务 ${task.id} 开始执行，状态设为 autonomous_working`);
