...
import { invoke as baseInvoke, invokeWithRetry as baseInvokeWithRetry, invokeStream as baseInvokeStream } from './message-llm-client.js';
...
class LLMClient {
  ...
  async call(model, messages, options = {}) {
    ...
    return baseInvoke(model, processedMessages, {
      ...options,
      onRequest: (req) => {
  ...
  async callWithRetry(model, messages, options = {}) {
    ...
    const requestId = options.request_id || this._generateRequestId(userId);
    const processedMessages = this.processMultimodalMessages(messages, model);
    return baseInvokeWithRetry(model, processedMessages, {
      ...options,
      onRequest: (req) => {
        this._registerRequest(requestId, req);
        req.on('close', () => this._unregisterRequest(requestId));
  ...
  async callStream(model, messages, options = {}) {
    ...
    return baseInvokeStream(model, processedMessages, {
      ...options,
      onRequest: (req) => {
        this._registerRequest(requestId, req);
        req.on('close', () => this._unregisterRequest(requestId));
