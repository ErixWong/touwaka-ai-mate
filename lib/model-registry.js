...
class ModelRegistry {
   ...
   * 获取默认 embedding 模型
   */
  async getDefaultEmbeddingModelConfig() {
    if (!this.db) {
      throw new Error('[ModelRegistry] Not initialized');
    }

    const cacheKey = 'defaultEmbedding';
    const cached = this._getCache(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    if (!model) {
      throw new Error('[ModelRegistry] No active embedding model found');
    }

    const modelConfig = await this.getModelConfig(model.id);
    this._setCache(cacheKey, modelConfig);
    logger.info(`[ModelRegistry] Default embedding model: ${modelConfig.model_name}`);
    return modelConfig;
  }
   ...
   * 获取默认文本模型
   */
  async getDefaultTextModelConfig() {
    if (!this.db) {
      throw new Error('[ModelRegistry] Not initialized');
    }

    const cacheKey = 'defaultText';
    const cached = this._getCache(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    if (!model) {
      throw new Error('[ModelRegistry] No active text model found');
    }

    const modelConfig = await this.getModelConfig(model.id);
    this._setCache(cacheKey, modelConfig);
    logger.info(`[ModelRegistry] Default text model: ${modelConfig.model_name}`);
    return modelConfig;
  }
