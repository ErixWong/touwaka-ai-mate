...
class Database {
  ...
  async getExpertFullConfig(expertId) {
    ...
    if (typeof expert.knowledge_config === 'string') {
      try {
        expert.knowledge_config = JSON.parse(expert.knowledge_config);
      } catch {
        expert.knowledge_config = null;
      }
    }

    if (typeof expert.psyche_config === 'string') {
      try {
        expert.psyche_config = JSON.parse(expert.psyche_config);
      } catch {
        expert.psyche_config = null;
      }
    }
  ...
  async getModelConfig(modelId) {
    ...
    const model = await this.models.ai_model.findOne({
      ...
      include: [{
        ...
        required: true,
        attributes: ['id', 'base_url', 'api_key', 'timeout', 'name', 'user_agent'],
      }],
      raw: true,
      nest: true,
    });
    ...
    return {
      ...
      user_agent: model.provider?.user_agent,
    };
  }
   ...
   * 获取时间范围内的消息
   * 注意：时区处理
   * - 数据库中的 created_at 是本地时间（UTC+8）
   * - 传入的 startTime/endTime 是 Date 对象，Sequelize 会自动处理时区转换
   * - 使用 Date 对象而非 ISO 字符串，避免时区歧义
   */
  async getMessagesByTimeRange(expertId, userId, startTime, endTime) {
    ...
    logger.debug(`[DB] getMessagesByTimeRange: 本地时间=${start.toLocaleString('zh-CN')} ~ ${end.toLocaleString('zh-CN')}, UTC=${start.toISOString()} ~ ${end.toISOString()}`);
   ...
   * 更新消息的 topic_id
   */
  async updateMessageTopicId(messageIds, topicId, options = {}) {
   ...
   */
  async createTopic(topicData, options = {}) {
    const {
      id, expertId, userId, name, description,
      category, startTime, endTime, status, keywords, taskId
    } = topicData;
    ...
    const normalizedEndTime = endTime !== undefined
      ? endTime
      : (normalizedStatus === 'archived' ? new Date() : null);
    ...
    return await this.models.topic.create({
      ...
      status: normalizedStatus,  // 默认 active
   ...
   */
  async getTopicsByExpertAndUser(expertId, userId, limit = 10, status = 'active', offset = 0) {
    const where = {
      expert_id: expertId,
      user_id: userId,
    };
  ...
  async countTopicsByExpertAndUser(expertId, userId, status = 'active') {
    const where = {
      expert_id: expertId,
      user_id: userId,
    };
    ...
    return await this.models.topic.count({ where });
  }
   ...
   * 更新 Topic 的消息数量
   */
  async updateTopicMessageCount(topicId, options = {}) {
    const count = await this.models.message.count({
    ...
    return await this.models.topic.update(
      { message_count: count },
      { where: { id: topicId }, transaction: options.transaction }
    );
  }
   ...
   */
  async updateTopic(topicId, data, options = {}) {
    return await this.models.topic.update(
  ...
  async getTopicById(topicId, expertId = null, userId = null, options = {}) {
    const where = { id: topicId };
    if (expertId) where.expert_id = expertId;
    if (userId) where.user_id = userId;

    return await this.models.topic.findOne({
      where,
      raw: true,
      transaction: options.transaction,
    });
  ...
  async getMessagesByTopicId(expertId, userId, topicId, limit = 1000, options = {}) {
    return await this.models.message.findAll({
      where: {
        expert_id: expertId,
        user_id: userId,
      ...
      attributes: ['id', 'role', 'content', 'inner_voice', 'tool_calls', 'created_at', 'topic_id'],
      order: [['created_at', 'ASC']],
      limit,
      raw: true,
      transaction: options.transaction,
   ...
   */
  async searchTopicsByKeyword(expertId, userId, keyword, limit = 10, offset = 0) {
  ...
  async countSearchTopicsByKeyword(expertId, userId, keyword) {
    const searchPattern = `%${keyword.trim()}%`;
    return await this.models.topic.count({
      where: {
        expert_id: expertId,
        user_id: userId,
        [Op.or]: [
  ...
  async getExpertSkills(expertId) {
    ...
    return skills.map(skill => ({
      ...
      mark: skill.skill?.mark,  // Issue #417: 技能标识，用于生成 tool_name
      ...
      argument_hint: skill.skill?.argument_hint,
      disable_model_invocation: skill.skill?.disable_model_invocation,
      user_invocable: skill.skill?.user_invocable,
      allowed_tools: skill.skill?.allowed_tools,
    }));
  }
