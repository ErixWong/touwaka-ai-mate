...
class Database {
  ...
  async execute(sql, params = [], options = {}) {
    try {
      ...
      // 如果启用 raw 模式，不设置任何参数选项，直接发送原始 SQL
      // 这对于 MariaDB VECTOR 类型很重要，因为 VEC_FromText 函数需要原始字符串
      if (!options.raw) {
        queryOptions.replacements = params;
      }
      // 当 raw: true 时，不设置 replacements 或 bind，Sequelize 将直接发送 SQL
      
      const result = await this.sequelize.query(sql, queryOptions);
