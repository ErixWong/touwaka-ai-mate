...
class Database {
   ...
   * @param {boolean} options.raw - 是否使用原始查询（禁用所有参数处理，用于 VECTOR 类型）
   */
  async execute(sql, params = [], options = {}) {
    try {
      // 如果启用 raw 模式，使用 sequelize.query 但不传递任何参数选项
      // 这将使 Sequelize 直接发送 SQL 而不进行任何参数替换
      if (options.raw) {
        const result = await this.sequelize.query(sql, {
          type: Sequelize.QueryTypes.UPDATE,
          // 不设置 replacements 或 bind，Sequelize 将直接发送原始 SQL
        });
        // Sequelize UPDATE 返回 [result, metadata]
        const affectedRows = Array.isArray(result) && result[1] ? result[1] : 0;
        return {
