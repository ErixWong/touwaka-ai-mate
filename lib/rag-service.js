...
class RAGService {
  ...
  async retrieve(query, options = {}) {
    const { topK = 5, threshold = 0.7, userId, collectionId, docTypes } = options;
    ...
    try {
      ...
      if (collectionId) {
        recallOptions.collectionId = collectionId;
      }
      if (docTypes && docTypes.length > 0) {
        recallOptions.doc_types = docTypes;
      }

      const result = await this.docRecallService.recall(query, recallOptions);
      ...
      const formattedResults = result.items.map(item => ({
        ...
        title: item.chunk?.title || item.document?.title,
        content: item.chunk?.content,
        context: '',
        similarity: item.score,
        section_id: item.chunk?.outline_id || null,
        section_title: item.chunk?.title || null,
