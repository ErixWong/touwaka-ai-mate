 ...
 * - buildContextMessage(): 将旧格式 results 转为 LLM 上下文文本（仅旧自动路径使用）
 * - buildEvidenceContextMessage(): ⚠️ 迁移中 — 将从本壳层迁出至 chat-service 或独立证据格式化模块。
 *   当前仍由 chat-service.buildEvidenceInjection() 作为兼容入口调用，新逻辑不应继续往此方法堆叠。
 *
 * === 迁移计划 ===
 * Stage 1 (当前): buildEvidenceContextMessage() 保留在此，但标记为待迁出
 * Stage 2 (下一阶段): 迁至 lib/evidence-formatter.js 或 chat-service 内部
 * Stage 3 (最终): rag-service 退化为纯旧自动路径适配器，不再承担新证据格式化职责
 *
 * 使用方式：
