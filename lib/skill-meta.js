 ...
 * 技能元数据定义
 * 
 * @property {boolean} builtin - 是否为内置技能（代码注入）
...
export const SKILL_META = {
  ...
  execute_javascript: {
    builtin: true,
    risk_level: 'critical',
    required_role: 'creator',  // 只允许 admin 和 creator 角色执行
    description: '在 VM 沙箱中执行 JavaScript 代码',
  },
  recall_topic: {
    builtin: true,
    risk_level: 'low',
    required_role: 'user',
    description: '回忆话题内容：列出最近话题或获取指定话题的消息',
  },
  recall: {
    builtin: true,
    risk_level: 'low',
    required_role: 'user',
    description: '回忆话题或消息内容：列出话题、搜索话题、获取话题消息、获取消息完整内容',
  },
  exec: {
    builtin: true,
    risk_level: 'high',
  ...
  // ==================== 文件操作技能（skills/file-operations）====================
  read_lines: {
    builtin: false,
    risk_level: 'low',
    required_role: 'user',
    description: '读取文件内容（按行）',
  },
  read_bytes: {
    builtin: false,
    risk_level: 'low',
  ...
  search_in_file: {
    ...
    risk_level: 'low',
    required_role: 'user',
    description: '在单个文件中搜索文本',
  },
  grep: {
    builtin: false,
    risk_level: 'low',
    required_role: 'user',
    description: '在多个文件中搜索文本',
  },
  write_file: {
    builtin: false,
    risk_level: 'medium',
 ...
 * 角色等级定义
 * 数值越大权限越高
 * 注：creator 角色对应数据库中的 power_user 等级
 */
export const ROLE_LEVEL = {
  user: 1,
  creator: 2,  // 专家创作者（对应数据库中的 power_user）
  admin: 3,
};
