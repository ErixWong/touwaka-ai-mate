/**
 * Config Loader
 * 从数据库加载专家/模型/技能配置，支持缓存
 */

import logger from './logger.js';
import { normalizeBaseUrl } from './llm-url-utils.js';

class ConfigLoader {
  constructor(db) {
    this.db = db;
    this.cache = new Map();
    this.cacheExpiry = new Map();
    this.defaultCacheTTL = 5 * 60 * 1000; // 默认缓存 5 分钟
  }

  /**
   * 加载专家完整配置（包含模型、技能）
   * @param {string} expertId - 专家ID
   * @param {boolean} useCache - 是否使用缓存
   * @returns {Object} 完整配置
   */
  async loadExpertConfig(expertId, useCache = true) {
    // 检查缓存
    if (useCache && this.cache.has(expertId)) {
      const expiry = this.cacheExpiry.get(expertId);
      if (expiry && Date.now() < expiry) {
        logger.debug(`Config loaded from cache for expert: ${expertId}`);
        return this.cache.get(expertId);
      }
      // 缓存过期，清除
      this.cache.delete(expertId);
      this.cacheExpiry.delete(expertId);
    }

    try {
      // 从数据库加载完整配置
      const config = await this.db.getExpertFullConfig(expertId);

      // 验证配置完整性
      this.validateExpertConfig(config, expertId);

      // 缓存配置
      this.cache.set(expertId, config);
      this.cacheExpiry.set(expertId, Date.now() + this.defaultCacheTTL);

      logger.info(`Config loaded and validated for expert: ${expertId}`);
      return config;
    } catch (error) {
      logger.error(`Failed to load config for expert ${expertId}:`, error.message);
      throw error;
    }
  }

  /**
   * 验证专家配置完整性
   * @param {Object} config - 配置对象
   * @param {string} expertId - 专家ID（用于错误信息）
   * @throws {Error} 配置无效时抛出错误
   */
  validateExpertConfig(config, expertId) {
    if (!config) {
      throw new Error(`Config is empty for expert: ${expertId}`);
    }

    // 验证专家基本信息
    if (!config.expert) {
      throw new Error(`Expert record not found: ${expertId}`);
    }

    const requiredFields = ['id', 'name', 'expressive_model_id'];
    for (const field of requiredFields) {
      if (!config.expert[field]) {
        throw new Error(`Expert config missing required field: ${field}`);
      }
    }

    // 验证表达模型配置
    if (!config.expressiveModel) {
      throw new Error(`Expressive model not found or inactive: ${config.expert.expressive_model_id}`);
    }
    this.validateModelConfig(config.expressiveModel, 'expressive');

    // 验证反思模型配置（如果配置了）
    if (config.expert.reflective_model_id && config.reflectiveModel) {
      this.validateModelConfig(config.reflectiveModel, 'reflective');
    }

    // 验证 Psyche 配置（如果启用）
    if (config.expert.context_strategy === 'minimal') {
      this.validatePsycheConfig(config.expert);
    }

    logger.debug(`Config validation passed for expert: ${expertId}`);
  }

  /**
   * 验证 Psyche 配置
   * @param {Object} expert - 专家配置
   * @throws {Error} 配置无效时抛出错误
   */
  validatePsycheConfig(expert) {
    // P0 修复：psyche_config 可能为 JSON 字符串，统一解析为对象并回写
    let psycheConfig = expert.psyche_config || {};
    if (typeof psycheConfig === 'string') {
      try {
        psycheConfig = JSON.parse(psycheConfig);
        logger.debug(`[ConfigLoader] psyche_config JSON 解析成功`);
      } catch (e) {
        logger.warn(`[ConfigLoader] psyche_config JSON 解析失败，已降级为空对象: ${e.message}`);
        psycheConfig = {};
      }
    } else if (typeof psycheConfig !== 'object') {
      logger.warn(`[ConfigLoader] psyche_config 类型异常 (${typeof psycheConfig})，已降级为空对象`);
      psycheConfig = {};
    }

    // 回写到 expert.psyche_config，确保下游获取的是对象
    expert.psyche_config = psycheConfig;

    // 验证 max_tokens_ratio
    if (psycheConfig.max_tokens_ratio !== undefined) {
      const ratio = psycheConfig.max_tokens_ratio;
      if (ratio < 0.1 || ratio > 0.5) {
        throw new Error(`Psyche max_tokens_ratio must be between 0.1 and 0.5, got: ${ratio}`);
      }
    }

    // 验证 reflection_lookback
    if (psycheConfig.reflection_lookback !== undefined) {
      const lookback = psycheConfig.reflection_lookback;
      if (lookback < 1 || lookback > 10) {
        throw new Error(`Psyche reflection_lookback must be between 1 and 10, got: ${lookback}`);
      }
    }

    // 验证 enable_notes（必须是 boolean）
    if (psycheConfig.enable_notes !== undefined) {
      if (typeof psycheConfig.enable_notes !== 'boolean') {
        throw new Error(`Psyche enable_notes must be boolean, got: ${typeof psycheConfig.enable_notes}`);
      }
    }

    logger.debug(`Psyche config validation passed: max_tokens_ratio=${psycheConfig.max_tokens_ratio}, reflection_lookback=${psycheConfig.reflection_lookback}, enable_notes=${psycheConfig.enable_notes}`);
  }

  /**
   * 验证模型配置
   * @param {Object} model - 模型配置
   * @param {string} type - 模型类型（用于错误信息）
   * @throws {Error} 配置无效时抛出错误
   */
  validateModelConfig(model, type) {
    if (!model.base_url) {
      throw new Error(`${type} model missing base_url`);
    }

    if (!model.api_key) {
      throw new Error(`${type} model missing api_key`);
    }

    if (!model.model_name) {
      throw new Error(`${type} model missing model_name`);
    }

    // 验证 URL 格式
    try {
      const baseUrl = String(model.base_url).trim();
      const normalizedBaseUrl = normalizeBaseUrl(baseUrl);
      new URL(normalizedBaseUrl);
    } catch {
      throw new Error(`${type} model has invalid base_url: ${model.base_url}`);
    }
  }

  /**
   * 获取模型配置（包含 Provider 信息）
   * @param {string} modelId - 模型ID
   * @returns {Object} 模型配置
   */
  async getModelConfig(modelId) {
    return await this.db.getModelConfig(modelId);
  }

  /**
   * 获取专家技能列表
   * @param {string} expertId - 专家ID
   * @returns {Array} 技能列表
   */
  async getExpertSkills(expertId) {
    return await this.db.getExpertSkills(expertId);
  }

  /**
   * 获取单个技能详情
   * @param {string} skillId - 技能ID
   * @returns {Object} 技能详情
   */
  async getSkill(skillId) {
    return await this.db.getSkill(skillId);
  }

  /**
   * 清除指定专家的缓存
   * @param {string} expertId - 专家ID
   */
  clearCache(expertId = null) {
    if (expertId) {
      this.cache.delete(expertId);
      this.cacheExpiry.delete(expertId);
      logger.info(`Cache cleared for expert: ${expertId}`);
    } else {
      this.cache.clear();
      this.cacheExpiry.clear();
      logger.info('All config cache cleared');
    }
  }

  /**
   * 设置缓存 TTL
   * @param {number} ttlMs - 缓存时间（毫秒）
   */
  setCacheTTL(ttlMs) {
    this.defaultCacheTTL = ttlMs;
  }

  /**
   * 获取缓存统计
   */
  getCacheStats() {
    return {
      size: this.cache.size,
      keys: Array.from(this.cache.keys()),
    };
  }
}

export default ConfigLoader;
