...
- {mode: 'messages', action: 'detail', message_id: 'xxx'} 获取单条消息完整内容`,
  ...
  },
  {
    ...
    function: {
      ...
      description: `从文档平台检索与用户问题相关的内容。用于回答需要依据文档证据的问题。
      ...
      parameters: {
        ...
        properties: {
          ...
          goal: {
            ...
            description: '（可选）检索目标：answer_question（回答问题）、find_document（查找文档）、verify_fact（核实事实）。默认 answer_question。',
          },
        },
        required: ['query'],
      },
...
class ToolManager {
  ...
  async executeBuiltinTool(toolId, params, context, display) {
    ...
    // 执行文档检索工具
    if (toolId === 'document_retrieval') {
      return await this.executeDocumentRetrieval(params, context, display);
    }
  ...
  async executeDocumentRetrieval(params, context, display) {
    ...
    logger.info('[ToolManager] 执行文档检索:', {
    ...
    try {
      ...
      logger.info('[ToolManager] 文档检索完成:', {
        strategy: result.strategy,
        sufficiency: response.evidence_sufficiency,
      ...
      logger.error('[ToolManager] 文档检索失败:', error);
      return {
        success: false,
        error: error.message,
        toolId: 'document_retrieval',
