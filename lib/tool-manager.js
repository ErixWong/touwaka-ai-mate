...
class ToolManager {
  ...
  async executeTool(toolId, params, context = {}) {
    // 检查是否是内置工具
    const builtinCapability = this.capabilityRegistry.get(toolId);
    const builtinTool = builtinCapability?.kind === CAPABILITY_KINDS.BUILTIN
      ? builtinCapability.definition
      : null;
    if (builtinTool) {
  ...
  async executeBuiltinTool(toolId, params, context, display) {
    ...
    // Definition and execution share the same capability policy.
    const capability = this.capabilityRegistry.get(toolId);
    const allowedRoles = capability?.metadata?.allowedRoles;
    if (allowedRoles?.length && !this.capabilityRegistry.isAllowed(toolId, context)) {
