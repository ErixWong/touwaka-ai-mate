    ...
    try {
      ...
      logger.info('[ToolManager] answer_from_documents 完成:', {
        ...
        identity_distribution: response.documents?.map(d => ({
          id: d.document_id, confidence: d.identity_confidence, source: d.identity_source,
        })) || [],
      });
  ...
  async _handleFindDocument(params, context, display) {
    ...
    try {
      ...
      const candidates = (packet?.documents || []).map(doc => ({
        ...
        supporting_evidence: (candidates.length <= 1 && doc.candidate_confidence === 'high'
      ...
      logger.info('[ToolManager] find_document 完成:', {
        ...
        identity_distribution: candidates.map(c => ({
          id: c.document_id, confidence: c.identity_confidence, source: 'search_match',
        })),
      });
