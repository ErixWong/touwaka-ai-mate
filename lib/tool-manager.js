...
- 30秒超时、1MB输出限制`,
      parameters: {
        ...
        properties: {
          ...
          code: {
            type: 'string',
            description: `要执行的代码。当 type=javascript 时为 JS 代码片段；当 type=shell 时为系统命令。当 type=nodejs 或 python 时此参数可选，脚本内容从 script_path 文件加载。`,
          },
          script_path: {
            type: 'string',
...
class ToolManager {
  ...
  async executeNodeScript(scriptPath, args, workingDirectory, context) {
    ...
    if (ext !== '.js' && ext !== '.mjs') {
      throw new Error(`Script must be a .js or .mjs file: ${scriptPath}`);
    }
    ...
    return new Promise((resolve) => {
      ...
      const proc = spawn('node', [fullScriptPath, ...args], {
        ...
        env: {
          ...
          IS_ADMIN: context?.session?.isAdmin ? 'true' : 'false',
          IS_SKILL_CREATOR: context?.session?.roles?.includes('creator') ? 'true' : 'false',
  ...
  async executePythonScript(scriptPath, args, workingDirectory, context) {
    ...
    return new Promise((resolve) => {
      ...
      const proc = spawn(pythonCmd, [fullScriptPath, ...args], {
        cwd: absoluteWorkingDir,
        env: {
          ...process.env,
