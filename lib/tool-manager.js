...
const BUILTIN_TOOLS = [
  {
    ...
    function: {
      name: 'execute',
      description: '执行 JavaScript 代码或安全的系统命令。支持两种模式：1) javascript - 在 VM 沙箱中执行 JS 代码，可访问 console、Buffer、URL 等基础 API；2) shell - 执行白名单内的安全命令（如 ls、grep、cat 等），用于文件查看和文本处理。注意：shell 模式仅支持只读操作，禁止修改系统或删除文件。',
      parameters: {
        type: 'object',
        properties: {
          type: {
          ...
          code: {
            type: 'string',
            description: '要执行的代码。当 type=javascript 时为 JS 代码；当 type=shell 时为系统命令（如 "ls -la"、"grep -n \"keyword\" file.txt"）',
          },
          script_path: {
            type: 'string',
...
const SHELL_COMMAND_WHITELIST = [
  // 文本处理类
  'cat', 'head', 'tail', 'grep', 'wc', 'sort', 'uniq', 'cut', 'awk', 'sed', 'tr', 'diff',
  // 信息查看类
...
function validateShellCommand(command) {
  ...
  for (const pattern of DANGEROUS_ARG_PATTERNS) {
    if (pattern.test(trimmedCommand)) {
      return { 
        safe: false, 
        error: `Command contains dangerous pattern: ${pattern}. Only read-only operations are allowed.` 
      };
    }
  }

  // 特殊检查：find 命令禁止 -exec 和 -delete
  if (cmdName === 'find') {
    if (/\s+-exec\s+/.test(trimmedCommand) || /\s+-delete\s*/.test(trimmedCommand)) {
      return { 
        safe: false, 
        error: 'find command with -exec or -delete is not allowed for security reasons.' 
      };
    }
  }
