...
- {mode: 'messages', action: 'detail', message_id: 'xxx'} 获取单条消息完整内容`,
  ...
  },
  {
    ...
    function: {
      ...
      description: `系统级文档检索能力（document_retrieval skill 主入口）。
从文档平台检索与用户问题相关的内容，用于回答需要依据文档证据的问题。

=== 任务类型（通过 goal 参数选择）===
- answer_question（默认）：基于文档证据回答用户问题，返回结构化证据包
- find_document：定位可能相关的文档，返回文档候选列表（不含 chunk 级证据）
- verify_fact：校验某个命题是否得到文档支持，返回 verdict + 支持/反驳证据

=== 使用场景 ===
- 用户询问文档中的具体信息（合同条款、发票内容、报告数据等）
- 需要核实某个事实是否有文档支撑
- 需要查找特定文档
- 用户问题涉及专业知识，需要从文档中检索证据
...
=== 返回说明 ===
返回结构化证据，包含：
- evidence_sufficiency：证据充分性（strong/medium/weak/none）
- suggested_response_mode：建议回答模式
- strategy：内部检索策略（document_first/chunk_first_fallback/degrade）
- documents：候选文档及其 top evidence`,
      parameters: {
        type: 'object',
        properties: {
          query: {
          ...
          goal: {
            type: 'string',
            enum: ['answer_question', 'find_document', 'verify_fact'],
            description: '（可选）检索目标：answer_question（回答问题，默认）、find_document（查找文档）、verify_fact（核实事实）。兼容期内通过此参数做内部 dispatch，后续演进为独立 tool。',
          },
        },
        required: ['query'],
      },
    ...
    _meta: {
      ...
      /** 系统级 skill 标识：此 tool 是 document_retrieval skill 的 LLM 可见入口 */
...
class ToolManager {
  ...
  async executeBuiltinTool(toolId, params, context, display) {
    ...
    // 执行文档检索 skill 工具族（document_retrieval skill namespace）
    if (toolId === 'document_retrieval' || this._isDocRetrievalTool(toolId)) {
  ...
  _isDocRetrievalTool(toolId) {
    // 从 BUILTIN_TOOLS 中查找该 toolId 的 _meta.skillNamespace
    // 兼容直接匹配 'document_retrieval'（过渡态单 tool 入口）
    if (toolId === 'document_retrieval') return true;
    const builtinTool = BUILTIN_TOOLS.find(t => t.function.name === toolId);
    return builtinTool?._meta?.skillNamespace === 'document_retrieval';
  }

  /**
   * document_retrieval skill 统一 dispatch 入口
   *
   * 兼容期内，所有请求通过 document_retrieval 单 tool 进入，
   * 由 goal 参数决定内部 dispatch 到哪个 handler。
   * 未来拆分为独立 tool 后，此方法将按 toolId 直接路由。
   *
   * @param {string} toolId - LLM 调用的 tool 名称
   * @param {object} params - tool 参数
   * @param {string} params.goal - 任务类型（兼容期内做 dispatch key）
   * @param {object} context - 执行上下文
   * @param {string} display - 工具显示名称
  ...
  async _dispatchDocRetrievalTool(toolId, params, context, display) {
    ...
    // 兼容期：按 goal 参数 dispatch
    // 未来：按 toolId 直接路由到对应 handler
    switch (goal) {
      case 'find_document':
   ...
   * @param {string} [params.goal] - 检索目标（兼容期 dispatch key）
   * @param {object} context - 执行上下文
   * @param {string} display - 工具显示名称
  ...
  async _handleAnswerFromDocuments(params, context, display) {
    const startTime = Date.now();
    const { query, collection_id, doc_types, goal } = params;
    const userId = context.userId || context.user_id;
    const effectiveToolName = 'answer_from_documents';

    logger.info('[ToolManager] document_retrieval → answer_from_documents:', {
      tool_name: effectiveToolName,
      goal: goal || 'answer_question',
  ...
  /**
   * find_document handler（兼容期内由 goal='find_document' dispatch 进入）
   *
  ...
  async _handleFindDocument(params, context, display) {
    ...
    logger.info('[ToolManager] document_retrieval → find_document:', {
      tool_name: effectiveToolName,
      query_length: query?.length || 0,
      collection_id,
      doc_types,
    ...
    try {
      ...
      // find_document：走 document-first 检索但不召回 chunk evidence
      const result = await retrievalService.retrieve(query.trim(), {
        userId,
        doc_types: doc_types?.length > 0 ? doc_types : undefined,
        collection_id: effectiveCollectionId || undefined,
        top_k_candidates: 10,
        top_k_evidence: 0,  // 不召回 chunk 证据
        evidence_threshold: 0.1,
        allow_fallback: false,  // find_document 不回退
      });

      const packet = result.packet;
      const duration = Date.now() - startTime;

      const candidates = (packet?.documents || []).map(doc => ({
        document_id: doc.document_id,
        document_title: doc.document_title,
        doc_type: doc.doc_type,
        collection_name: doc.collection_name,
  ...
  /**
   * verify_fact handler（兼容期内由 goal='verify_fact' dispatch 进入）
   *
   * 与 answer_from_documents 的区别：
   * - 返回 verdict 枚举（supported / contradicted / insufficient_evidence）
   * - 区分支持证据和反驳证据
   */
  async _handleVerifyFact(params, context, display) {
    ...
    logger.info('[ToolManager] document_retrieval → verify_fact:', {
      tool_name: effectiveToolName,
      query_length: query?.length || 0,
      collection_id,
      doc_types,
  ...
  /**
   * [DEPRECATED] 执行文档检索工具（document_retrieval）
   *
   * @deprecated 请使用 _dispatchDocRetrievalTool() → _handleAnswerFromDocuments()
   * 保留此方法作为向后兼容壳层，内部委托给新 dispatch 入口。
   */
  async executeDocumentRetrieval(params, context, display) {
    return await this._handleAnswerFromDocuments(params, context, display);
  }
