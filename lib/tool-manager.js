...
class ToolManager {
  ...
  async executeRecallTopic(params, context, display) {
    ...
    try {
      ...
      if (topic_id) {
        const messages = await this.getTopicMessages(topic_id, userId, count);
      ...
      const topics = await memorySystem.getTopics(userId, start + count, 'active');
   ...
   */
  async getTopicMessages(topicId, userId, limit = 20) {
