...
const NOTES_TOOL_NAMES = new Set([
  ...
  'notes_list',
  'notes_forget',
  ...Object.keys(NOTES_TOOL_ALIASES),
]);

const NOTES_TOOL_DEFINITION_CONFIGS = Object.freeze({
  notes_take: {
    description: '将材料存入临时 Notes 工作记忆。首选名称是 note_take（兼容别名：notes_take）。Notes 可能过期，也可能不跨服务重启保留；重要结论应写入回答正文或依靠 Topic 归档长期保存。内容上限 4000 字符。',
    parameters: {
      type: 'object',
      properties: {
        key: {
        ...
        content: {
          type: 'string',
          description: '笔记内容，可以是关键信息摘要、配置参数、搜索结果等；上限 4000 字符。',
        },
        type: {
          type: 'string',
  ...
  notes_read: {
    description: '从临时 Notes 工作记忆加载笔记内容。首选名称是 note_read（兼容别名：notes_read）。读取命中会滑动续期，但 Notes 仍可能过期或因重启丢失。',
    parameters: {
      type: 'object',
      properties: {
        key: {
          type: 'string',
          description: '笔记标识，即之前使用 note_take 或 notes_take 存储时使用的 key。',
        },
      },
      required: ['key'],
    },
  ...
  notes_list: {
    description: '列出当前临时 Notes 工作记忆中的笔记清单，首选名称是 note_list（兼容别名：notes_list）。',
    parameters: {
      type: 'object',
      properties: {},
    },
  ...
  notes_forget: {
    description: '从宿主临时 Notes 工作记忆中删除指定笔记。兼容别名：note_forget。',
    parameters: {
      type: 'object',
      properties: {
        key: {
          type: 'string',
          description: '笔记标识，即之前使用 notes_take 或 note_take 存储时使用的 key。',
        },
      },
      required: ['key'],
    },
...
function createNotesToolDefinition(toolName, description = null) {
  const canonicalToolName = NOTES_TOOL_ALIASES[toolName] || toolName;
  const config = NOTES_TOOL_DEFINITION_CONFIGS[canonicalToolName];
  if (!config) {
    throw new Error(`Unknown notes tool definition: ${toolName}`);
  }
...
- {mode: 'messages', action: 'detail', message_id: 'xxx', max_chars: 4000} 获取单条消息明细（内容会按上限截断）`,
  ...
  createNotesToolDefinition('notes_take'),
  createNotesToolDefinition('notes_read'),
  createNotesToolDefinition('notes_list'),
  createNotesToolDefinition('notes_forget'),
  createNotesToolDefinition(
    'note_take',
    '将材料存入宿主临时 Notes 工作记忆。兼容别名：notes_take；两者共享同一 psyche-store 存储。',
  ),
  createNotesToolDefinition(
    'note_read',
    '从宿主临时 Notes 工作记忆加载笔记内容。兼容别名：notes_read；读取命中会滑动续期。',
  ),
  createNotesToolDefinition(
    'note_list',
    '列出当前宿主临时 Notes 工作记忆中的笔记清单。兼容别名：notes_list。',
  ),
  createNotesToolDefinition(
    'note_forget',
    '从宿主临时 Notes 工作记忆中删除指定笔记。兼容别名：notes_forget。',
  ),
  // ============================================================
...
class ToolManager {
  ...
  _normalizeNotesToolId(toolId) {
    return NOTES_TOOL_ALIASES[toolId] || toolId;
  }
   ...
   * Psyche 上下文管理的手抄功能
   *
   * @param {string} toolId - 工具 ID（notes_* 或 note_*）
   * @param {object} params - 工具参数
   * @param {object} context - 执行上下文
   * @param {string} display - 工具显示名称
   * @returns {Promise<object>} 执行结果
  ...
  async executeNotesTool(toolId, params, context, display) {
    ...
    try {
      ...
      const notesManager = new NotesManager(notesStore, {
        ttl: context.notes_ttl_seconds || context.notesTtlSeconds || NOTES_DEFAULT_TTL_SECONDS,
      });
      ...
      switch (normalizedToolId) {
        case 'notes_take': {
          ...
          const saveResult = await notesManager.take(userId, expertId, key, {
            content,
            type,
          ...
          result = {
            success: true,
            message: saveResult.overwritten ? `笔记 "${key}" 已更新` : `笔记 "${key}" 已保存`,
            key,
            type,
            relevance: normalizedRelevance,
            ttl_seconds: notesManager.config.ttl,
        ...
        case 'notes_forget': {
          ...
          await notesManager.delete(userId, expertId, key);
          result = {
            success: true,
            message: `笔记 "${key}" 已删除`,
            key,
