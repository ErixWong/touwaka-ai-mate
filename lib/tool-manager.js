...
class ToolManager {
  ...
  async getTopicMessages(topicId, userId, start = 0, count = 20) {
    ...
    // 确保 start 和 count 是有效的非负整数
    const safeStart = Math.max(0, parseInt(start) || 0);
    const safeCount = Math.max(1, parseInt(count) || 20);

    logger.info(`[ToolManager] getTopicMessages: topic=${topicId}, user=${userId}, start=${safeStart}, count=${safeCount}, topic.message_count=${topic.message_count}`);
    ...
    logger.info(`[ToolManager] getTopicMessages: 实际消息总数=${totalCount}`);
    ...
    logger.info(`[ToolManager] getTopicMessages: 返回 ${messages.length} 条消息`);
