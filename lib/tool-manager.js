...
import { createBuiltinNotesTools } from 'erix-agent';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const RECALL_DEFAULT_COUNT = 10;
const RECALL_MAX_COUNT = 50;
const RECALL_SNIPPET_MAX_CHARS = 200;
const RECALL_DETAIL_MAX_CHARS = 4000;

/**
 * erix 0.10.0 原生 notes 工具（Phase 3 阶段二：唯一暴露的 notes 工具名）。
 * 旧宿主 notes_* 与兼容别名（notes.take 等）全部退役。
 */
const NOTE_TOOL_NAMES = new Set(['note_take', 'note_read', 'note_list', 'note_forget']);

/**
 * note_forget 原生 description 声称“保留墓碑”，与 touwaka 物理删除语义冲突
 * （DESIGN-REVIEW 决策 2）：注册时覆盖 description。
 */
const NOTE_FORGET_DESCRIPTION_OVERRIDE = '撤销并永久删除一个 run 作用域笔记（物理删除，删除后不可恢复）。when-to-use：值已失效或必须明确撤销时调用；上下文被折叠时先 note_list，再 note_read key=...；不要遍历归档目录凭记忆补值';

/**
 * erix definitions（name/description/inputSchema）→ touwaka OpenAI function 格式。
 * 只取静态定义，不触发任何存储 IO（toolSet 实例在执行期按 scopeRef 另行创建）。
 */
function convertErixNotesDefinition(definition) {
  const tool = {
    type: 'function',
    function: {
  ...
  if (definition.name === 'note_forget') {
    tool.function.description = NOTE_FORGET_DESCRIPTION_OVERRIDE;
  }
  return tool;
}

const ERIX_NOTES_TOOL_DEFINITIONS = (() => {
  // 模块加载期仅解析定义；显式注入 no-op store（五方法全部 reject），
  // 避免 createBuiltinNotesTools 默认实例化 FileNotesStore（评审修正项）。
  const rejectingNotesStore = Object.fromEntries(
    ['write', 'read', 'list', 'complete', 'janitor'].map((method) => [
      method,
      async () => {
        throw new Error(`notes definitions store is not executable: ${method}`);
      },
    ]),
  );
  const definitions = createBuiltinNotesTools({
    scopeRef: 'touwaka-notes-definitions',
    notesStore: rejectingNotesStore,
  }).definitions;
  return definitions
    .filter((definition) => NOTE_TOOL_NAMES.has(definition.name))
    .map(convertErixNotesDefinition);
})();
...
- {mode: 'messages', action: 'detail', message_id: 'xxx', max_chars: 4000} 获取单条消息明细（内容会按上限截断）`,
      parameters: {
        type: 'object',
        properties: {
          mode: {
          ...
          count: {
            type: 'integer',
            description: '查询数量。默认 10，最大 50',
          },
          max_chars: {
            type: 'integer',
            description: 'mode=messages/action=detail 时返回内容的最大字符数。默认 4000，最大 4000',
          },
        },
  ...
  // Notes 工具 - erix 0.10.0 原生 note_*（Phase 3 阶段二切换，唯一权威实现）
  ...ERIX_NOTES_TOOL_DEFINITIONS,
  // ============================================================
  ...
  // ============================================================
  {
    ...
    function: {
      ...
      description: `按标题/元数据/附件文件名在文档平台定位候选文档。只返回文档元信息（标题、类型、相关度、document_id、版本列表），不返回正文内容。

使用场景：
- "帮我找某份制度/合同/标准"
- "有没有一份关于[某主题]的文档"
- "某份文档有几个版本 / 有哪些版本号"（直接根据返回的 version_count 与 versions 回答）
- 需要 document_id 以供后续 read_document_content 使用

返回：候选文档摘要列表 + doc_ref handle（可用于 search_chunks_in_document 锁定文档范围）。
每个候选包含 version_count 与 versions（该文档的全部版本：revision_label、revision_status、is_current），回答版本数量/版本号问题时必须以此为准，不要把"文档条数"当作"版本数"。
...
class ToolManager {
  ...
  constructor(db, expertId, options = {}) {
    ...
    // Unified in-process capability index. Source loading remains delegated to
    // SkillLoader/ResidentSkillManager; this registry owns exposure metadata.
    this.capabilityRegistry = new CapabilityRegistry();
    this.registerBuiltinCapabilities();

    // Explicit resident runtime dependency. Never resolve this from global.
    this.residentSkillManager = options.residentSkillManager || null;

    // 是否已初始化
    this.initialized = false;

    // erix notes toolSet 缓存（scopeRef -> toolSet）；测试注入点：_noteRecordStore
    this._notesToolSets = new Map();
    this._noteRecordStore = options.noteRecordStore || null;
  ...
  /**
   * 获取（或创建）某 (userId, expertId) 作用域的 erix notes toolSet。
   * scopeRef = buildNotesScopeRef(userId, expertId) 稳定派生（跨请求工作记忆），
   * 存储走 ErixNotesStoreAdapter（MariaDB CAS，revoked→物理删除；issue #1132 决策 3 修订）。
   * @returns {Promise<object>} erix createBuiltinNotesTools 返回的 toolSet
   */
  async _getNotesToolSet(userId, expertId) {
    const {
      buildNotesScopeRef,
      getSharedNoteRecordStore,
      ErixNotesStoreAdapter,
      NOTES_TTL_SECONDS,
    } = await import('./notes/index.js');
    const scopeRef = buildNotesScopeRef(userId, expertId);
    if (this._notesToolSets.has(scopeRef)) return this._notesToolSets.get(scopeRef);

    // 测试可注入 duck-type store（manager._noteRecordStore = fakeStore）
    const store = this._noteRecordStore || await getSharedNoteRecordStore(this.db);
    const adapter = new ErixNotesStoreAdapter({
      store,
      scopeRef,
      ttlSeconds: NOTES_TTL_SECONDS,
    });
    const toolSet = createBuiltinNotesTools({ scopeRef, notesStore: adapter });
    this._notesToolSets.set(scopeRef, toolSet);
    logger.debug(`[ToolManager] 创建 erix notes toolSet: ${scopeRef}`);
    return toolSet;
  }

  _isNotesTool(toolId) {
    return typeof toolId === 'string' && NOTE_TOOL_NAMES.has(toolId);
  }

  _canUseNotes(context = {}) {
    const strategy = context.context_strategy || context.contextStrategy || context.strategy || 'full';
    const enableNotes = context.enable_notes ?? context.enableNotes ?? context.notes_enabled ?? context.notesEnabled;
    return strategy === 'minimal' && enableNotes !== false;
  }
  ...
  registerBuiltinCapabilities() {
    this.capabilityRegistry.registerMany(BUILTIN_TOOLS.map(tool => ({
    ...
    for (const tool of tools) {
      ...
      if (toolId) {
        ...
        this.capabilityRegistry.register({
          id: toolId,
          kind: scriptPath.startsWith('resident://')
            ? CAPABILITY_KINDS.RESIDENT
            : CAPABILITY_KINDS.SKILL,
          definition: tool,
          metadata: {
            ...(tool._meta || {}),
            skillId,
  ...
  async getToolDefinitions(context = {}) {
    ...
    // 添加经过 capability policy 过滤的内置工具。
    for (const capability of this.capabilityRegistry.list({
      kind: CAPABILITY_KINDS.BUILTIN,
      context,
    })) {
      const tool = capability.definition;
      if (this._isNotesTool(tool.function?.name) && !this._canUseNotes(context)) {
        continue;
      }
    ...
    for (const skill of this.skills.values()) {
      ...
      for (const tool of tools) {
        const toolId = this.extractToolName(tool);
        if (toolId && !this.capabilityRegistry.has(toolId)) {
          this.capabilityRegistry.register({
            id: toolId,
        ...
        if (toolId && !this.capabilityRegistry.isAllowed(toolId, context)) {
          continue;
        }
        const { _meta, ...llmTool } = tool;
        definitions.push(llmTool);
  ...
  async getMcpToolDefinitions(context = {}) {
    this.mcpToolRegistry.clear();
    for (const capability of this.capabilityRegistry.list({ kind: CAPABILITY_KINDS.MCP })) {
  ...
  buildAgentDelegateControlContext(context = {}) {
    ...
    const inheritedWorkspaceScope = parentInvocation?.workspace_scope || {};
    const explicitWorkspaceScope = context.workspace_scope
      || context.workspace_capability_scope
      || context.delegation_workspace_scope
      || {};
  ...
  async executeTool(toolId, params, context = {}) {
    ...
    const builtinTool = capability?.kind === CAPABILITY_KINDS.BUILTIN
      ...
      : null;
    const display = this.isAgentDelegateControlTool(toolId)
      ? this.formatAgentDelegateControlToolDisplay(toolId)
      : builtinTool
        ? this.formatBuiltinToolDisplay(toolId, builtinTool)
        : this.formatToolDisplay(toolId);
    const permissionDenied = this.getCapabilityPermissionDenied(toolId, context, display);
    if (permissionDenied) {
    ...
    if (toolId.startsWith('mcp_')) {
      if (capability?.kind !== CAPABILITY_KINDS.MCP || !this.mcpToolRegistry.has(toolId)) {
        this.logToolExecution(display, toolId, params);
        return {
          success: false,
          error: `MCP tool not registered: ${toolId}`,
        };
      }
      this.logToolExecution(display, toolId, params);
    ...
    if (!capability || ![CAPABILITY_KINDS.SKILL, CAPABILITY_KINDS.RESIDENT].includes(capability.kind)) {
      this.logToolExecution(display, toolId, params);
      return {
        success: false,
        error: `Capability not registered for tool: ${toolId}`,
      };
    }
  ...
  async executeBuiltinTool(toolId, params, context, display) {
    ...
    if (this._isNotesTool(toolId)) {
      return await this.executeErixNotesTool(toolId, params, context, display);
    }
  ...
  _normalizeRecallWindow(start, count) {
    const normalizedStart = Math.max(0, Number.parseInt(start, 10) || 0);
    const parsedCount = Number.parseInt(count, 10);
    const normalizedCount = Math.min(
      RECALL_MAX_COUNT,
      Math.max(1, Number.isFinite(parsedCount) ? parsedCount : RECALL_DEFAULT_COUNT)
    );
    return { start: normalizedStart, count: normalizedCount };
  }

  _buildRecallPagination(start, requestedCount, returnedCount, totalCount) {
    const total = Number.isFinite(totalCount) ? totalCount : start + returnedCount;
    const hasMore = start + returnedCount < total;
    return {
      start,
      ...
      total_count: total,
      has_more: hasMore,
      next_start: hasMore ? start + returnedCount : null,
    };
  }

  _truncateRecallSnippet(content) {
    const text = String(content || '');
    return text.substring(0, RECALL_SNIPPET_MAX_CHARS) + (text.length > RECALL_SNIPPET_MAX_CHARS ? '...' : '');
  }

  _truncateRecallDetail(content, maxChars = RECALL_DETAIL_MAX_CHARS) {
    const text = String(content || '');
    const parsed = Number.parseInt(maxChars, 10);
    const effectiveMax = Math.min(
      RECALL_DETAIL_MAX_CHARS,
      Math.max(1, Number.isFinite(parsed) ? parsed : RECALL_DETAIL_MAX_CHARS)
    );
    const truncated = text.length > effectiveMax;
    return {
      content: truncated ? `${text.substring(0, effectiveMax)}\n...[recall detail truncated]` : text,
      content_length: text.length,
      returned_content_length: truncated ? effectiveMax : text.length,
  ...
  async executeRecall(params, context, display) {
    const startTime = Date.now();
    const { mode, action, topic_id, message_id, keyword, max_chars } = params;
    const { start, count } = this._normalizeRecallWindow(params.start, params.count);
    const userId = context.userId || context.user_id;
    ...
    try {
      ...
      if (mode === 'messages') {
        ...
        if (action === 'detail') {
          ...
          return await this.recallMessageDetail(message_id, userId, display, startTime, max_chars);
        }
      }
  ...
  async recallTopicList(context, userId, start, count, display, startTime) {
    ...
    // 获取话题（不限制状态），使用 DB offset 实现分页
    const [topics, total_count] = await Promise.all([
      memorySystem.getTopics(userId, count, null, start),
      memorySystem.countTopics ? memorySystem.countTopics(userId, null) : Promise.resolve(null),
    ]);
    logger.info(`[ToolManager] recall topic list: 查询到 ${topics?.length || 0} 个话题`);
    const pagination = this._buildRecallPagination(start, count, topics?.length || 0, total_count);
  ...
  async recallTopicSearch(context, userId, keyword, start, count, display, startTime) {
    ...
    // 搜索话题，使用 DB offset 实现分页
    const [topics, total_count] = await Promise.all([
      memorySystem.searchTopics(userId, keyword, count, start),
      memorySystem.countSearchTopics ? memorySystem.countSearchTopics(userId, keyword) : Promise.resolve(null),
    ]);
  ...
  async recallTopicMessages(topicId, userId, start, count, display, startTime) {
    ...
    const pagination = this._buildRecallPagination(start, count, messages.length, total_count);
    ...
    return {
      ...
      data: {
        ...
        messages: messages.map(m => ({
          id: m.id,
          role: m.role,
          content: this._truncateRecallSnippet(m.content),
          has_full_content: (m.content?.length || 0) > RECALL_SNIPPET_MAX_CHARS,
   ...
   * 获取单条消息明细（完整内容）
   */
  async recallMessageDetail(messageId, userId, display, startTime, maxChars = RECALL_DETAIL_MAX_CHARS) {
    const Message = this.db.getModel('message');
    if (!Message) {
      return {
        success: false,
    ...
    const detail = this._truncateRecallDetail(fullContent, maxChars);
    ...
    return {
      ...
      data: {
        ...
        returned_content_length: detail.returned_content_length,
  ...
  /**
   * 执行 erix 原生 Notes 工具（note_take/note_read/note_list/note_forget）
   *
   * 执行走 erix toolSet（createBuiltinNotesTools），存储为 ErixNotesStoreAdapter：
   * - scopeRef 由 userId+expertId 稳定派生（跨请求工作记忆）；
   * - note_forget 在 adapter 层转为物理删除，结果归一化 {status:"deleted"}；
   * - note_take ≤4000 字符、relevance 非法返回 invalid（不静默 clamp）由 erix 原生保证。
   *
   * @param {string} toolId - 工具 ID（仅 erix 原生 note_*）
   * @param {object} params - 工具参数
   * @param {object} context - 执行上下文
   * @param {string} display - 工具显示名称
   * @returns {Promise<object>} 执行结果
   */
  async executeErixNotesTool(toolId, params, context, display) {
    const startTime = Date.now();
    const userId = context.userId || context.user_id;
    const expertId = context.expertId || context.expert_id;
    ...
    try {
      ...
      if (!userId || !expertId) {
        return {
          success: false,
          error: 'Notes tools require userId and expertId in context',
          toolId,
          toolName: display,
        };
      }

      const toolSet = await this._getNotesToolSet(userId, expertId);
      const raw = await toolSet.executeTool(toolId, params, {});

      let parsed;
      try {
        parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
      } catch {
        // erix registry 对 schema 校验失败（如 content 超 4000）返回纯文本错误，非 JSON
        logger.warn(`[ToolManager] Notes 工具返回非 JSON 结果: ${toolId}`, String(raw).slice(0, 200));
        return {
          success: false,
          error: typeof raw === 'string' ? raw : 'invalid notes response',
          toolId,
          toolName: display,
          duration: Date.now() - startTime,
        };
      ...
      // note_forget 归一化：adapter 已做物理删除，revoked 墓碑语义对 LLM 呈现为 deleted
      if (toolId === 'note_forget' && parsed && parsed.status === 'revoked') {
        parsed = {
          ...parsed,
          status: 'deleted',
          next: '该记录已物理删除，不可恢复',
      ...
      if (parsed && parsed.status === 'invalid') {
        logger.warn(`[ToolManager] Notes 工具返回 invalid: ${toolId}`, parsed.reason);
        return {
          success: false,
          error: parsed.reason || 'invalid notes request',
          data: parsed,
          toolId,
          toolName: display,
          duration,
  ...
  async _handleSearchDocumentsByMetadata(params, context, display) {
    ...
    try {
      ...
      const documents = (result.documents || []).map(d => ({
        ...
        current_revision_label: d.revision_label ?? null,
  ...
  async executeResidentTool(scriptPath, skillId, params, context, display, toolId) {
    ...
    if (capability?.kind !== CAPABILITY_KINDS.RESIDENT) {
      return {
        success: false,
        error: `Resident capability not registered: ${toolId}`,
        toolId,
        toolName: display,
      };
    }
  ...
  async executeMcpTool(toolId, params, context, display) {
    ...
    const mcpToolInfo = this.mcpToolRegistry.get(toolId);
    const capability = this.capabilityRegistry.get(toolId);
    if (!mcpToolInfo || capability?.kind !== CAPABILITY_KINDS.MCP) {
      return {
        success: false,
        error: `MCP capability not registered: ${toolId}`,
        toolId,
        toolName: display,
      };
    }
