...
class ToolManager {
  ...
  async executeTool(toolId, params, context = {}) {
    ...
    try {
      ...
      // 2. 其次使用完整路径（fullWorkspacePath）- 兼容旧字段
      // 3. 无任务时：使用用户 temp 目录
      ...
      } else if (taskContext?.fullWorkspacePath) {
        // 兼容旧字段：如果是绝对路径直接使用，否则报错
        if (path.isAbsolute(taskContext.fullWorkspacePath)) {
          workingDirectory = taskContext.fullWorkspacePath;
          logger.info(`[ToolManager] 使用任务工作目录(绝对路径 from fullWorkspacePath): ${workingDirectory}`);
        } else {
          // 旧字段是逻辑路径，不再自动转换，抛��错误
          logger.error(`[ToolManager] fullWorkspacePath 应该是绝对路径，收到: ${taskContext.fullWorkspacePath}`);
          throw new Error(`工作目录必须是绝对路径，请使用 absolute_workspace_path 字段`);
        }
      } else if (userId) {
  ...
  async executeMcpTool(toolId, params, context, display) {
    ...
    try {
      ...
      } else if (context.taskContext?.fullWorkspacePath) {
        if (path.isAbsolute(context.taskContext.fullWorkspacePath)) {
