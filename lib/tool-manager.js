...
class ToolManager {
    ...
    try {
      ...
      const result = await this.skillLoader.executeSkillTool(
        ...
        params,
        {
          ...
          isAdmin: context?.session?.isAdmin || context?.is_admin || false,  // 从 session 读取管理员标识
        },
   ...
   */
  getUserRole(context) {
    // 优先从 context.session.roles 数组中获取最高权限角色
    const roles = context?.session?.roles || context?.roles || [];
    ...
    // 兼容旧的 is_admin 标识（从 session 或 context 读取）
    if (context?.session?.isAdmin || context?.is_admin) {
      return 'admin';
    }
