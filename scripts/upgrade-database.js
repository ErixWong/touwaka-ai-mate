...
const MIGRATIONS = [
  ...
  // ==================== 合同管理扩展表 ====================
  // Issue #645: 合同管理 App 元数据结构优化
  // 创建 app_contract_rows 扩展表（需索引字段）
  {
    ...
    migrate: async (conn) => {
      ...
      `);
      console.log('  ✓ Created app_contract_rows table (without FK)');
  ...
  // 创建 app_document_content 内容表（OCR 中间结果）
  {
    ...
    migrate: async (conn) => {
      ...
      await conn.execute(`
        CREATE TABLE IF NOT EXISTS app_document_content (
          ...
          created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
          updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='文档内容表（OCR中间结果）'
      `);
      console.log('  ✓ Created app_document_content table (without FK)');
