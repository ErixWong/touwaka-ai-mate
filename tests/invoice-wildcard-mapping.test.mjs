/**
 * Invoice-Mgr wildcard 路由映射验证脚本
 * 复制 app-wildcard-router.js 的核心匹配逻辑，验证所有 Invoice-Mgr URL 正确落到 handler
 */
import fs from 'fs';
...
const cases = [
  ['GET', '/list', 'list.js'],
  ['GET', '/export', 'export.js'],
  ['POST', '/re-extract/ROW123', 're-extract.js'],
  ['POST', '/records', 'records.js'],
  ['GET', '/records/ROW123', 'records.js'],
  ['DELETE', '/records/ROW123', 'records.js'],
];
...
for (const [method, url, expected] of cases) {
  ...
  const result = resolveHandlerPath(appInternalPath, 'invoice-mgr');
