/**
 * Contract-Mgr wildcard 路由映射验证脚本
 * 复制 app-wildcard-router.js 的核心匹配逻辑，验证所有 Contract-Mgr URL 正确落到 handler
 */
import fs from 'fs';
...
const cases = [
  ['GET', '/records', 'records.js'],
  ['GET', '/records/ROW123', 'records.js'],
  ['POST', '/records/ROW123/confirm', 'records.js'],
  ['POST', '/records/ROW123/retry', 'records.js'],
  ['GET', '/status-summary', 'status-summary.js'],
];
...
for (const [method, url, expected] of cases) {
  ...
  const result = resolveHandlerPath(appInternalPath, 'contract-mgr');
